...
qboolean R_LoadMDXM( model_t *mod, void *buffer, const char *mod_name, qboolean &bAlreadyCached ) 
{
	...
	for ( l = 0 ; l < mdxm->numLODs ; l++)
	{
		...
		for ( i = 0 ; i < mdxm->numSurfaces ; i++)
		{
			...
			{
				Com_Error (ERR_DROP, "R_LoadMDXM: %s has more than %i verts on a surface (%i)",	mod_name, SHADER_MAX_VERTEXES, surf->numVerts );
			}
			if ( surf->numTriangles*3 > SHADER_MAX_INDEXES )
			{
				Com_Error (ERR_DROP, "R_LoadMDXM: %s has more than %i triangles on a surface (%i)",	mod_name, SHADER_MAX_INDEXES / 3, surf->numTriangles );
			}
