...
static void CG_DrawblockingMode(centity_t * cent, menuDef_t * menuHUD)
{
...
static void CG_Drawweapontype(centity_t * cent, menuDef_t * menuHUD)
{
	itemDef_t* focusItem;
...
{//render the CG_Drawgunfatigue meter.
	...
	if (cg.mishapHUDTotalFlashTime > cg.time || (cg.snap->ps.BlasterAttackChainCount > BLASTERMISHAPLEVEL_FULL))
	{
		...
		if (cg.mishapHUDNextFlashTime < cg.time)
		{
			...
			trap->S_StartSound(NULL, 0, CHAN_LOCAL, cgs.media.overheatgun);
	...
	value = cg.snap->ps.BlasterAttackChainCount;

	for (i = MAX_HUD_TICS - 1; i >= 0; i--)
	{
		focusItem = Menu_FindItemByName(menuHUD, mishapTics[i]);
...
{//render the balance/mishap meter.
	...
	value = cg.snap->ps.saberAttackChainCount;

	for (i = MAX_SJEHUD_TICS - 1; i >= 0; i--)
	{
		focusItem = Menu_FindItemByName(menuHUD, sabfatticS[i]);
...
static void CG_DrawblockPoints(int x, int y, menuDef_t * menuHUD)
{
	...
	hold = cg.snap->ps.fd.blockPoints - (BLOCK_POINTS_MAX / 2);
	...
	focusItem = Menu_FindItemByName(menuHUD, "blockpointamount");
	...
	if (cg.predictedPlayerState.pm_type != PM_SPECTATOR)
	{
		...
		if (menuHUD)
		{
			...
			if (cg.predictedPlayerState.pm_type != PM_SPECTATOR)
			{
				...
				if (cent->currentState.weapon == WP_SABER)
				{
					CG_DrawblockPoints(x, y, menuHUD);
				}
			}
		}
		else
		...
		if (menuHUD)
		{
			...
			if (cent->currentState.weapon == WP_SABER)
			{
				...
				CG_Drawsaberfatigue(menuHUD);
				CG_DrawblockingMode(cent, menuHUD);
			}
			else if (cent->currentState.weapon == WP_SABER && cent->currentState.saberHolstered)
			{
				CG_DrawForcePower(menuHUD);
				CG_Drawgunfatigue(menuHUD);
			...
			else if (cent->currentState.weapon == WP_MELEE)
			{
				CG_DrawForcePower(menuHUD);
				CG_Drawgunfatigue(menuHUD);
				CG_Drawweapontype(cent, menuHUD);
			}
			else
			{
				CG_DrawAmmo(cent, menuHUD);
...
static void CG_DrawCrosshair(vec3_t worldPoint, int chEntValid)
{
	...
	if (corona) // drawing extra bits
	{
		...
		trap->R_DrawStretchPic(x + cg.refdef.x + 0.5f * (640 - w), y + cg.refdef.y + 0.5f * (480 - h), w, h, 0, 0, 1, 1, cgs.media.forceCoronaShader);
	}
...
static void CG_DrawCrosshairNames(void)
{
	...
	{
		UI_DrawProportionalString(320, 170, "Press Use to Interact", UI_CENTER, colorTable[CT_WHITE], FONT_SMALL2);
		return;
	}
	...
	else if (cg_entities[cg.crosshairClientNum].currentState.eType == ET_NPC)
	{
		...
		case CLASS_PROTOCOL:
			...
			break;
		case CLASS_R2D2:
			CG_DrawSmallStringColor(320 - w / 2, 170, va("R2D2"), colorTable[CT_YELLOW]);
			break;
		case CLASS_PROBE:
			CG_DrawSmallStringColor(320 - w / 2, 170, va("Probe Droid"), colorTable[CT_YELLOW]);
			break;
		case CLASS_R5D2:
			CG_DrawSmallStringColor(320 - w / 2, 170, va("R5D2"), colorTable[CT_YELLOW]);
			break;
		case CLASS_DROIDEKA:
	...
	else
	{
		...
		else if (cg_entities[cg.crosshairClientNum].currentState.eType == ET_ITEM)
		{
			UI_DrawProportionalString(320, 170, "Press Use to Interact", UI_CENTER, colorTable[CT_WHITE], FONT_SMALL2);
			return;
		}
		else
		{
			...
			else
			{
				...
				{
					CG_DrawSmallStringColor(320 - w / 2, 170, name, colorTable[CT_VDKORANGE]);
				}
			}
		}
	}
...
//
//		if (cent->currentState.health > 1)
//		{
//			VectorCopy(cent->lerpOrigin, pos);
//			pos[2] += cent->currentState->maxs[2] + HEALTH_BAR_HEIGHT + 8;
