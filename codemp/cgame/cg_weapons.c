...
static float	CG_MachinegunSpinAngle(centity_t *cent)
{
	...
	if (cent->pe.barrelSpinning == !(cent->currentState.eFlags & EF_FIRING))
	{
		...
		{
			trap->S_StartSound(NULL, cent->currentState.number, CHAN_WEAPON, trap->S_RegisterSound("sound/weapons/barrelSpinning/barrelSpinning.wav"));
		}
		else
		{
			trap->S_StartSound(NULL, cent->currentState.number, CHAN_WEAPON, trap->S_RegisterSound("sound/weapons/barrelSpinning/barrelspindown.wav"));
		}		
	}
