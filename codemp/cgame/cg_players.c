...
sfxHandle_t	CG_CustomSound(int clientNum, const char* soundName)
{
	...
	for (i = 0; i < MAX_CUSTOM_SOUNDS; i++)
	{
		...
		else if (i < numCSiegeSounds && !strcmp(lSoundName, bg_customSiegeSoundNames[i]))
		{ //siege only
		...
		else if (clientNum >= MAX_CLIENTS && i < numCCallSounds && !strcmp(lSoundName, cg_customCalloutSoundNames[i]))
		{ //npc only
...
static void CG_InitG2SaberData(int saberNum, clientInfo_t * ci)
{
	...
	trap->G2API_InitGhoul2Model(&ci->ghoul2HolsterWeapons[saberNum], ci->saber[saberNum].model, 0, ci->saber[saberNum].skin, 0, 0, 0);

	if (ci->ghoul2HolsterWeapons[saberNum])
	{
		if (ci->saber[saberNum].skin)
		{
			trap->G2API_SetSkin(ci->ghoul2HolsterWeapons[saberNum], 0, ci->saber[saberNum].skin, ci->saber[saberNum].skin);
		}
	}
}
...
extern int CheckAnimFrameForEventType(animevent_t * animEvents, int keyFrame, animEventType_t eventType);
...
static void CG_SetLerpFrameAnimation(centity_t * cent, clientInfo_t * ci, lerpFrame_t * lf, int newAnimation, float animSpeedMult, qboolean torsoOnly, qboolean flipState)
{
	...
	if (cent->ghoul2)
	{
		...
		{//make me parry
			if (Manual_Saberreadyanim(newAnimation))
			{
				blendTime *= 1.8f;
			}
			if (Manual_Saberblockinganim(newAnimation))
			{
				blendTime *= 1.8f;
			}
		...
		PM_SaberStartTransAnim(cent->currentState.number, cent->currentState.fireflag, cent->currentState.weapon, newAnimation, &animSpeed, cent->currentState.brokenLimbs, cent->currentState.userInt3);
...
static void CG_PlayerAnimation(centity_t * cent, int* legsOld, int* legs, float* legsBackLerp, int* torsoOld, int* torso, float* torsoBackLerp)
{
	...
	if (!PM_RunningAnim(cent->currentState.legsAnim) &&
		...
		(!cent->currentState.ManualBlockingFlags& (1 << MBF_BLOCKING) || cent->currentState.ManualBlockingFlags& (1 << MBF_BLOCKING))))
...
static void CG_G2PlayerAngles(centity_t * cent, matrix3_t legs, vec3_t legsAngles)
{
	...
	{ //don't do these things on non-humanoids
		...
		BG_G2PlayerAngles(cent->ghoul2, ci->bolt_motion, &cent->currentState, cg.time,
			...
			lookAngles, ci->lastHeadAngles, ci->lookTime, emplaced, &ci->superSmoothTime, cent->currentState.ManualBlockingFlags);
...
static void CG_PlayerPowerups(centity_t * cent, refEntity_t * torso)
{
	...
	//------------------------------------------------------
	if (powerups & (1 << PW_GALAK_SHIELD) && cent->currentState.NPC_class == CLASS_GALAKMECH)
	{
		//	int bolt;
		//	refEntity_t tent;

		//	assert(cent->ghoul2);
		//	bolt = trap->G2API_AddBolt(cent->ghoul2, 0, "thoracic");
		//	assert(bolt > -1);
		...
		//	trap->G2API_GetBoltMatrix(cent->ghoul2, 0, bolt, &boltMatrix, angles, cent->lerpOrigin, cg.time, cgs.gameModels, cent->modelScale);
		//	BG_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, tent.origin);// pass in the emitter origin here

		//	tent.endTime = cent->fx_time + 1000;			// if you want the shell to build around the guy, pass in a time that is 1000ms after the start of the turn-on-effect
		//	tent.customShader = trap->R_RegisterShader("gfx/effects/irid_shield");

		//	trap->R_AddRefEntityToScene(&cent);
	}
}
...
void CG_DoCustomSaber(vec3_t blade_muz, vec3_t blade_tip, vec3_t trail_tip, vec3_t trail_muz, float lengthMax, float radius, saber_colors_t color, int rfx, qboolean doLight, qboolean doTrail, int cnum, int bnum)
{
...
void CG_DoTFASaber(vec3_t blade_muz, vec3_t blade_tip, vec3_t trail_tip, vec3_t trail_muz, float lengthMax, float radius, saber_colors_t color, int rfx, qboolean doLight, qboolean doTrail, int cnum, int bnum)
{
...

	{
		...
		{// Add the saber surface that provides color.
			...
			sbak.customShader = cgs.media.rgbTFASaberCoreShader;
			saber.reType = RT_LINE;
			saber.shaderTexCoord[0] = saber.shaderTexCoord[1] = 1.0f;
			saber.shaderRGBA[0] = saber.shaderRGBA[1] = saber.shaderRGBA[2] = saber.shaderRGBA[3] = 0xff;
			saber.radius = coreradius;
...
void CG_DoBattlefrontSaber(vec3_t blade_muz, vec3_t blade_tip, vec3_t trail_tip, vec3_t trail_muz, float lengthMax, float radius, saber_colors_t color, int rfx, qboolean doLight, qboolean doTrail, int cnum, int bnum)
{
	vec3_t	dif, mid, blade_dir, end_dir, trail_dir, base_dir;
	float	radiusmult, effectradius, coreradius, effectalpha, AngleScale;
	float	blade_len, end_len, trail_len, base_len, DisTip, DisMuz, DisDif;
...
void CG_DoOTSaber(vec3_t blade_muz, vec3_t blade_tip, vec3_t trail_tip, vec3_t trail_muz, float lengthMax, float radius, saber_colors_t color, int rfx, qboolean doLight, qboolean doTrail, int cnum, int bnum)
{
	...
	effectradius = ((radius * 1.6f * v1) + Q_flrand(0.3f, 1.8f)) * radiusmult * cg_SFXSabersGlowSize.value;
	coreradius = ((radius * 0.4f * v2) + Q_flrand(-1.0f, 1.0f)) * radiusmult * cg_SFXSabersCoreSize.value;
...
void CG_DoSFXSaber(vec3_t blade_muz, vec3_t blade_tip, vec3_t trail_tip, vec3_t trail_muz, float lengthMax, float radius, saber_colors_t color, int rfx, qboolean doLight, qboolean doTrail, int cnum, int bnum)
{
	...
	if (base_len > 2)
	{
		...
		{// Add the saber surface that provides color.
			...
			sbak.customShader = cgs.media.sfxSaberBladeShader;
			saber.reType = RT_LINE;
			saber.shaderTexCoord[0] = saber.shaderTexCoord[1] = 1.0f;
			saber.shaderRGBA[0] = saber.shaderRGBA[1] = saber.shaderRGBA[2] = saber.shaderRGBA[3] = 0xff;
			saber.radius = coreradius;
	...
	while (!doneWithTraces)
	{
		...
		{ //hit a client..
			...
			{ //it succeeded with the ghoul2 trace
				...
				trap->S_StartSound(trace.endpos, trace.entityNum, CHAN_AUTO, trap->S_RegisterSound(va("sound/weapons/saber/saberhit%i.wav", Q_irand(1, 7))));
			}
		}
					...
					&& cg.time < saberTrail->lastTime + 2000) // if we have a stale segment, don't draw until we have a fresh one
				{
...
#endif
					{
						...
						{ //don't draw it if the last time is way out of date
							...
							{
								if (client->saber[saberNum].type == SABER_SITH_SWORD
									|| (!WP_SaberBladeUseSecondBladeStyle(&client->saber[saberNum], bladeNum) && client->saber[saberNum].trailStyle == 1)
									|| (WP_SaberBladeUseSecondBladeStyle(&client->saber[saberNum], bladeNum) && client->saber[saberNum].trailStyle2 == 1))
								{//motion trail
...
JustDoIt:
	...
	{// Draw the Raven blade.
		if (cent->currentState.botclass == BCLASS_UNSTABLESABER || client->saber[saberNum].type == SABER_UNSTABLE || client->saber[saberNum].type == SABER_STAFF_UNSTABLE || client->saber[saberNum].type == SABER_ELECTROSTAFF)
		{
			CG_DoTFASaber(fx.mVerts[0].origin, fx.mVerts[1].origin, fx.mVerts[2].origin, fx.mVerts[3].origin, (client->saber[saberNum].blade[bladeNum].lengthMax), (client->saber[saberNum].blade[bladeNum].radius), scolor, renderfx, (qboolean)(client->saber[saberNum].numBlades < 3 && !(client->saber[saberNum].saberFlags2 & SFL2_NO_DLIGHT)), (qboolean)(cg_saberTrail.integer > 0), cent->currentState.clientNum, saberNum);
		}
		...
		{
			CG_DoCustomSaber(fx.mVerts[0].origin, fx.mVerts[1].origin, fx.mVerts[2].origin, fx.mVerts[3].origin, (client->saber[saberNum].blade[bladeNum].lengthMax), (client->saber[saberNum].blade[bladeNum].radius), scolor, renderfx, (qboolean)(client->saber[saberNum].numBlades < 3 && !(client->saber[saberNum].saberFlags2 & SFL2_NO_DLIGHT)), (qboolean)(cg_saberTrail.integer > 0), cent->currentState.clientNum, saberNum);
		}
		else if ((cent->currentState.powerups & (1 << PW_CLOAKED)))
		{
			CG_DoSaber(org_, axis_[0], saberLen, client->saber[saberNum].blade[bladeNum].lengthMax, client->saber[saberNum].blade[bladeNum].radius, scolor, renderfx, (qboolean)(client->saber[saberNum].numBlades < 3 && !(client->saber[saberNum].saberFlags2 & SFL2_NO_DLIGHT)), cent->currentState.clientNum, saberNum);
		}
		else if (client->saber[saberNum].type == SABER_GRIE || client->saber[saberNum].type == SABER_GRIE4)
		{
			CG_DoBattlefrontSaber(fx.mVerts[0].origin, fx.mVerts[1].origin, fx.mVerts[2].origin, fx.mVerts[3].origin, (client->saber[saberNum].blade[bladeNum].lengthMax), (client->saber[saberNum].blade[bladeNum].radius), scolor, renderfx, (qboolean)(client->saber[saberNum].numBlades < 3 && !(client->saber[saberNum].saberFlags2 & SFL2_NO_DLIGHT)), (qboolean)(cg_saberTrail.integer > 0), cent->currentState.clientNum, saberNum);
		}
		else
		{
			CG_DoSaber(org_, axis_[0], saberLen, client->saber[saberNum].blade[bladeNum].lengthMax, client->saber[saberNum].blade[bladeNum].radius, scolor, renderfx, (qboolean)(client->saber[saberNum].numBlades < 3 && !(client->saber[saberNum].saberFlags2 & SFL2_NO_DLIGHT)), cent->currentState.clientNum, saberNum);
...
void CG_AddLightningBeam(vec3_t start, vec3_t end)
{
	...
	s1 = sin(cg.time * 0.005f) * 2.0f + Q_flrand(-1.0f, 1.0f) * 0.2f;
...
void SmoothTrueView(vec3_t eyeAngles)
{
	...
	if ((cg.predictedPlayerState.torsoAnim) == BOTH_A2_STABBACK1 || (cg.predictedPlayerState.torsoAnim) == BOTH_A2_STABBACK1B)
	{
		eyeRange = qfalse;
		DidSpecial = qtrue;
	}
				...
				{//using single saber(s)
					...
					{//only have holstered saber if we current have our saber in our posession.
						...
						{//have specialized holster surface tag for single sabers
							...
							{//we haven't bolted the saber to the model yet. Do it now.
								trap->G2API_CopySpecificGhoul2Model(CG_G2WeaponInstance(cent, WP_SABER), 0, cent->ghoul2, G2MODEL_SABER_HOLSTERED);
					...
					{//have a second saber
						...
						{//use specialized bolt.
							trap->G2API_CopySpecificGhoul2Model(ci->ghoul2Weapons[1], 0, cent->ghoul2, G2MODEL_SABER2_HOLSTERED);
					...
					{//only have holstered saber if we current have our saber in our posession.
						...
						{//only use if you're not wearing a jetpack.
							...
							{//have specialized holster surface tag for single sabers
								if (!ci->saber_holstered)
								{//we haven't bolted the saber to the model yet. Do it now.
									trap->G2API_CopySpecificGhoul2Model(CG_G2WeaponInstance(cent, WP_SABER), 0, cent->ghoul2, G2MODEL_STAFF_HOLSTERED);
		...
		{//render WP_BRYAR_PISTOL on right hip
			...
			{//have specialized bolt
				...
				{//don't already have the WP_BRYAR_PISTOL bolted.
					...
					trap->G2API_CopySpecificGhoul2Model(CG_G2WeaponInstance(cent, WP_BRYAR_PISTOL), 0, cent->ghoul2, G2MODEL_BLASTER_HOLSTERED);
					trap->G2API_SetBoltInfo(cent->ghoul2, G2MODEL_BLASTER_HOLSTERED, ci->holster_blaster);
		...
		{//render WP_BRYAR_OLD on right hip
			...
			{//have specialized bolt
				...
				{//don't already have the WP_BRYAR_OLD bolted.
					...
					trap->G2API_CopySpecificGhoul2Model(CG_G2WeaponInstance(cent, WP_BRYAR_OLD), 0, cent->ghoul2, G2MODEL_BLASTER_HOLSTERED);
					trap->G2API_SetBoltInfo(cent->ghoul2, G2MODEL_BLASTER_HOLSTERED, ci->holster_blaster);
		...
		{//render concussion on right hip
			...
			{//have specialized bolt
				...
				{//don't already have the concussion bolted.
					...
					trap->G2API_CopySpecificGhoul2Model(CG_G2WeaponInstance(cent, WP_CONCUSSION), 0, cent->ghoul2, G2MODEL_LAUNCHER_HOLSTERED);
		...
		{//render weapon on back
			...
			{//have specialized bolt
				...
				{//don't already have the concussion bolted.
					...
					//now bolt the weapon
					trap->G2API_CopySpecificGhoul2Model(CG_G2WeaponInstance(cent, WP_REPEATER), 0, cent->ghoul2, G2MODEL_LAUNCHER_HOLSTERED);
					trap->G2API_SetBoltInfo(cent->ghoul2, G2MODEL_LAUNCHER_HOLSTERED, ci->holster_launcher);
		...
		{//render weapon on back
			...
			{//have specialized bolt
				...
				{//don't already have the weapon bolted.
					...
					//now bolt the weapon
					trap->G2API_CopySpecificGhoul2Model(CG_G2WeaponInstance(cent, WP_FLECHETTE), 0, cent->ghoul2, G2MODEL_LAUNCHER_HOLSTERED);
					trap->G2API_SetBoltInfo(cent->ghoul2, G2MODEL_LAUNCHER_HOLSTERED, ci->holster_launcher);
		...
		{//render weapon on back
			...
			{//have specialized bolt
				...
				{//don't already have the weapon bolted.
					...
					//now bolt the weapon
					trap->G2API_CopySpecificGhoul2Model(CG_G2WeaponInstance(cent, WP_DISRUPTOR), 0, cent->ghoul2, G2MODEL_LAUNCHER_HOLSTERED);
					trap->G2API_SetBoltInfo(cent->ghoul2, G2MODEL_LAUNCHER_HOLSTERED, ci->holster_launcher);
		...
		{//render weapon on back
			...
			{//have specialized bolt
				...
				{//don't already have the weapon bolted.
					...
					//now bolt the weapon
					trap->G2API_CopySpecificGhoul2Model(CG_G2WeaponInstance(cent, WP_BOWCASTER), 0, cent->ghoul2, G2MODEL_LAUNCHER_HOLSTERED);
					trap->G2API_SetBoltInfo(cent->ghoul2, G2MODEL_LAUNCHER_HOLSTERED, ci->holster_launcher);
		...
		{//render demp 2 on right hip
			...
			{//have specialized bolt
				...
				{//don't already have the demp2 bolted.
					...
					//now add the demp2
					trap->G2API_CopySpecificGhoul2Model(CG_G2WeaponInstance(cent, WP_DEMP2), 0, cent->ghoul2, G2MODEL_LAUNCHER_HOLSTERED);
					trap->G2API_SetBoltInfo(cent->ghoul2, G2MODEL_LAUNCHER_HOLSTERED, ci->holster_launcher);
...
void CG_Player(centity_t * cent)
{
	...
	if ((cent->currentState.botclass == BCLASS_BOBAFETT ||
		...
		cent->currentState.botclass == BCLASS_MANDOLORIAN2) && (cent->currentState.eFlags2 & EF2_FLYING) && !(cent->currentState.eFlags & EF_DEAD))
	{
		mdxaBone_t	mat;
		vec3_t	flamePos, flameDir;
		qboolean	wj = qfalse;
		fxHandle_t	flameEffect = (cent->currentState.botclass == BCLASS_BOBAFETT ||
			...
			cent->currentState.botclass == BCLASS_MANDOLORIAN ||
			cent->currentState.botclass == BCLASS_MANDOLORIAN1 ||
			cent->currentState.botclass == BCLASS_MANDOLORIAN2) ? cgs.effects.mBobaJet : cgs.effects.mBobaJet;
		...
		{
			//1st flamme
			trap->G2API_GetBoltMatrix(cent->ghoul2, 0, cent->bolt_jet1, &mat, cent->turAngles, cent->lerpOrigin, cg.time, cgs.gameModels, cent->modelScale);
			BG_GiveMeVectorFromMatrix(&mat, ORIGIN, flamePos);
			...
			//2nde flamme
			trap->G2API_GetBoltMatrix(cent->ghoul2, 0, cent->bolt_jet2, &mat, cent->turAngles, cent->lerpOrigin, cg.time, cgs.gameModels, cent->modelScale);
			BG_GiveMeVectorFromMatrix(&mat, ORIGIN, flamePos);
		...
		trap->S_AddLoopingSound(cent->currentState.number, cent->lerpOrigin, vec3_origin, trap->S_RegisterSound("sound/chars/boba/jethover"));
	}
...
SkipTrueView:
	...
	if (cent->currentState.userInt3 & (1 << FLAG_FLAMETHROWER) || cg.snap->ps.forceHandExtend == HANDEXTEND_FLAMETHROWER_HOLD)
	...
	if (cent->currentState.eFlags & EF_INVULNERABLE || cent->currentState.powerups & (1 << PW_INVINCIBLE))
	{
