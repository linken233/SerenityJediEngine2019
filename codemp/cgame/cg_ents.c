...
static void CG_General(centity_t *cent)
{
	...
	if (cent->currentState.weapon == WP_MELEE)
	{
		...
		trap->G2API_GetBoltMatrix(parent->ghoul2, 0, 0, &mat, parent->turAngles, parent->lerpOrigin, cg.time, cgs.gameModels, parent->modelScale);
	...
	*/
		...
		if (cent->bodyFadeTime > cg.time)
		{
			...
			if (curTimeDif < 3400)
			{
				if (lightSide)
				{
					if (curTimeDif < 2200)
					{ //probably temporary
						trap->S_StartSound(NULL, cent->currentState.number, CHAN_AUTO, trap->S_RegisterSound("sound/weapons/saber/saberhum1"));
					}
				}
				else
	...
	if ((cent->currentState.eFlags & EF_FORCE_VISIBLE) && (cg.snap->ps.fd.forcePowersActive & (1 << FP_SEE)))
	{
		...
		{	// A team game
			if (cg.predictedPlayerState.persistant[PERS_TEAM] == TEAM_SPECTATOR || cg.predictedPlayerState.persistant[PERS_TEAM] == TEAM_FREE)
			{//yellow
				ent.shaderRGBA[0] = 255;
	...
	*/
	...
	if (!cg_simpleItems.integer)
	{
		...
		if (item->giType == IT_POWERUP)
		{
			...
			if (item->giTag == PW_FORCE_ENLIGHTENED_LIGHT || item->giTag == PW_FORCE_BOON || item->giTag == PW_GALAK_SHIELD)
			{
...
===============
*/
extern void CG_AddSaberBlade(centity_t *cent, centity_t *scent, refEntity_t *saber, int renderfx, int modelIndex, int saberNum, int bladeNum, vec3_t origin, vec3_t angles, qboolean fromSaber, qboolean dontDraw);
...
static void CG_Missile(centity_t *cent)
{
	...
	if (s1->weapon == WP_MELEE)
	{
		...
		trap->G2API_GetBoltMatrix(parent->ghoul2, 1, 0, &mat, parent->turAngles, parent->lerpOrigin, cg.time, cgs.gameModels, parent->modelScale);
		BG_GiveMeVectorFromMatrix(&mat, ORIGIN, start);
		if (cent->currentState.eType == ET_GRAPPLE)
		{
			CG_GrappleLine(start, end, 1, color, 1);
	...
	if (s1->weapon == WP_SABER && ((s1->owner == cg.snap->ps.clientNum)))
