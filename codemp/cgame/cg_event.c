...
void CG_EntityEvent(centity_t * cent, vec3_t position)
{
	...
	case EV_SABERLOCK:
		DEBUGNAME("EV_SABERLOCK");
		{
			...
			if (client && client->infoValid && enemyClient && enemyClient->infoValid)
			{
				...
				//get their blade.
				trap->G2API_GetBoltMatrix(enemy->ghoul2, 1, 0, &boltMatrix, enemy->lerpAngles, enemy->lerpOrigin, cg.time, cgs.gameModels, enemy->modelScale);
	...
	case EV_CRYOBAN:
		...
		{
			vec3_t up = { 0,0,1 };
			trap->FX_PlayEffectID(trap->FX_RegisterEffect("cryoban/stuck"), cent->playerState->origin, up, -1, -1, qfalse);
		}
		break;
	case EV_STASIS:
		DEBUGNAME("EV_STASIS");
	...
	case EV_SABER_BLOCK:
		DEBUGNAME("EV_SABER_BLOCK");
		{
			...
			{ //saber block
				...
				int			perfectblockFXID = cgs.effects.mSaberprfectBlock;
				qhandle_t	blockSound = trap->S_RegisterSound(va("sound/weapons/saber/saberblock%d", Q_irand(1, 9)));
				qhandle_t	knockSound = trap->S_RegisterSound(va("sound/weapons/saber/saberknock%d", Q_irand(1, 4)));
					...
					if (client && client->infoValid)
					{
						...
						{//use second blade style values
							...
							{//custom hit sound
								...
								{
									knockSound = client->saber[saberNum].block2Sound[Q_irand(0, 2)];
								}
							}
						}
						else
						{
							...
							{//custom saber block effect
								perfectblockFXID = client->saber[saberNum].blockEffect;
							}
							if (client->saber[saberNum].blockSound[0])
							{//custom hit sound
								if (Q_irand(0, 1))
								{
									knockSound = client->saber[saberNum].blockSound[Q_irand(0, 2)];
								}
								else
								{
			...
			{ //projectile block
				...
				if (Q_irand(0, 1))
				{
					trap->FX_PlayEffectID(cgs.effects.mBlasterDeflectpassthrough, es->origin, fxDir, -1, -1, qfalse);
				}
				else
				{
	...
	case EV_DISRUPTOR_MAIN_SHOT:
		...
		{
			if (!cg.renderingThirdPerson && (cg.lastFPFlashPoint[0] || cg.lastFPFlashPoint[1] || cg.lastFPFlashPoint[2]))
