...
extern qboolean BG_GunInAttack(int anim);
...
int PM_ReadyPoseForSaberAnimLevel(void)
{
	...
	{//dual sabers, both on
		...
		else
		{
			...
			{
				return BOTH_SABERDUAL_STANCE_ALT;
			}
			else
			{
				return BOTH_SABERDUAL_STANCE_IDLE;
			}
		}
	}
	...
	case SS_DUAL:
		if ((pm->cmd.buttons & BUTTON_BLOCK) && !(pm->cmd.buttons & BUTTON_ATTACK))
		{
			...
			else if (saber1->type == SABER_BACKHAND)//saber backhand
			{
			...
			else if (saber1->type == SABER_GRIE)//saber
			{
				anim = BOTH_SABERDUAL_STANCE_GRIEVOUS;
			}
			else if (saber1->type == SABER_GRIE4)//saber
			{
				anim = BOTH_SABERDUAL_STANCE_GRIEVOUS;
			}
			else
		...
		else
		{
			if (pm->ps->fd.blockPoints < BLOCKPOINTS_FULL)
			{
				if (saber1->type == SABER_OBIWAN)//saber obi
				{
					anim = BOTH_SABEROBI_STANCE;
				}
				else if (saber1->type == SABER_REY)//saber backhand
				{
			...
			else if (saber1->type == SABER_UNSTABLE)//saber kylo
			{
	...
	case SS_STAFF:
		...
		else if (pm->ps->fd.blockPoints < BLOCKPOINTS_FULL)
		{
			if (saber1->type == SABER_OBIWAN)//saber obi
			{
				anim = BOTH_SABEROBI_STANCE;
	...
	case SS_MEDIUM:
		if ((pm->cmd.buttons & BUTTON_BLOCK) && !(pm->cmd.buttons & BUTTON_ATTACK))
		{
			...
			else if (saber1->type == SABER_WINDU)//saber backhand
			{
	...
	default:
		...
		else
		{
			...
			else if (saber1->type == SABER_OBIWAN)//saber obi
			{
				anim = BOTH_SABEROBI_STANCE;
			}
			else if (saber1->type == SABER_REY)//saber backhand
...
int PM_ReadyPoseForSaberAnimLevelBOT(void)
{
	...
	case SS_MEDIUM:
		...
		else if (saber1->type == SABER_WINDU)//saber obi
		{
			anim = BOTH_SABEREADY_STANCE;
		}
		else
	...
	default:
		...
		else if (saber1->type == SABER_YODA)
		{
			anim = BOTH_SABERYODA_STANCE;
		}
		else if (saber1->type == SABER_UNSTABLE)//saber kylo
...
static void PM_Friction(void)
{
	...
	float	drop, friction = pm_friction;
...
#endif
								//G_PlayBoltedEffect(G_EffectIndex("materials/sand_large.efx"), pm->ps, "*r_leg_foot");
								//pm->ps->fd.forceJumpSound = 0;
							}
						}
					}
					control = speed < pm_stopspeed ? pm_stopspeed : speed;
	...
	{//hugging wall, getting ready to jump off
		...
		{//jump off
			...
			else
			{
				...
				{
					PM_SetAnim(SETANIM_LEGS, BOTH_FORCEJUMP2, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD | SETANIM_FLAG_RESTART);
				}
				else
				{
...
qboolean PM_InForceFall()
{
	...
	if (PM_CanForceFall())
	{
		...
		if (pm->ps->fd.forcePowerDebounce[FP_LEVITATION] < pm->cmd.serverTime)
		{
			...
			WP_ForcePowerDrain(pm->ps, FP_HEAL, FM_FORCEFALL + ForceManaModifier);
...
static qboolean PM_CheckJump(void)
{
	...
	else if (pm->waterlevel < 3)
	{
		...
		{//can't do this in zero-G
			...
			{//in the middle of a force long-jump
				...
				&& !(pm->ps->pm_flags & PMF_JUMP_HELD)
				&& (pm->ps->fd.forcePowerDebounce[FP_SPEED]) <= 500//have to have just started the force speed within the last half second
				&& pm->ps)
		...
		BG_CanUseFPNow(pm->gametype, pm->ps, pm->cmd.serverTime, FP_LEVITATION))
	{
		...
		{//in the air
			...
			else if (pm->ps->legsAnim == BOTH_FORCEWALLRUNFLIP_START)
			{
				...
				&& (pm->ps->legsAnim == BOTH_JUMP1 || pm->ps->legsAnim == BOTH_JUMP2 || pm->ps->legsAnim == BOTH_INAIR1))//not in a flip or spin or anything
	...
	if (pm->ps->groundEntityNum == ENTITYNUM_NONE)
	{
		if (!(pm->waterlevel == 1 && pm->cmd.upmove > 0 && pm->ps->fd.forcePowerLevel[FP_LEVITATION] == FORCE_LEVEL_3 && pm->ps->fd.forcePower >= 5))
		{
			return qfalse;
		}
		else
...
static void PM_AirMove(void)
{
	...
	{ //reduced air control while not jetting
		...
		{//some x/y movement
			...
			{
				if (i == 2)
					wishvel[i] = scale * pml.forward[i] * pm->cmd.forwardmove + scale * pml.right[i] * pm->cmd.rightmove * 1.6;
				else
					wishvel[i] = scale * pml.forward[i] * pm->cmd.forwardmove + scale * pml.right[i] * pm->cmd.rightmove * 1.2;
			}
...
void PM_JetPackAnim(void)
{
	...
	else if (pm->cmd.forwardmove > 0)
	{
		if (pm->ps->weapon == WP_SABER)//saber out
		{
			PM_ContinueLegsAnim(BOTH_FORCEJUMP2);
		}
		else
		{
...
static void PM_GroundTraceMissed(void)
{
	...
	else if (pm->ps->groundEntityNum != ENTITYNUM_NONE || (pm->ps->legsAnim) == BOTH_CHOKE3 || (pm->ps->legsAnim) == BOTH_CHOKE4)
	{
...
static void PM_GroundTrace(void)
{
	...
	if (pm->ps->velocity[2] > 0 && DotProduct(pm->ps->velocity, trace.plane.normal) > 10)
	{
		...
		else if (BG_KickingAnim(pm->ps->legsAnim))
		{
			//in kick
		}
		else if (pm->cmd.forwardmove >= 0)
...
static void PM_Footsteps(void)
{
	...
	if ((pm->ps->clientNum >= MAX_CLIENTS && pm_entSelf && pm_entSelf->s.NPC_class == CLASS_RANCOR)//does this catch NPCs, too?
		|| (pm->ps->clientNum >= MAX_CLIENTS && pm_entSelf && pm_entSelf->s.NPC_class == CLASS_WAMPA))//does this catch NPCs, too?
					...
					else
					{
						if (pm->ps->weapon != WP_SABER || !PM_AdjustStandAnimForSlope())
						{
							if (pm->ps->weapon == WP_SABER)
							{
								...
								{
									PM_ContinueLegsAnim(PM_LegsSlopeBackTransition(PM_ReadyPoseForSaberAnimLevel()));
								}
#endif
							}
							else
				...
				else
				{
					...
					case SS_STAFF:
						...
						{//blades off
							...
							{
								desiredAnim = BOTH_RUN10;
							}
							else if (saber1->type == SABER_UNSTABLE)//saber kylo
							{
								desiredAnim = BOTH_SPRINT;
							}
							else if (saber1->type == SABER_STAFF_UNSTABLE)//saber kylo
							{
								desiredAnim = BOTH_SPRINT;
							}
							else if (saber1->type == SABER_GRIE)//saber kylo
							{
								desiredAnim = BOTH_RUN7;
							}
							else if (saber1->type == SABER_GRIE4)//saber kylo
							{
								desiredAnim = BOTH_RUN7;
							}
							else
			...
			else
			{
				...
				{// add gun walk animas somehow
						...
						pm->ps->weapon == WP_TRIP_MINE)
					{
						if (!pm->ps->weaponTime)
						{
							PM_SetAnim(SETANIM_BOTH, BOTH_WALK10, SETANIM_FLAG_NORMAL);
						}
						else
						{
							desiredAnim = BOTH_WALK2;
	...
	if (weapon == WP_SABER)
	{
		...
		{//have saber(s)
			...
			else
			{
				...
				else if (saber1->type == SABER_DOOKU)//saber dooku
				{
					PM_SetAnim(SETANIM_TORSO, BOTH_DOOKU_SMALLDRAW, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
				}
				else if (saber1->type == SABER_UNSTABLE)//saber kylo
				...
				else if (saber1->type == SABER_OBIWAN)//saber kylo
				{
					PM_SetAnim(SETANIM_TORSO, BOTH_SHOWOFF_OBI, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
				}
				else if (saber1->type == SABER_SFX)//saber backhand
				{
					switch (pm->ps->fd.saberAnimLevel)
					{
					case SS_FAST:
				...
				{
					if (saber1->type == SABER_GRIE || saber1->type == SABER_GRIE4)//saber GRIEVOUS
					{
...
static void PM_Weapon(void)
{
	...
	{ //nothing else should be allowed to happen during this time, including weapon fire
		...
		case HANDEXTEND_CHOKE:
			...
			{
				desiredAnim = BOTH_CHOKE4; //arms waving
			}
			break;
		...
		case HANDEXTEND_TAUNT:
			...
			{
				desiredAnim = pm->ps->forceDodgeAnim;
				if (desiredAnim != BOTH_ENGAGETAUNT && desiredAnim != BOTH_VADERTAUNT
	...
	if (pm->ps->weaponstate == WEAPON_RAISING) {
		...
		if (PM_CanSetWeaponAnims())
		{
			if (pm->ps->weapon == WP_SABER)
			{
				...
				{
					PM_StartTorsoAnim(PM_ReadyPoseForSaberAnimLevel());
				}
#endif
			}
	...
	{//special anims for standard melee attacks
		...
		{ //if riding a vehicle don't do this stuff at all
			if ((pm->cmd.buttons & BUTTON_ATTACK) &&
				(pm->cmd.buttons & BUTTON_ALT_ATTACK) || (pm->cmd.buttons & BUTTON_KICK))
			{ //ok, grapple time
			...
			else if (pm->cmd.buttons & BUTTON_ALT_ATTACK && pm->ps->weapon == WP_MELEE)
			{ //kicks
				if (!BG_KickingAnim(pm->ps->torsoAnim) &&
					!BG_KickingAnim(pm->ps->legsAnim) &&
					!BG_InRoll(pm->ps, pm->ps->legsAnim))
			...
			else if (pm->cmd.buttons & BUTTON_KICK && pm->ps->weapon == WP_MELEE)
			{ //kicks
				if (!BG_KickingAnim(pm->ps->torsoAnim) &&
					!BG_KickingAnim(pm->ps->legsAnim) &&
					!BG_InRoll(pm->ps, pm->ps->legsAnim))
...
static void PM_Animate(void)
{
	if (pm->cmd.buttons & BUTTON_GESTURE)
	{
			...
			pm->ps->legsTimer < 1 && pm->ps->weaponTime < 1 && pm->ps->saberLockTime < pm->cmd.serverTime)
		{
			...
			{
				pm->ps->forceDodgeAnim = BOTH_VADERTAUNT;
			}
			else
			{
...
qboolean G_OkayToLean(playerState_t * ps, const usercmd_t * uscmd, qboolean interruptOkay)
{
	if ((ps->clientNum < MAX_CLIENTS)//player
		...
		&& !(uscmd->buttons & (BUTTON_ATTACK | BUTTON_ALT_ATTACK | BUTTON_FORCE_LIGHTNING | BUTTON_DASH | BUTTON_FORCE_DRAIN | BUTTON_FORCEGRIP))//not trying to attack
...
void PM_UpdateViewAngles(playerState_t * ps, const usercmd_t * cmd)
{
	...
	{	//only in the real pmove
		if ((cmd->buttons & BUTTON_USE) && !(cmd->buttons & BUTTON_WALKING) && !(pm->ps->pm_flags & PMF_DUCKED))
		{//check leaning
			if (G_OkayToLean(ps, &pm->cmd, qtrue))
			{//third person lean
			...
			else if ((!pm->ps->zoomMode) && pm->cmd.rightmove != 0 && cmd->rightmove != 0 && !cmd->forwardmove && cmd->upmove <= 0)
			...
			{//third person lean
					...
					&& !PM_WalkingOrRunningAnim(pm->ps->legsAnim))
				{
					...
					{//lean right
						...
						{
							if (ps->torsoAnim == BOTH_BLOCK_BACK_HOLD_R)
							{
								anim = ps->torsoAnim;
							}
							else
					...
					{//lean left
						if (saber1->type == SABER_STAFF || saber1->type == SABER_STAFF_SFX || saber1->type == SABER_STAFF_MAUL
							...
							|| saber1->type == SABER_PALP || saber1->type == SABER_ANAKIN)//saber staff
						{
							if (ps->torsoAnim == BOTH_BLOCK_BACK_HOLD_L)
							{
								anim = ps->torsoAnim;
							}
							else
				...
				else if (!cmd->rightmove && (cmd->buttons & BUTTON_ATTACK))
				{
					switch (pm->ps->fd.saberAnimLevel)
					{
					case SS_STAFF:
...
#ifdef _GAME
	if (!(m_nerf.integer & (1 << EOC_SPINATTACK)) && (ps->saberMove == LS_SPINATTACK_DUAL || ps->saberMove == LS_SPINATTACK || ps->saberMove == LS_SPINATTACK_GRIEV))
#else
	if (!(cgs.m_nerf & (1 << EOC_SPINATTACK)) && (ps->saberMove == LS_SPINATTACK_DUAL || ps->saberMove == LS_SPINATTACK || ps->saberMove == LS_SPINATTACK_GRIEV))
#endif
	{
		ps->speed *= 0.5;
	}
...
#ifdef _GAME
	if (!(m_nerf.integer & (1 << EOC_DUALSPECIAL)) && (ps->saberMove == LS_JUMPATTACK_DUAL || ps->saberMove == LS_GRIEVOUS_LUNGE))
#else
	if (!(cgs.m_nerf & (1 << EOC_DUALSPECIAL)) && (ps->saberMove == LS_JUMPATTACK_DUAL || ps->saberMove == LS_GRIEVOUS_LUNGE))
#endif
	{
		ps->speed *= 0.66;
	}
...
void BG_G2PlayerAngles(void* ghoul2, int motionBolt, entityState_t * cent, int time, vec3_t cent_lerpOrigin,
	...
	entityState_t * emplaced, int* crazySmoothFactor, int ManualBlockingFlags)
{
	int					adddir = 0;
	static int			dir;
	static int			i;
	...
	{ //if using an emplaced gun, then we want to make sure we're angled to "hold" it right
		...
		{ //misc emplaced
			...
			{ //try to adjust so it doesn't look wrong
				...
				BG_G2ClientSpineAngles(ghoul2, motionBolt, cent_lerpOrigin, cent_lerpAngles, cent, time, viewAngles, ciLegs, ciTorso, angles, thoracicAngles, ulAngles, llAngles, modelScale, tPitchAngle, tYawAngle, corrTime, cent->NPC_class);
...
static QINLINE void PM_CmdForSaberMoves(usercmd_t * ucmd)
{
	...
	else if (pm->ps->torsoAnim == BOTH_DASH_F)
	{
		ucmd->forwardmove = 127;
		ucmd->rightmove = 0;
	}
	else if (pm->ps->torsoAnim == BOTH_DASH_R)
	{
		ucmd->forwardmove = 0;
		ucmd->rightmove = 75;
	}
	else if (pm->ps->torsoAnim == BOTH_DASH_L)
	{
		ucmd->forwardmove = 0;
		ucmd->rightmove = -75;
	}
	else if (pm->ps->torsoAnim == BOTH_DASH_B)
	{
		ucmd->forwardmove = -127;
		ucmd->rightmove = 0;
	}
...
void PmoveSingle(pmove_t * pmove)
{
	...
	else if (BG_FullBodyTauntAnim(pm->ps->legsAnim))
	{
		...
		{//stop the anim
			...
			else if (pm->ps->legsAnim == BOTH_MEDITATE1 && pm->ps->torsoAnim == BOTH_MEDITATE1)
			{
		...
		else
		{
			...
			if (pm->ps->torsoAnim == BOTH_MEDITATE1)
			{
				if (pm->ps->torsoTimer < 100)
				{
					pm->ps->legsTimer = 100;
	...
	else if (pm->ps->legsAnim == BOTH_MEDITATE_END1
		&& pm->ps->legsTimer > 0)
	{
		stiffenedUp = qtrue;
		PM_SetPMViewAngle(pm->ps, pm->ps->viewangles, &pm->cmd);
	...
	else if (BG_InBlockedAttackBounce(pm->ps))
	{//can't move during a attack bounce
		stiffenedUp = qtrue;
	}
...
{
	bgEntity_t* pEnt = pm_entSelf;
	int npcgetUpTime = NPC_KNOCKDOWN_HOLD_EXTRA_TIME;
	...
	else if (pEnt->s.NPC_class == CLASS_STORMTROOPER)
	{
		return (npcgetUpTime + 100);
	}
	else if (pEnt->s.NPC_class == CLASS_CLONETROOPER)
	{
		return (npcgetUpTime + 100);
	}
	else if (pEnt->s.NPC_class == CLASS_STORMCOMMANDO)
	{
		return (npcgetUpTime + 100);
	}
