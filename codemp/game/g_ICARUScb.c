...
int Icarus_SoundTime(char soundName[MAX_QPATH])
{
	...
	//trap->SE_GetStringTextString(va("%s_%s", temp, sound), subtitle, sizeof(subtitle));
	//s = subtitle;

	//if (s[0] == '?' && s[1] == '?')
	//{//couldn't find it in the original file.  Try looking into OJP's supplimental file.
	//	trap->SE_GetStringTextString(va("zojp_coop_%s", sound), subtitle, sizeof(subtitle));
	//	s = subtitle;
	//}
...
static void Q3_SetVampire( int entID, qboolean vampire )
{
	...
	if (!self || !self->client)
	{
		G_DebugPrint(WL_WARNING, "Q3_SetVampire: entID %d not a client\n", entID);
		return;
	}

	if (vampire)
	{
		self->client->ps.powerups[PW_DISINT_4] = Q3_INFINITE;
	}
	else
	{
...
qboolean UseSpawnWeapons; //Indicates if SpawnWeapons is to be used.used to set default forcepowers for spawn when using ICARUS scripts.
...
qboolean Q3_Set( int taskID, int entID, const char *type_name, const char *data )
{
	...
	case SET_MISSIONFAILED:
		...
		//trap->SE_GetStringTextString(va("SP_INGAME_%s", data), char_data, sizeof(char_data));
		//trap->SendServerCommand(-1, va("cp \""S_COLOR_RED"Mission Failed\n%s\"", char_data));
		//LogExit("Mission Failed.");
		break;
