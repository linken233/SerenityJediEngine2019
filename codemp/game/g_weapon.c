...
//---------------------------------------------------------
{
	...
	gentity_t   *missile2 = CreateMissile(muzzle2, forward, BRYAR_PISTOL_VEL, 10000, ent, altFire);
...
//---------------------------------------------------------
{
	...
	if ( altFire && !(ent->r.svFlags & SVF_BOT))
	{
	...
	else
	{
		...
		if ( ent->client && ent->NPC && !(ent->r.svFlags & SVF_BOT) &&
		...
		else if (!WalkCheck(ent) && !(ent->r.svFlags & SVF_BOT))//if running aim is shit
		{
			angs[PITCH] += Q_flrand(-2.0f, 2.0f) * (RUNNING_SPREAD + 1.5f);
			angs[YAW]   += Q_flrand(-2.0f, 2.0f) * (RUNNING_SPREAD + 1.5f);
		}
...
//---------------------------------------------------------
{
	...
	{//need to loop this in case we hit a Jedi who dodges the shot
		...
		if (WP_SaberMustBlock(traceEnt, ent, qfalse, tr.endpos, -1, -1))
		{//saber can be used to block the shot.
		 ...
		 //broadcast shot blocked effect
			...
			G_DodgeDrain(traceEnt, ent, WP_SaberBlockCost(traceEnt, ent, tr.endpos));
...
//---------------------------------------------------------
{
	...
	for (i = 0; i < count; i++ )
	{
		...
		if (!(ent->client->ps.fd.forcePowersActive&(1 << FP_SEE)) || ent->client->ps.fd.forcePowerLevel[FP_SEE] < FORCE_LEVEL_2)
...
void WP_GrenadeBlow(gentity_t*self)
{
		...
		|| Q_stricmp(self->classname, "flash_grenade") == 0)
	{
		...
		num = trap->EntitiesInBox(mins, maxs, entitys, MAX_GENTITIES);
		for (i = 0; i < num; i++)
		{
...
{//a rail-gun-like beam
	...
	for ( i = 0; i < traces; i++ )
	{
		...
		if (traceEnt
			&& (traceEnt->s.weapon == WP_SABER || (traceEnt->client && (traceEnt->client->NPC_class == CLASS_BOBAFETT || traceEnt->client->NPC_class == CLASS_MANDO || traceEnt->client->NPC_class == CLASS_REBORN))))
		{
...
void FireWeapon( gentity_t *ent, qboolean altFire )
{
	...
	else
	{
		...
		{
			if (ent->client->pers.cmd.forwardmove || ent->client->pers.cmd.rightmove)
			{ 
