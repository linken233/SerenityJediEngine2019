...
void CrystalCratePain(gentity_t *self, gentity_t *attacker, int damage)
{
	...
	G_RadiusDamage(pt, self, 16, 32, self, self, MOD_UNKNOWN);
...
void GunRackAddItem(gitem_t *gun, vec3_t org, vec3_t angs, float ffwd, float fright, float fup)
{
	...
	int skill = (int)trap->Cvar_VariableIntegerValue("g_npcspskill");
	int t;
...
void SP_misc_model_gun_rack(gentity_t *ent)
{
	...
	if (ct)
	{
		for (i = 0; i < ct; i++)
		{
			GunRackAddItem(itemList[i], ent->s.origin, ent->s.angles, randoms() * 2, (i - 1) * 9 + randoms() * 2, ofz[i]);
		}
	}
...
void camera_aim(gentity_t *self)
{
	...
	{//I am the viewEntity
		...
		if ((player->client->pers.cmd.buttons&BUTTON_BLOCK) || player->client->pers.cmd.forwardmove || player->client->pers.cmd.rightmove || player->client->pers.cmd.upmove)
...
void misc_model_cargo_die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int mod, int dFlags, int hitLoc)
{
	...
	if ((flags & DROP_MEDPACK))
	{
		...
		if (health)
		{
			...
			LaunchItem(health, temp, (float *)vec3_origin);
		}
	}
	if ((flags & DROP_SHIELDS))
	{
		shields = BG_FindItem("item_shield_sm_instant");
		...
		if (shields)
		{
			...
			LaunchItem(shields, temp, (float *)vec3_origin);
		}
	}

	if ((flags & DROP_BACTA))
	{
		bacta = BG_FindItem("item_bacta");
		...
		if (bacta)
		{
			...
			LaunchItem(bacta, temp, (float *)vec3_origin);
		}
	}

	if ((flags & DROP_BATTERIES))
	{
		batteries = BG_FindItem("item_battery");
		...
		if (batteries)
		{
			...
			LaunchItem(batteries, temp, (float *)vec3_origin);
		}
	}
}
