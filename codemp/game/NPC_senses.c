...
{//FIXME: make a calculate visibility for ents that takes into account
	...
	vec3_t	spot;
	vec3_t	deltaVector;
	float	visrange = (NPCS.NPCInfo->stats.visrange * NPCS.NPCInfo->stats.visrange) * 2;
	...
	{//being pushed
		...
		{//events were just cleared out so add me again
		 //NOTE: presumes the player did the pushing, this is not always true, but shouldn't really matter?
			...
			//else
			//{
				AddSightEvent( player, ent->r.currentOrigin, 128, AEL_SUSPICIOUS, 0);//hmm... or should this always be discovered?
			//}
		}
	}
