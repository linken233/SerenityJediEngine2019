...
qboolean G_ValidSaberStyle(gentity_t *ent, int saberStyle)
{
	if (saberStyle == SS_MEDIUM && !(ent->client->saber[0].type == SABER_BACKHAND))
...
void ClientCommand( int clientNum )
{
	...
	if (Q_stricmp(cmd, "serenitysabersystems") == 0)
	{
		trap->SendServerCommand(ent - g_entities, va("print \"This mod improves the hit detection and alters the saber traces. Additionally, the first part of a swing\n\""));
		trap->SendServerCommand(ent - g_entities, va("print \"will now do damage. This allows the beginning of the swing to be blocked/parried. Because of this new\n\""));
		trap->SendServerCommand(ent - g_entities, va("print \"early-block system, saber spam or heavy exploits will be less effective. A player can 'deny' an enemy who\n\""));
		trap->SendServerCommand(ent - g_entities, va("print \"wildly swings their saber (fans, aggressive wiggle) and will force them to make cleaner, more precise hits.\n\""));
		trap->SendServerCommand(ent - g_entities, va("print \"To make up for the beginning of the swing's shortcomings, I added a 50 percent increase to end-phase damage.\n\""));
		trap->SendServerCommand(ent - g_entities, va("print \"More damage at the end of the swing should maintain the fast-paced feel of JKA. Well-timed and well-aimed\n\""));
		trap->SendServerCommand(ent - g_entities, va("print \"slashes are more effective than on basejka.\n\n\""));
		trap->SendServerCommand(ent - g_entities, va("print \"This could potentially improve the way we play saber-only JKA quite a lot. Less spam-friendly, and more accurate\n\""));
		trap->SendServerCommand(ent - g_entities, va("print \"hit detection could make the game more fast-paced and controlled.\n\""));
	}
	else if (Q_stricmp(cmd, "autosave_add") == 0 )
	{
		int args = trap->Argc();
