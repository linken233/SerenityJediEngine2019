...
qboolean BG_CanItemBeGrabbed( int gametype, const entityState_t *ent, const playerState_t *ps )
{
	...
	if ( ps )
	{
			...
			gametype != GT_CTY)
		{
			...
			if (ps->fd.forcePowerLevel[FP_SABER_OFFENSE] >= FORCE_LEVEL_1 && item && (item->giType == IT_WEAPON || item->giType == IT_AMMO || item->giType == IT_HEALTH))
			{//jedi cannot pick up weapons
				return qfalse;
			}
			if (ps->weapon == WP_SABER && item && (item->giType == IT_WEAPON || item->giType == IT_AMMO || item->giType == IT_HEALTH))
...
void BG_PlayerStateToEntityState( playerState_t *ps, entityState_t *s, qboolean snap )
{
	...
	s->ManualBlockingFlags = ps->ManualBlockingFlags;
...
int BG_GetGametypeForString( const char *gametype )
{
		 if ( !Q_stricmp( gametype, "ffa")
			...
			||!Q_stricmp( gametype, "MB" ) )			return GT_FFA;
			...
			||!Q_stricmp( gametype, "missions" ) )		return GT_SINGLE_PLAYER;
