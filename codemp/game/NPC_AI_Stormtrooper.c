		 ...
		 //check to see if they're in my FOV
			...
			{//in my cone
			 //check to see that they're close enough
				if (DistanceSquared(NPCS.NPC->client->renderInfo.eyePoint, enemy->r.currentOrigin) - 256 <= NPCS.NPC->speed * NPCS.NPC->speed)
				{//within range
...
void NPC_BSST_Attack(void)
{
	...
	{
		if (!in_camera && (TIMER_Done(NPCS.NPC, "flee") && NPC_CheckForDanger(NPC_CheckAlertEvents(qtrue, qtrue, -1, qfalse, AEL_DANGER))))
		{
			ST_Speech(NPCS.NPC, SPEECH_COVER, 0);
	...
	{//too close, so switch to primary fire
		if (NPCS.NPC->client->ps.weapon == WP_BLASTER)
		{
			...
			{//use primary fire
				trace_t	trace;
				trap->Trace(&trace, NPCS.NPC->r.currentOrigin, NPCS.NPC->enemy->r.mins, NPCS.NPC->r.maxs, NPCS.NPC->r.currentOrigin, NPCS.NPC->s.number, NPCS.NPC->clipmask, qfalse, 0, 0);
				...
				{//he can get right to me
					...
					TIMER_Set(NPCS.NPC, "attackDelay", -1); // If we move into position right after he fires, he can sit there and charge his weapon and look dumb
	...
	{//move toward goal
		if (NPCS.NPCInfo->goalEntity)
		{
			...
			if ((NPCS.NPC->client->NPC_class != CLASS_ROCKETTROOPER ||
				NPCS.NPC->s.weapon != WP_ROCKET_LAUNCHER || enemyDist < MIN_ROCKET_DIST_SQUARED)//rockettroopers who use rocket launchers turn around and run if you get too close (closer than 128)
			...
			{//time to smack
			 //recheck enemyDist and InFront
					...
					&& InFront(NPCS.NPC->enemy->r.currentOrigin, NPCS.NPC->r.currentOrigin, NPCS.NPC->client->ps.viewangles, 0.3f))
				{
					...
					G_Damage(NPCS.NPC->enemy, NPCS.NPC, NPCS.NPC, smackDir, NPCS.NPC->r.currentOrigin, (g_npcspskill.integer + 1) * Q_irand(1, 3), DAMAGE_NO_KNOCKBACK, MOD_MELEE);
					if (NPCS.NPC->client->ps.torsoAnim == BOTH_A7_KICK_F)
					{//smackdown
						int knockAnim = BOTH_KNOCKDOWN1;
						if (PM_CrouchAnim(NPCS.NPC->enemy->client->ps.legsAnim))
		...
		&& !PM_InKnockDown(&NPCS.NPC->client->ps))
	{
		...
		{//enemy within 80, if very close, use melee attack to slap away
			...
			{//animate me
				int swingAnim;
				swingAnim = Q_irand(BOTH_A7_KICK_F, BOTH_A7_KICK_F2);
			...
			{//time to smack
			 //recheck enemyDist and InFront
					...
					&& InFront(NPCS.NPC->enemy->r.currentOrigin, NPCS.NPC->r.currentOrigin, NPCS.NPC->client->ps.viewangles, 0.3f))
				{
					...
					VectorNormalize(smackDir);
					//hurt them
					G_Damage(NPCS.NPC->enemy, NPCS.NPC, NPCS.NPC, smackDir, NPCS.NPC->r.currentOrigin, (g_npcspskill.integer + 1) * 1, DAMAGE_NO_KNOCKBACK, MOD_MELEE);
					...
					{
						G_Throw(NPCS.NPC->enemy, smackDir, 10);
						NPC_SetAnim(NPCS.NPC->enemy, SETANIM_BOTH, BOTH_PAIN3, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
					}
					else if (NPCS.NPC->client->ps.torsoAnim == BOTH_MELEEUP)
					{
						G_Throw(NPCS.NPC->enemy, smackDir, 10);
						NPC_SetAnim(NPCS.NPC->enemy, SETANIM_BOTH, BOTH_PAIN4, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
					}
					else
					{//uppercut
					 //throw them
						G_Throw(NPCS.NPC->enemy, smackDir, 10);
						//make them backflip
						NPC_SetAnim(NPCS.NPC->enemy, SETANIM_BOTH, BOTH_PAIN1, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
					}
					//done with the damage
					NPCS.NPCInfo->blockedDebounceTime = 1;
				}
			...
			{//time to smack
			 //recheck enemyDist and InFront
					...
					&& InFront(NPCS.NPC->enemy->r.currentOrigin, NPCS.NPC->r.currentOrigin, NPCS.NPC->client->ps.viewangles, 0.3f))
				{
					...
					//hurt them
					G_Sound(NPCS.NPC->enemy, CHAN_AUTO, G_SoundIndex("sound/weapons/melee/kick2.mp3"));
					G_Damage(NPCS.NPC->enemy, NPCS.NPC, NPCS.NPC, smackDir, NPCS.NPC->r.currentOrigin, (g_npcspskill.integer + 1) * Q_irand(5, 10), DAMAGE_NO_KNOCKBACK, MOD_MELEE);
					if (NPCS.NPC->client->ps.torsoAnim == BOTH_A7_KICK_F)
					{//smackdown
						int knockAnim = BOTH_KNOCKDOWN1;
						if (PM_CrouchAnim(NPCS.NPC->enemy->client->ps.legsAnim))
		...
		if (NPCS.NPC->enemy->client //enemy is a client
			...
			&& (!NPCS.NPC->enemy->enemy || !NPCS.NPC->enemy->enemy->client
				|| (NPCS.NPC->enemy->enemy->client->NPC_class != CLASS_RANCOR
					&& NPCS.NPC->enemy->enemy->client->NPC_class != CLASS_WAMPA)))//enemy's enemy is not a client or is not a wampa or rancor (which is scarier than me)
