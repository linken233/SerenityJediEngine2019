...
void NPC_GM_Pain(gentity_t *self, gentity_t *attacker, int damage)
{
	...
	{//shield is currently down
		...
		{
			int newBolt = trap->G2API_AddBolt( self->ghoul2, 0, "*antenna_base" );
			if ( newBolt != -1 )
			{
				GM_CreateExplosion( self, newBolt, qfalse );
			}
	...
	{//my enemy is dead
		if (NPCS.NPC->client->ps.torsoAnim == BOTH_STAND2TO1 )
		{
			if (NPCS.NPC->client->ps.torsoTimer <= 500 )
			{
				...
				NPC_SetAnim(NPCS.NPC, SETANIM_BOTH, BOTH_TRIUMPHANT1START, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD );
				NPCS.NPC->client->ps.legsTimer += 500;
				NPCS.NPC->client->ps.torsoTimer += 500;
			}
		}
		else if (NPCS.NPC->client->ps.torsoAnim == BOTH_TRIUMPHANT1START )
		{
			if (NPCS.NPC->client->ps.torsoTimer <= 500 )
			{
				NPC_SetAnim(NPCS.NPC, SETANIM_BOTH, BOTH_TRIUMPHANT1STARTGESTURE, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD );
				NPCS.NPC->client->ps.legsTimer += 500;
				NPCS.NPC->client->ps.torsoTimer += 500;
			}
		}
		else if (NPCS.NPC->client->ps.torsoAnim == BOTH_TRIUMPHANT1STARTGESTURE )
		{
			if (NPCS.NPC->client->ps.torsoTimer <= 500 )
			{
				NPC_SetAnim(NPCS.NPC, SETANIM_BOTH, BOTH_TRIUMPHANT1STOP, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD );
				NPCS.NPC->client->ps.legsTimer += 500;
				NPCS.NPC->client->ps.torsoTimer += 500;
			}
		}
		...
		else if (NPCS.NPC->wait )
		{
			...
			else if ( DistanceHorizontalSquared(NPCS.NPC->client->renderInfo.eyePoint, NPCS.NPC->enemy->r.currentOrigin ) > 4096 && (NPCS.NPCInfo->scriptFlags&SCF_CHASE_ENEMIES) )//64 squared
			{
