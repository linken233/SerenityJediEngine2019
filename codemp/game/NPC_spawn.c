...
void NPC_SetWeapons(gentity_t * ent)
{
	...
	for (curWeap = WP_SABER; curWeap < WP_NUM_WEAPONS; curWeap++)
	{
		if ((weapons & (1 << curWeap)))
		{
			...
			RegisterItem(BG_FindItemForWeapon((weapon_t)(curWeap)));	//precache the weapon
	...
	else if (ent->NPC->stats.health)	// Was health supplied in NPC.cfg?
	{
		...
		{// up everyone except jedi
			...
			{//bosses are a bit different
				ent->NPC->stats.health = ceil((float)ent->NPC->stats.health * 0.75f + ((float)ent->NPC->stats.health / 4.0f * g_npcspskill.integer)); // 75% on easy, 100% on medium, 125% on hard
			}
			else
			{
...
gentity_t* NPC_Spawn_Do(gentity_t * ent)
{
	...
	if (ent->s.teamowner != TEAM_FREE || ent->s.teamowner != NPCTEAM_FREE || level.gametype != GT_SINGLE_PLAYER)
	{
	...
	else if (newent->s.teamowner != TEAM_FREE || newent->s.teamowner != NPCTEAM_FREE)
	{
		newent->client->sess.sessionTeam = newent->s.teamowner;
	}
	else if (newent->alliedTeam != TEAM_FREE || newent->alliedTeam != NPCTEAM_FREE)
	{
		newent->client->sess.sessionTeam = newent->alliedTeam;
	}
	else if (newent->teamnodmg != TEAM_FREE || newent->teamnodmg != NPCTEAM_FREE)
	{
...
void SP_NPC_spawner(gentity_t * self)
{
	...
	if (level.is_t3_hevil_map == qtrue &&
		(Q_stricmp(self->NPC_type, "Noghri") == 0 ||
			Q_stricmp(self->NPC_type, "StormTrooper") == 0 ||
			Q_stricmp(self->NPC_type, "StormTrooper2") == 0 ||
			Q_stricmp(self->NPC_type, "StormTrooper_red") == 0 ||
			Q_stricmp(self->NPC_type, "StormTrooper_blue") == 0 ||
			Q_stricmp(self->NPC_type, "Droideka_trooper") == 0 ||
			Q_stricmp(self->NPC_type, "Battle_Trooper") == 0 ||
			Q_stricmp(self->NPC_type, "SBD_Trooper") == 0))
	...
	{ // removing npcs from SP maps which are not that useful
		if (level.sp_map == qtrue &&
			(Q_stricmp(self->NPC_type, "r2d2_imp") == 0 ||
				Q_stricmp(self->NPC_type, "r2d2") == 0 ||
				Q_stricmp(self->NPC_type, "protocol_imp") == 0 ||
				Q_stricmp(self->NPC_type, "tavion_new") == 0 ||
				Q_stricmp(self->NPC_type, "dkothos") == 0 ||
				Q_stricmp(self->NPC_type, "vkothos") == 0 ||
				Q_stricmp(self->NPC_type, "rosh_penin") == 0 ||
				Q_stricmp(self->NPC_type, "rosh_dark") == 0 ||
				Q_stricmp(self->NPC_type, "rosh_penin_noforce") == 0 ||
				Q_stricmp(self->NPC_type, "tavion_scepter") == 0 ||
				Q_stricmp(self->NPC_type, "tavion_sith_sword") == 0 ||
				Q_stricmp(self->NPC_type, "kyle_boss") == 0 ||
				Q_stricmp(self->NPC_type, "luke") == 0 ||
				Q_stricmp(self->NPC_type, "chewie") == 0 ||
				Q_stricmp(self->NPC_type, "alora") == 0 ||
				Q_stricmp(self->NPC_type, "alora_dual") == 0 ||
				Q_stricmp(self->NPC_type, "boba_fett") == 0 ||
				Q_stricmp(self->NPC_type, "saber_droid_training") == 0 ||
				Q_stricmp(self->NPC_type, "merchant") == 0 ||
				Q_stricmp(self->NPC_type, "elder") == 0 ||
				...
				Q_stricmp(self->NPC_type, "protocol") == 0 ||
				Q_stricmp(self->NPC_type, "r5d2") == 0 ||
				Q_stricmp(self->NPC_type, "r5d2_imp") == 0 ||
