		...
		for (i = 0; i < numEnts; i++)
		{
			...
			{//must be a client
			 //RAFIXME: impliment
				...
				{//damage breakables within range, but not as much
					G_Damage(radiusEnt, NPCS.NPC, NPCS.NPC, vec3_origin, radiusEnt->r.currentOrigin, 100, 0, MOD_ROCKET_SPLASH);
				}
				continue;
			}
			...
			if (dist <= radius)
			{
				...
				{//close enough to do damage, too
					G_Damage(radiusEnt, NPCS.NPC, NPCS.NPC, vec3_origin, radiusEnt->r.currentOrigin, Q_irand(20, 30), DAMAGE_NO_KNOCKBACK, MOD_ROCKET_SPLASH);
				}
				if (radiusEnt->client
...
{//Activate the scepter beam for the current NPC.
 //RAFIXME:  This probably needs to be moved to client side.
	...
	trap->G2API_GetBoltMatrix(NPCS.NPC->client->weaponGhoul2[1], 0, NPCS.NPC->NPC->genericBolt1, &boltMatrix, angles, NPCS.NPC->r.currentOrigin, level.time, NULL, NPCS.NPC->modelScale);
	BG_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, base);
	BG_GiveMeVectorFromMatrix(&boltMatrix, NEGATIVE_X, dir);
...
void Boba_JetpackDecideDefault(void)
{
	...
	gDist = G_GroundDistance(NPCS.NPC);
...
void Boba_ChooseWeapon(void)
{
	if (TIMER_Done(NPCS.NPC, "nextWeaponDelay"))
	{
		...
		while (1)
		{
			int	nw = Q_irand(0, WP_NUM_WEAPONS - 1);

			if (HaveWeapon(nw))
			{
				int	hbw = NPC_HaveBetterWeapon(nw);
...
void Boba_StartFlameThrower(gentity_t * self)
{
	...
	trap->G2API_GetBoltMatrix(NPCS.NPC->ghoul2, 0, NPCS.NPC->client->renderInfo.handLBolt, &boltMatrix, NPCS.NPC->r.currentAngles,
...
void Boba_DoSniper(gentity_t * self)
{
	if (TIMER_Done(self, "PickNewSniperPoint"))
	{
		...
		int		SniperPoint = NPC_FindCombatPoint(NPCS.NPC->r.currentOrigin, 0, NPCS.NPC->r.currentOrigin, CP_SNIPE | CP_CLEAR | CP_HAS_ROUTE | CP_TRYFAR | CP_HORZ_DIST_COLL, 0, -1);
		if (SniperPoint != -1)
		{
...
wristWeapon_t missileStates[4] = {
	{BOBA_MISSILE_ROCKET, FP_FIRST, WP_ROCKET_LAUNCHER, qfalse, 1, BOBA_FLAMEDURATION, 150, BOTH_FLAMETHROWER, qfalse, qtrue, qtrue},
	{BOBA_MISSILE_LASER, FP_GRIP, WP_EMPLACED_GUN, qfalse, 5, BOBA_FLAMEDURATION, 150, BOTH_FLAMETHROWER, qfalse, qtrue, qtrue},
...
void Boba_FireWristMissile(gentity_t* self, int whichMissile)
{
	...
	trap->G2API_GetBoltMatrix(self->ghoul2, 0, missileStates[whichMissile].leftBolt ? self->genericBolt3 : self->client->renderInfo.handLBolt, &boltMatrix, self->r.currentAngles, self->r.currentOrigin, level.time, NULL, self->modelScale);
...
void Boba_TacticsSelect()
{
	...
	//--------------------------------------------------------------------
	float	enemyDistance = Distance(NPCS.NPC->r.currentOrigin, NPCS.NPC->enemy->r.currentOrigin);
	...
	else if (level.time - NPCS.NPCInfo->enemyLastSeenTime < 10000)
	{
		...
		nextState = (!enemyDistance > BOBA_ROCKETRANGEMIN && enemyDistance < BOBA_ROCKETRANGEMAX || Q_irand(0, NPCS.NPC->count) < 1) ? (BTS_RIFLE) : (BTS_MISSILE);
	}
	...
	else
	{
		...
		int	AmbushPointNear = qfalse;		 // TODO
		...
		{
			TIMER_Set(NPCS.NPC, "Boba_TacticsSelect", Q_irand(15000, 25000));// More patience here
...
void Boba_Tactics()
{
	...
	return;			// Do Not Use Normal Jedi Or Seeker Movement
}
...
static void Jedi_AggressionErosion(int amt)
{
	...
	if (NPCS.NPCInfo->stats.aggression < 4 || (NPCS.NPCInfo->stats.aggression < 6 && NPCS.NPC->client->NPC_class == CLASS_VADER))
	{//turn off the saber
		WP_DeactivateSaber(NPCS.NPC, qfalse);
	}
}
...
static void Jedi_AdjustSaberAnimLevel(gentity_t * self, int newLevel)
{
	...
	if (self->client->playerTeam == NPCTEAM_ENEMY)
	{
		...
		{//fast only
			self->client->ps.fd.saberAnimLevel = Q_irand(SS_FAST, SS_MEDIUM);
		}
...
{//FIXME: for many of these checks, what we really want is horizontal distance to enemy
		...
		&& NPCS.NPC->enemy->s.weapon == WP_SABER)
	{
		...
		if (TIMER_Done(NPCS.NPC, "TalkTime"))
		{
			...
			else if (NPC_IsLightJedi(NPCS.NPC))
			{// Do taunt...
	...
	if (NPCS.NPC->client->NPC_class == CLASS_KYLE ||
		...
		&& !(NPCS.ucmd.buttons & (BUTTON_ATTACK | BUTTON_ALT_ATTACK | BUTTON_FORCEPOWER | BUTTON_DASH | BUTTON_USE | BUTTON_BLOCK))
		&& !PM_InKnockDown(&NPCS.NPC->client->ps))
	{
			...
			{//time to smack
			 //recheck enemyDist and InFront
					...
					&& InFront(NPCS.NPC->enemy->r.currentOrigin, NPCS.NPC->r.currentOrigin, NPCS.NPC->client->ps.viewangles, 0.3f))
				{
					...
					G_Damage(NPCS.NPC->enemy, NPCS.NPC, NPCS.NPC, smackDir, NPCS.NPC->r.currentOrigin, (g_npcspskill.integer + 1) * Q_irand(2, 5), DAMAGE_NO_KNOCKBACK, MOD_MELEE);
					...
					{//uppercut
					 //throw them
						...
						//make them backflip
						NPC_SetAnim(NPCS.NPC->enemy, SETANIM_BOTH, BOTH_KNOCKDOWN2, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
					}
		...
		&& !PM_InKnockDown(&NPCS.NPC->client->ps))
	{
		if (enemyDist < MELEE_DIST_SQUARED
			&& !NPCS.NPC->client->ps.weaponTime //not firing
			&& !PM_InKnockDown(&NPCS.NPC->client->ps)//not knocked down
			&& InFront(NPCS.NPC->enemy->r.currentOrigin, NPCS.NPC->r.currentOrigin, NPCS.NPC->client->ps.viewangles, 0.3f))//within 80 and in front
		{//enemy within 80, if very close, use melee attack to slap away
			...
			&& !NPCS.NPC->client->ps.weaponTime //not firing
			&& !PM_InKnockDown(&NPCS.NPC->client->ps)//not knocked down
			&& !InFront(NPCS.NPC->enemy->r.currentOrigin, NPCS.NPC->r.currentOrigin, NPCS.NPC->client->ps.viewangles, -0.25f))//within 80 and generally behind
		{//enemy within 80, if very close, use melee attack to slap away			
			if (TIMER_Done(NPCS.NPC, "attackDelay"))
			{//animate me
				int swingAnim;
				swingAnim = Q_irand(BOTH_A7_KICK_B2, BOTH_A7_KICK_B3);
				G_Sound(NPCS.NPC, CHAN_AUTO, G_SoundIndex("sound/weapons/melee/kick1.mp3"));
				NPC_SetAnim(NPCS.NPC, SETANIM_BOTH, swingAnim, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
				TIMER_Set(NPCS.NPC, "attackDelay", NPCS.NPC->client->ps.torsoTimer + Q_irand(10000, 15000));
				//delay the hurt until the proper point in the anim
		...
		&& !PM_InKnockDown(&NPCS.NPC->client->ps))
	{
		...
		{//enemy within 80, if very close, use melee attack to slap away			
			...
			{//animate me
				int swingAnim;
				swingAnim = Q_irand(BOTH_A7_KICK_B2, BOTH_A7_KICK_B);
				G_Sound(NPCS.NPC, CHAN_AUTO, G_SoundIndex("sound/weapons/melee/kick1.mp3"));
				NPC_SetAnim(NPCS.NPC, SETANIM_BOTH, swingAnim, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
				TIMER_Set(NPCS.NPC, "attackDelay", NPCS.NPC->client->ps.torsoTimer + Q_irand(15000, 20000));
				//delay the hurt until the proper point in the anim
	...
	{//we're out of striking range and we are allowed to attack
		...
		{//They're being gripped, rush them!
			...
			if (NPCS.NPC->client->ps.fd.forcePowerLevel[FP_SABERTHROW] > FORCE_LEVEL_2
				&& TIMER_Done(NPCS.NPC, "attackDelay") || (NPCS.NPCInfo->rank >= RANK_LT_COMM || WP_ForcePowerUsable(NPCS.NPC, FP_SABERTHROW))
				&& !Q_irand(0, 5)
				&& !(NPCS.NPC->client->ps.fd.forcePowersActive & (1 << FP_SPEED))
				&& !(NPCS.NPC->client->ps.saberEventFlags & SEF_INWATER))//saber not in water
			{//throw saber
			...
			NPCS.NPC->enemy->client->ps.saberInFlight && NPCS.NPC->enemy->client->ps.saberEntityNum && //enemy throwing saber
		...
		else
		{
			...
			{
				int chanceScale = 0;
				if ((NPCS.NPC->client->NPC_class == CLASS_KYLE || NPCS.NPC->client->NPC_class == CLASS_YODA) && (NPCS.NPC->spawnflags & 1))
				{
					chanceScale = 4;
				}
...
evasionType_t Jedi_CheckFlipEvasions(gentity_t * self, float rightdot, float zdiff)
{
	...
	{//already running on a wall
		...
		animLength = PM_AnimLength(self->localAnimIndex, (animNumber_t)self->client->ps.legsAnim);
...
static qboolean Jedi_SaberBlock(void)
{
	...
	dist = ShortestLineSegBewteen2LineSegs(NPCS.NPC->enemy->client->saber[closestSaberNum].blade[closestBladeNum].muzzlePoint, saberTip, bottom, top, saberPoint, axisPoint);
	VectorSubtract(saberPoint, NPCS.NPC->enemy->client->saber[closestSaberNum].blade[closestBladeNum].muzzlePoint, pointDir);
	...
	VectorSubtract(NPCS.NPC->enemy->client->saber[closestSaberNum].blade[closestBladeNum].muzzlePoint, NPCS.NPC->enemy->client->saber[closestSaberNum].blade[closestBladeNum].muzzlePointOld, baseDir);
...
static void Jedi_SetEnemyInfo(vec3_t enemy_dest, vec3_t enemy_dir, float* enemy_dist, vec3_t enemy_movedir, float* enemy_movespeed, int prediction)
{
	...
	{//see where enemy is headed
		...
		*enemy_movespeed = VectorNormalize(enemy_movedir);
		...
		//FIXME: enemy_dist calc needs to include all blade lengths, and include distance from hand to start of blade....
		*enemy_dist = VectorNormalize(enemy_dir) - (NPCS.NPC->client->saber[0].blade[0].lengthMax + NPCS.NPC->r.maxs[0] * 1.5 + 16);
	}
...
static qboolean Jedi_AttackDecide(int enemy_dist)
{
	...
	if (!(NPCS.ucmd.buttons & BUTTON_ATTACK) && !(NPCS.ucmd.buttons & BUTTON_ALT_ATTACK) && !(NPCS.ucmd.buttons & BUTTON_FORCEPOWER) && !(NPCS.ucmd.buttons & BUTTON_DASH))
...
{//FIXME: if land on enemy, knock him down & jump off again
	...
	{//a more complicated jump
		...
		if (NPCS.NPC->r.currentOrigin[2] > dest[2])
		{
			VectorCopy(NPCS.NPC->r.currentOrigin, p1);
			VectorCopy(dest, p2);
		}
		else if (NPCS.NPC->r.currentOrigin[2] < dest[2])
		{
			VectorCopy(dest, p1);
...
static void Jedi_CheckEnemyMovement(float enemy_dist)
{
	...
	if (!(NPCS.NPCInfo->aiFlags & NPCAI_BOSS_CHARACTER))
	{
		...
		{//enemy is mad at *me*
			...
			else if (NPCS.NPC->enemy->client->ps.legsAnim == BOTH_A2_STABBACK1 || NPCS.NPC->enemy->client->ps.legsAnim == BOTH_A2_STABBACK1B)
...
void RT_CheckJump(void)
{
	...
	{//hunt him down
		if ((NPC_ClearLOS4(NPCS.NPC->enemy) || NPCS.NPCInfo->enemyLastSeenTime > level.time - 500) && NPC_FaceEnemy(qtrue))
		{
		...
		if ((Jedi_Hunt() || Jedi_Track()) && !(NPCS.NPCInfo->aiFlags & NPCAI_BLOCKED))
		 ...
		 //If we can't get straight at him
			...
			{//hunt him down
			 //Com_Printf( "No Clear Path\n" );
				...
				{
					if (NPCS.NPC->client && NPCS.NPC->client->NPC_class == CLASS_BOBAFETT || NPCS.NPC->client && NPCS.NPC->client->NPC_class == CLASS_MANDO)
					{
						Boba_FireDecide();
					}
				}
			...
			{//have one to consider
				...
				{//we have a clear (of architecture) LOS to him
					...
					{//just get mad right away
						if (DistanceHorizontalSquared(NPCS.NPC->r.currentOrigin, best_enemy->r.currentOrigin) < (1024 * 1024))
						{
		...
		{//saber wielders only.
			...
			{//certain NPCS will kick in force speed if the player does...
					...
					&& !(NPCS.NPC->client->ps.fd.forcePowersActive & (1 << FP_SPEED)))
				{
					...
					if (!Q_irand(0, chance))
					{
						if (IsPressingDashButton(NPCS.NPC))
						{
							ForceSpeedDash(NPCS.NPC, 0);
						}
						else
						{
			...
			&& G_ClearLineOfSight(NPCS.NPC->client->leader->client->renderInfo.eyePoint, NPCS.NPC->client->renderInfo.eyePoint, NPCS.NPC->s.number, MASK_OPAQUE))
		{
			...
			if (NPCS.NPC->client->leader->client)
			{
				...
				{//let him get up now
					...
					NPCS.NPC->client->leader->client->ps.powerups[PW_INVINCIBLE] = level.time + NPCS.NPC->client->leader->client->ps.torsoTimer;
			...
			{//Vil and/or Dasariah still around to heal me
				...
				{//FIXME: custom anims
					...
					{//twins are still alive
						...
						NPCS.NPC->client->ps.saberBlocked = BLOCKED_NONE;
						NPCS.NPC->client->ps.saberMove /* not sure what this is = NPC->client->ps.saberMoveNext*/ = LS_NONE;
...
void NPC_CheckEvasion(void)
{
	...
	{//do we need to do any evasion for other kinds of enemies?
		if (NPCS.NPC->enemy->client)
		{
			...
			if (NPCS.NPC->enemy->client->ps.weaponstate == WEAPON_FIRING && InFieldOfVision(NPCS.NPC->enemy->client->ps.viewangles, 90, ang))
			...
			{// Check for nearby missiles/grenades to evade...
				...
				numEnts = trap->EntitiesInBox(mins, maxs, entityList, MAX_GENTITIES);
