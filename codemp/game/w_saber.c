...
int WP_SaberBlockCost(gentity_t* defender, gentity_t* attacker, vec3_t hitLoc);
...
extern void AddFatigueDamageBonus(gentity_t* attacker, gentity_t* victim);
...
extern void SabBeh_AttackVsBlock(gentity_t* attacker, sabmech_t* mechAttacker, gentity_t* blocker, sabmech_t* mechBlocker, vec3_t hitLoc, qboolean hitSaberBlade, qboolean* attackerMishap, qboolean* blockerMishap);
extern qboolean SabBeh_RollBalance(gentity_t* self, sabmech_t* mechSelf, qboolean forceMishap);
extern void SabBeh_AddBalance(gentity_t* self, sabmech_t* mechSelf, int amount, qboolean attack);
extern void SabBeh_AnimateSlowBounce(gentity_t* self, gentity_t* inflictor);
extern void SabBeh_AnimateHeavySlowBounce(gentity_t* self, gentity_t* inflictor);
extern void SabBeh_AnimateMassiveBounce(gentity_t* self, gentity_t* inflictor);//attacker anim
extern void SabBeh_AnimateMassiveSlowBounce(gentity_t* hitEnt, qboolean move);//defender anim
extern void SabBeh_AttackVsAttack(gentity_t* self, sabmech_t* mechSelf, gentity_t* otherOwner, sabmech_t* mechOther, qboolean* selfMishap, qboolean* otherMishap);
extern void NPC_SetPainEvent(gentity_t* self);
extern qboolean G_ControlledByPlayer(gentity_t* self);
extern void SabBeh_CrushTheParry(gentity_t* self);

static QINLINE void ClearSabMech(sabmech_t* sabmech)
{
	sabmech->doStun = qfalse;
	sabmech->dofatiguedStun = qfalse;
	...
	sabmech->doMassiveBounce = qfalse;
	sabmech->doMassiveSlowBounce = qfalse;
	sabmech->dofatiguedParry = qfalse;
	sabmech->doNormalBounce = qfalse;
	sabmech->doCrushTheParry = qfalse;
#ifdef _DEBUG
...
{//This function is for rolling saber mishaps outside the saber behavior code.
	...
	{//mishap occurred, animate it
		...
		else if (mechSelf.dofatiguedStun)
		{
			AnimateStun(self, inflictor, self->client->ps.origin);
		}
		...
		else if (mechSelf.doMassiveBounce)
		{
			SabBeh_AnimateMassiveBounce(self, inflictor);
		}
		else if (mechSelf.doMassiveSlowBounce)
		{
			SabBeh_AnimateMassiveSlowBounce(self, qtrue);
		}
		else if (mechSelf.doCrushTheParry)
		{
			SabBeh_CrushTheParry(self);
		}
	}
}

void Advanced_Saberblocking(gentity_t * self, sabmech_t * mechSelf, gentity_t * otherOwner, sabmech_t * mechOther, vec3_t hitLoc, qboolean * didHit, qboolean otherHitSaberBlade)
{
	...
	{//self is attacking
		...
		else if (WP_SaberMustBlock(otherOwner, self, qfalse, hitLoc, -1, -1))
		{//and otherOwner is blocking or parrying
		 //this is called with dual with both sabers
			SabBeh_AttackVsBlock(self, mechSelf, otherOwner, mechOther, hitLoc, otherHitSaberBlade, &selfMishap, &otherMishap);
			*didHit = qfalse;
		}
		else
	...
	else if (WP_SaberMustBlock(self, otherOwner, qfalse, hitLoc, -1, -1))
	{//self is blocking or parrying
		if (BG_SaberInNonIdleDamageMove(&otherOwner->client->ps, otherOwner->localAnimIndex))
		{//and otherOwner is attacking
			SabBeh_AttackVsBlock(otherOwner, mechOther, self, mechSelf, hitLoc, qtrue, &otherMishap, &selfMishap);
		}
		else if (WP_SaberMustBlock(otherOwner, self, qfalse, hitLoc, -1, -1))
		{//and otherOwner is blocking or parrying
...
static QINLINE int G_SaberAttackPower(gentity_t * ent, qboolean attacking)
{
	...
	if ((ent->client->ps.brokenLimbs & (1 << BROKENLIMB_RARM)) || (ent->client->ps.brokenLimbs & (1 << BROKENLIMB_LARM)))
...
extern qboolean G_CheckIncrementLockAnim(int anim, int winOrLose); //bg_saber.c
...
static QINLINE int G_GetAttackDamage(gentity_t * self, int minDmg, int maxDmg, float multPoint)
{
	...
	PM_SaberStartTransAnim(self->s.number, self->client->ps.fd.saberAnimLevel, self->client->ps.weapon, self->client->ps.torsoAnim, &animSpeedFactor, self->client->ps.brokenLimbs, self->client->ps.userInt3);
...
static QINLINE qboolean G_ClientIdleInWorld(gentity_t * ent)
{
	...
	if (!ent->client->pers.cmd.upmove &&
		...
		!(ent->client->pers.cmd.buttons& BUTTON_KICK) &&
		!(ent->client->pers.cmd.buttons& BUTTON_DASH))
	{
		return qtrue;
	}
...
static QINLINE qboolean G_SaberFaceCollisionCheck(int fNum, saberFace_t * fList, vec3_t atkStart, vec3_t atkEnd, vec3_t atkMins, vec3_t atkMaxs, vec3_t impactPoint)
{
...
qboolean WP_SaberIsOff(gentity_t * self, int saberNum);
qboolean WP_BladeIsOff(gentity_t * self, int saberNum, int bladeNum);
static QINLINE qboolean G_Enhanced_SaberCollide(gentity_t * atk, gentity_t * def, vec3_t atkStart, vec3_t atkEnd, vec3_t atkMins, vec3_t atkMaxs, trace_t * tr)
{
	static int i, j;
...
static QINLINE qboolean G_SaberCollide(gentity_t * atk, gentity_t * def, vec3_t atkStart, vec3_t atkEnd, vec3_t atkMins, vec3_t atkMaxs, vec3_t impactPoint)
{
	static int i, j;
...
{//returns the cost to block this attack for this attacker/defender combo.
	...
	{//standard bolt block!
			...
			|| attacker->activator && attacker->activator->s.weapon == WP_FLECHETTE)
		{
			...
			{
				float distance = VectorBlockDistance(attacker->activator->r.currentOrigin, defender->r.currentOrigin);
				...
				if (defender->client->ps.fd.forcePowerLevel[FP_SABER_OFFENSE] >= FORCE_LEVEL_3 && defender->client->ps.fd.saberAnimLevel != SS_MEDIUM)
				{
					saberBlockCost++;
				}
	...
	{//"normal" swing moves
		...
		{//attacker is in an attack fake
			if (attacker->client->ps.fd.saberAnimLevel == SS_STRONG
				&& !G_ActiveParry(defender, attacker, hitLoc))
	...
	{//attacker is a player so he must have just hit you with a saber blow.
		if (G_ActiveParry(defender, attacker, hitLoc))
		...
		{//player is behind us, costs more to block
		 //staffs back block at normal cost.
			if (defender->client->ps.fd.saberAnimLevel == SS_STAFF && defender->client->ps.fd.forcePowerLevel[FP_SABER_DEFENSE] == FORCE_LEVEL_3)
			{// Having both staff and defense 3 allow no extra back hit damage
				saberBlockCost *= 1;
			}
			else if (defender->client->ps.fd.saberAnimLevel != SS_STAFF && defender->client->ps.fd.forcePowerLevel[FP_SABER_DEFENSE] == FORCE_LEVEL_2)
			{//level 2 defense lowers back damage more
				saberBlockCost *= 1.50;
			}
			else if (defender->client->ps.fd.saberAnimLevel != SS_STAFF && defender->client->ps.fd.forcePowerLevel[FP_SABER_DEFENSE] == FORCE_LEVEL_1)
...
qboolean WP_UsingDualSaberAsPrimary(playerState_t * ps);
int WP_SaberMustBlock(gentity_t * self, gentity_t * atk, qboolean checkBBoxBlock, vec3_t point, int rSaberNum, int rBladeNum)
{//similar to WP_SaberCanBlock but without the same sorts of restrictions.
	...
	if (self->client->ps.weapon == WP_SABER && self->client->ps.saberHolstered)  //saber not here
	...
	{//our saber is currently dropped or in flight.
		if (!WP_UsingDualSaberAsPrimary(&self->client->ps))
	...
	{
		if (self->r.svFlags & SVF_BOT || self->s.eType == ET_NPC)
		{
			return 1;
		}
		else
	...
	{//player is attacking with saber
		...
		if (PM_SuperBreakWinAnim(atk->client->ps.torsoAnim) && self->client->ps.fd.forcePower < 35)
	...
	{//player attacker, if they are here they're using their saber to attack.  
	 ...
	 //create a line seqment thru the center of the player.
		...
		VectorSubtract(atk->client->saber[rSaberNum].blade[rBladeNum].muzzlePoint, atk->client->saber[rSaberNum].blade[rBladeNum].muzzlePointOld, saberMoveDir);
...
int G_RealTrace(gentity_t * attacker, trace_t * tr, vec3_t start, vec3_t mins, vec3_t maxs, vec3_t end, int passEntityNum, int contentmask, int rSaberNum, int rBladeNum)
{
	...
	for (misses = 0; misses < MAX_REAL_PASSTHRU; misses++)
	{
		...
		{//initial trace hit a humanoid
			if (attacker && WP_SaberMustBlock(currentEnt, attacker, qtrue, tr->endpos, rSaberNum, rBladeNum))
		...
		{//hit a lightsaber, do the approprate collision detection checks.
			...
			G_Enhanced_SaberCollide((atkIsSaberer ? attacker : NULL), saberOwner, currentStart, end, mins, maxs, tr);
		}
...
static QINLINE int PM_PowerLevelForSaberAnim(gentity_t * ent, int saberNum, qboolean mySaberHit)
{
	saberInfo_t* saber1;
	...
	else
	{
		...
		{
			if (saber1->type == SABER_LANCE)
			{
				return FORCE_LEVEL_4;
			}
			else if (saber1->type == SABER_TRIDENT)
			{
...
void WP_SaberDoHit(gentity_t * self, int saberNum, int bladeNum)
{
	...
	for (i = 0; i < numVictims; i++)
	{
		...
		te = G_TempEntity(dmgSpot[i], EV_SABER_BODY_HIT);
		if (te)
		{
						...
						{//still alive
							...
							{//close enough and knockback high enough to possibly knock down
								...
								{//within range of my fist or within ground-shaking range and not in the air
									G_Knockdown(radiusEnt, ent, entDir, 70, qtrue);
								}
							}
						}
					}
...
void AnimatemassiveKnockaway(gentity_t* self, gentity_t* inflictor, vec3_t impact)
{//do an knockaway.
	if (!PM_SaberInKnockaway(self->client->ps.saberMove) && !PM_InKnockDown(&self->client->ps))
	{
		if (!PM_SaberInParry(self->client->ps.saberMove))
		{
			WP_SaberBlockNonRandom(self, impact, qfalse);
			self->client->ps.saberMove = PM_BrokenParryForAttack(self->client->ps.saberMove);
...
{//checks to see if there's a cliff in the direction that dodgeAnim would travel.
	...
	if (dodgeAnim == BOTH_ROLL_F || dodgeAnim == BOTH_ROLL_F1 || dodgeAnim == BOTH_ROLL_F2 || dodgeAnim == BOTH_HOP_F)
	{//forward rolls
	...
	trap->Trace(&tr, tracefrom_mod, mins, maxs, traceto_mod, self->client->ps.clientNum, MASK_PLAYERSOLID, qfalse, 0, 0);
...
qboolean G_DoDodge(gentity_t * self, gentity_t * shooter, vec3_t dmgOrigin, int hitLoc, int* dmg, int mod)
{
	...
	{//special saber moves have special effects.
		...
		if (!WalkCheck(self)
			&& (!InFront(shooter->client->ps.origin, self->client->ps.origin, self->client->ps.viewangles, -0.7f)
				|| PM_SaberInAttack(self->client->ps.saberMove)
				|| PM_SaberInStart(self->client->ps.saberMove)))
		{
			if (self->NPC)
		...
		{//can't just do a simple dodge
			...
			{//ok, we can do the dodge hops/rolls
				if (PM_InKnockDown(&self->client->ps)
					|| BG_HopAnim(rolled))
				{//ground dodge roll
					G_SetAnim(self, &self->client->pers.cmd, SETANIM_BOTH, rolled, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 0);
				}
				else
				{//normal dodge roll
...
static QINLINE qboolean CheckSaberDamage(gentity_t * self, int rSaberNum, int rBladeNum, vec3_t saberStart, vec3_t saberEnd, qboolean doInterpolate, int trMask, qboolean extrapolate)
{
	...
	realTraceResult = G_RealTrace(self, &tr, saberStart, saberTrMins, saberTrMaxs, saberEnd, self->s.number, trMask, rSaberNum, rBladeNum);
	...
	qboolean saberInSpecial = PM_SaberInSpecial(self->client->ps.saberMove);
	...
	{//full damage moves
		...
		if (self->client->ps.fd.saberAnimLevel == SS_STAFF || self->client->ps.fd.saberAnimLevel == SS_DUAL)
		{
			...
			{//it will get auto-ramped based on the point in the attack, later on
				if (self->client->ps.saberMove == LS_SPINATTACK || self->client->ps.saberMove == LS_SPINATTACK_DUAL || self->client->ps.saberMove == LS_SPINATTACK_GRIEV)
				...
				else
				{
					...
					{ //special kata move
						...
						{ //staff kata
							dmg = G_GetAttackDamage(self, SABER_MINHITDAMAGE, SABER_HIGHITDAMAGE, 0.5f);
						}
					}
					else
					{
						dmg = G_GetAttackDamage(self, 2, SABER_HIGHITDAMAGE, 0.5f);
					}
				}
			}
			else if (inBackAttack)
			{
				dmg = G_GetAttackDamage(self, 2, SABER_NORHITDAMAGE, 0.5f); //can hit multiple times (and almost always does), so..
			}
			else
			{ //otherwise we'll ramp up to 150 I guess, for both dual and staff
				dmg = SABER_KILHITDAMAGE;
			}
		}
		else if (self->client->ps.fd.saberAnimLevel == SS_STRONG || self->client->ps.fd.saberAnimLevel == SS_DESANN)
		{
			if (!saberInSpecial && !inBackAttack)
			{
				dmg = G_GetAttackDamage(self, SABER_MINHITDAMAGE, SABER_MAXHITDAMAGE, 0.5f);
			}
			else if (saberInSpecial && (self->client->ps.saberMove == LS_A_JUMP_T__B_))
			{
				dmg = G_GetAttackDamage(self, SABER_MINHITDAMAGE, SABER_MAXHITDAMAGE, 0.65f);
			}
			else if (saberInSpecial && (self->client->ps.saberMove == LS_A_JUMP_PALP_))
			{
				dmg = G_GetAttackDamage(self, SABER_MINHITDAMAGE, SABER_MAXHITDAMAGE, 0.65f);
			}
			else if (inBackAttack)
		...
		else if (self->client->ps.fd.saberAnimLevel == SS_MEDIUM)
		{
			...
			{ //a well-timed hit with this can do a full blow
				dmg = G_GetAttackDamage(self, 2, SABER_MAXHITDAMAGE, 0.5f);
			}
			else if (inBackAttack)
			{
				dmg = G_GetAttackDamage(self, 2, SABER_NORHITDAMAGE, 0.5f); //can hit multiple times (and almost always does), so..
		...
		else if (self->client->ps.fd.saberAnimLevel == SS_FAST || self->client->ps.fd.saberAnimLevel == SS_TAVION)
		{
			if (saberInSpecial && (self->client->ps.saberMove == LS_A_LUNGE))
			{
				dmg = G_GetAttackDamage(self, 2, SABER_KILHITDAMAGE - 5, 0.3f);
			}
			else if (inBackAttack)
			{
				dmg = G_GetAttackDamage(self, 2, SABER_NORHITDAMAGE, 0.5f); //can hit multiple times (and almost always does), so..
			...
			{
				dmg = SABER_HIGHITDAMAGE;
			}
		}
		else if (self->client->ps.saberMove == LS_LEAP_ATTACK || BG_StabDownAnim(self->client->ps.torsoAnim))
		{
			dmg = SABER_MAXHITDAMAGE;
		}
		else
		...
		attackStr = PM_PowerLevelForSaberAnim(self, rSaberNum, qfalse);
		DamageMove = qtrue;
	}
	...
	{//use idle damage for transition moves.  
		...
		if (self->client->pers.cmd.buttons& BUTTON_BLOCK)
		{
			dmg = SABER_BLOCKING_DAMAGE;
		}
		idleDamage = qtrue;
	}
	else
	...
	{//hit something that had health and takes damage
		...
		if (WP_SaberMustBlock(&g_entities[tr.entityNum], self, qfalse, tr.endpos, -1, -1))
	...
	{ //hit a saber blade
		...
		if (otherOwner->client->ps.saberInFlight && !WP_UsingDualSaberAsPrimary(&otherOwner->client->ps))
		{//Hit a thrown saber, deactive it.
			saberCheckKnockdown_Smashed(&g_entities[tr.entityNum], otherOwner, self, dmg);
			otherOwner = NULL;
		}
		else if (realTraceResult == REALTRACE_SABERBLOCKHIT || realTraceResult == REALTRACE_PLAYER)
	...
	Advanced_Saberblocking(self, &mechSelf, otherOwner, &mechOther, tr.endpos, &didHit, hitSaberBlade);
	...
	{//deal damage
	 //damage the thing we hit
		...
		else if (G_DoDodge(victim, self, tr.endpos, -1, &dmg, MOD_SABER))
		{
			//add saber impact debounce
			DebounceSaberImpact(self, otherOwner, rSaberNum, rBladeNum, sabimpactentitynum);
			return qtrue;
	...
	if (self->client->ps.saberInFlight && !WP_UsingDualSaberAsPrimary(&self->client->ps))
	...
	{
		SabBeh_AnimateMassiveBounce(self, otherOwner);
	}
	else if (mechSelf.doMassiveSlowBounce)
	{
		SabBeh_AnimateMassiveSlowBounce(self, qtrue);
	...
	else if (mechOther.dofatiguedStun)
	{//we stunned them, do the knockaway animation
		AnimateoldKnockaway(self, otherOwner, tr.endpos);
	}
	else if (mechSelf.doParry)
	{
		AnimateKnockaway(self, otherOwner, tr.endpos);
	}
	else if (mechSelf.dofatiguedParry)
	{
		AnimateoldKnockaway(self, otherOwner, tr.endpos);
	}
	else if (mechSelf.doNormalBounce)
	{
	...
	{//Didn't go into a special animation, so do a saber lock/bounce/deflection/etc that's approprate
		if (otherOwner && otherOwner->inuse && otherOwner->client
			...
			&& !mechOther.doMassiveSlowBounce
			&& !mechOther.doMassiveBounce
			&& !mechOther.doSlowBounce
			&& !mechOther.doParry
			&& !mechOther.dofatiguedParry
			&& !mechOther.doNormalBounce
			&& !mechOther.doCrushTheParry
			&& !mechOther.doStun)
		{//saberlock test		
			...
			if (saberHitWall)
			{
				if ((self->client->saber[rSaberNum].saberFlags & SFL_BOUNCE_ON_WALLS)
					&& (PM_SaberInAttackPure(self->client->ps.saberMove) //only in a normal attack anim
						|| self->client->ps.saberMove == LS_A_JUMP_T__B_ || self->client->ps.saberMove == LS_A_JUMP_PALP_)) //or in the strong jump-fwd-attack "death from above" move
				{
				...
				{//reflect from wall
					//do bounce sound & force feedback
					self->client->ps.saberBlocked = BLOCKED_ATK_BOUNCE;
					self->client->ps.saberBounceMove = LS_D1_BR + (saberMoveData[self->client->ps.saberMove].startQuad - Q_BR);
				}
			}
			else
			{
				if (!idleDamage)
				{
					if (self->client->ps.saberLockTime >= level.time  //in a saberlock otherOwner wasn't in an attack and didn't block the blow.
	...
	if (otherOwner && otherOwner->inuse && otherOwner->client)
	{
		...
		else if (mechOther.doMassiveBounce)
		{
			SabBeh_AnimateMassiveBounce(otherOwner, self);
		}
		else if (mechOther.doMassiveSlowBounce)
		{
			SabBeh_AnimateMassiveSlowBounce(otherOwner, qtrue);
		}
		else if (mechOther.doCrushTheParry)
		{
			SabBeh_CrushTheParry(otherOwner);
		}
		else if (mechSelf.doStun)
		{//we stunned them, do the knockaway animation
		...
		{//we stunned them, do the knockaway animation
			AnimateoldKnockaway(otherOwner, self, tr.endpos);
		}
		else if (mechOther.doParry)
		{
			AnimateKnockaway(otherOwner, self, tr.endpos);
		}
		else if (mechOther.dofatiguedParry)
		{
			AnimateoldKnockaway(otherOwner, self, tr.endpos);
		}
		else if (mechOther.doNormalBounce)
		{
			G_SaberBounce(otherOwner, self, qfalse);
		}
		else
	...
	if (incoming)
	{
		if (self->NPC)
		{
			...
			{//a hovering Boba Fett, not a tracking rocket
				...
				&& Jedi_SaberBlockGo(self, &self->NPC->last_ucmd, NULL, NULL, incoming, 0.0f) != EVASION_NONE)
		...
		else//player
		{
			...
			{
				WP_SaberBlockNonRandom(blocker, incoming->r.currentOrigin, qtrue);
			}
			else
			{
				...
				VectorMA(incoming->r.currentOrigin, dist, entDir, start);
...
#define MAX_LEAVE_TIME 5000
#define MAX_BOT_LEAVE_TIME 2500
...
void DownedSaberThink(gentity_t * saberent)
{
	...
	if (saberOwn->client->pers.cmd.buttons & BUTTON_ATTACK && !(saberOwn->r.svFlags & SVF_BOT))
...
{//similar to SaberBounceSound but the saber's owners can also pick up their saber by crouching or rolling over it
	...
	{//racc - picked up the saber.
		...
		G_Sound(self, CHAN_AUTO, G_SoundIndex("sound/weapons/saber/sabercatch.mp3"));
...
#define SABER_BOTRETRIEVE_DELAY 2500
...
void saberKnockDown(gentity_t * saberent, gentity_t * saberOwner, gentity_t * other)
{
	...
	{
		saberOwner->client->saberKnockedTime = level.time + SABER_BOTRETRIEVE_DELAY;
	}
	else
	{
...
qboolean WP_AbsorbKick(gentity_t * self, gentity_t * pusher, const vec3_t pushDir)
{
	...
	if (self->client->ps.BlasterAttackChainCount >= BLASTERMISHAPLEVEL_HEAVY)
	{
		return qfalse;
	}
	...
	if (self->client->ps.saberAttackChainCount >= MISHAPLEVEL_LIGHT && pusher->client->ps.fd.saberAnimLevel == SS_DESANN)
	{
		return qfalse;
	}

	if (self->client->ps.fd.forcePower < FATIGUE_DODGEING)
	{//must have enough force power
		return qfalse;
	}
	...
	if (pusher->client->ps.torsoAnim == BOTH_MELEEUP)//for the slap
	{
		if (pusher->client->ps.fd.forcePower >= 75)
		{
			G_SetAnim(self, &self->client->pers.cmd, SETANIM_BOTH, BOTH_KNOCKDOWN1, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 100);
			WP_ForcePowerDrain(&self->client->ps, FP_SABER_DEFENSE, FATIGUE_KICKHIT);
		}
		else if (Manual_Saberblocking(self))
		{
		...
		{
			G_SetAnim(self, &self->client->pers.cmd, SETANIM_BOTH, BOTH_FLIP_BACK2, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 100);
			WP_ForcePowerDrain(&self->client->ps, FP_SABER_DEFENSE, FATIGUE_KICKHIT);
		}
		AddFatigueDamageBonus(pusher, self);
	}
	else if (pusher->client->ps.torsoAnim == BOTH_WOOKIE_SLAP)//for the slap
	{
		if (pusher->client->ps.fd.forcePower >= 75)
		{
			G_SetAnim(self, &self->client->pers.cmd, SETANIM_BOTH, BOTH_SLAPDOWNLEFT, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 100);
			WP_ForcePowerDrain(&self->client->ps, FP_SABER_DEFENSE, FATIGUE_KICKHIT);
		}
		else if (Manual_Saberblocking(self))
		{
	...
	else if (pusher->client->ps.legsAnim == BOTH_A7_KICK_F)//for the slap
	{
		if (pusher->client->ps.fd.forcePower >= 75)
		{
			G_SetAnim(self, &self->client->pers.cmd, SETANIM_BOTH, BOTH_KNOCKDOWN1, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 100);
			WP_ForcePowerDrain(&self->client->ps, FP_SABER_DEFENSE, FATIGUE_KICKHIT);
			G_AddVoiceEvent(pusher, Q_irand(EV_ANGER1, EV_ANGER3), 5000 + Q_irand(0, 10000));
		}
		else if (Manual_Saberblocking(self))
		{
			G_SetAnim(self, &self->client->pers.cmd, SETANIM_BOTH, BOTH_FLIP_BACK1, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 100);
	...
	else if (pusher->client->ps.legsAnim == BOTH_A7_KICK_F2)//for the slap
	{
		if (pusher->client->ps.fd.forcePower >= 75)
		{
			G_SetAnim(self, &self->client->pers.cmd, SETANIM_BOTH, BOTH_KNOCKDOWN2, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 100);
			WP_ForcePowerDrain(&self->client->ps, FP_SABER_DEFENSE, FATIGUE_KICKHIT);
			G_AddVoiceEvent(pusher, Q_irand(EV_ANGER1, EV_ANGER3), 5000 + Q_irand(0, 10000));
		}
		else if (Manual_Saberblocking(self))
	...
	else if (pusher->client->ps.legsAnim == BOTH_A7_KICK_B)//for the slap
	{
		if (pusher->client->ps.fd.forcePower >= 75)
		{
			G_SetAnim(self, &self->client->pers.cmd, SETANIM_BOTH, BOTH_KNOCKDOWN1, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 100);
	...
	else if (pusher->client->ps.legsAnim == BOTH_A7_KICK_B2)//for the slap
	{
		if (pusher->client->ps.fd.forcePower >= 75)
		{
			G_SetAnim(self, &self->client->pers.cmd, SETANIM_BOTH, BOTH_KNOCKDOWN5, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 100);
			WP_ForcePowerDrain(&self->client->ps, FP_SABER_DEFENSE, FATIGUE_KICKHIT);
		}
		else if (Manual_Saberblocking(self))
		{
	...
	else if (pusher->client->ps.legsAnim == BOTH_A7_KICK_B3)//for the slap
	{
		if (pusher->client->ps.fd.forcePower >= 75)
		{
			G_SetAnim(self, &self->client->pers.cmd, SETANIM_BOTH, BOTH_KNOCKDOWN4, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 100);
			WP_ForcePowerDrain(&self->client->ps, FP_SABER_DEFENSE, FATIGUE_KICKHIT);
		}
		else if (Manual_Saberblocking(self))
		{
	...
	else if (pusher->client->ps.legsAnim == BOTH_A7_KICK_R)//for the slap
	{
		if (pusher->client->ps.fd.forcePower >= 75)
		{
			G_SetAnim(self, &self->client->pers.cmd, SETANIM_BOTH, BOTH_KNOCKDOWN2, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 100);
	...
	else if (pusher->client->ps.torsoAnim == BOTH_A7_SLAP_R)//for the slap
	{
		if (pusher->client->ps.fd.forcePower >= 75)
		{
			G_SetAnim(self, &self->client->pers.cmd, SETANIM_BOTH, BOTH_SLAPDOWNRIGHT, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 100);
			WP_ForcePowerDrain(&self->client->ps, FP_SABER_DEFENSE, FATIGUE_KICKHIT);
		}
		else if (Manual_Saberblocking(self))
		{
		...
		{
			G_SetAnim(self, &self->client->pers.cmd, SETANIM_BOTH, BOTH_BASHED1, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 100);
			WP_ForcePowerDrain(&self->client->ps, FP_SABER_DEFENSE, FATIGUE_KICKHIT);
		}
		AddFatigueDamageBonus(pusher, self);
	}
	else if (pusher->client->ps.legsAnim == BOTH_A7_KICK_L)//for the slap
	{
		if (pusher->client->ps.fd.forcePower >= 75)
		{
			G_SetAnim(self, &self->client->pers.cmd, SETANIM_BOTH, BOTH_KNOCKDOWN2, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 100);
	...
	else if (pusher->client->ps.torsoAnim == BOTH_A7_SLAP_L)//for the slap
	{
		if (pusher->client->ps.fd.forcePower >= 75)
		{
			G_SetAnim(self, &self->client->pers.cmd, SETANIM_BOTH, BOTH_SLAPDOWNLEFT, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 100);
	...
	else if (pusher->client->ps.torsoAnim == BOTH_TUSKENLUNGE1)//for the slap
	{
		if (pusher->client->ps.fd.forcePower >= 75)
		{
			G_SetAnim(self, &self->client->pers.cmd, SETANIM_BOTH, BOTH_SLAPDOWNRIGHT, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 100);
	...
	else if (pusher->client->ps.torsoAnim == BOTH_TUSKENATTACK1)//for the slap
	{
		if (pusher->client->ps.fd.forcePower >= 75)
		{
			G_SetAnim(self, &self->client->pers.cmd, SETANIM_BOTH, BOTH_KNOCKDOWN1, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 100);
	...
	else if (pusher->client->ps.torsoAnim == BOTH_TUSKENATTACK2)//for the slap
	{
		if (pusher->client->ps.fd.forcePower >= 75)
		{
			G_SetAnim(self, &self->client->pers.cmd, SETANIM_BOTH, BOTH_KNOCKDOWN2, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 100);
	...
	else if (pusher->client->ps.torsoAnim == MELEE_STANCE_HOLD_T ||
		...
		pusher->client->ps.torsoAnim == MELEE_STANCE_B)//for the KARATE	
	{
		if (pusher->client->ps.fd.forcePower >= 75)
		{
			G_SetAnim(self, &self->client->pers.cmd, SETANIM_BOTH, BOTH_KNOCKDOWN2, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 100);
	...
	{
		if (pusher->client->ps.fd.forcePower >= 50)
		{
			G_SetAnim(self, &self->client->pers.cmd, SETANIM_BOTH, BOTH_KNOCKDOWN5, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 100);
			WP_ForcePowerDrain(&self->client->ps, FP_SABER_DEFENSE, FATIGUE_KICKHIT);
		}
	...
	{//if anything else other than kick/slap/roundhouse or karate
		G_SetAnim(self, &self->client->pers.cmd, SETANIM_BOTH, BOTH_FLIP_BACK3, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 100);
		WP_ForcePowerDrain(&self->client->ps, FP_SABER_DEFENSE, FATIGUE_KICKHIT);
		AddFatigueDamageBonus(pusher, self);
	}
	return qtrue;
...
gentity_t* G_KickTrace(gentity_t * ent, vec3_t kickDir, float kickDist, vec3_t kickEnd, int kickDamage, float kickPush, qboolean doSoundOnWalls)
{
	...
	{//extrude
		VectorSet(traceOrg, ent->r.currentOrigin[0], ent->r.currentOrigin[1], ent->r.currentOrigin[2] + ent->maxs[2] * 0.5f);
		...
		{//we hit an entity
			...
			if (hitEnt->client)
			{
				if (!(hitEnt->client->ps.pm_flags & PMF_TIME_KNOCKBACK)
					&& TIMER_Done(hitEnt, "kickedDebounce") && G_CanBeEnemy(ent, hitEnt))//not already flying through air?  Intended to stop multiple hits, but...
				{//FIXME: this should not always work
					...
					{//hurt it
						if (hitEnt->client)
						{
							...
							{//kicks do fatigue damage if they were stunned.
								WP_ForcePowerDrain(&hitEnt->client->ps, FP_SABER_DEFENSE, FATIGUE_KICKHIT);
							}
						}
					...
					{//hit in head
						...
						{//knock down
							...
							{//knock them down
								...
								G_Knockdown(hitEnt, ent, kickDir, 150, qtrue);
							}
							else
					...
					else
					{
						...
						if ((hitEnt->client->ps.saberAttackChainCount >= MISHAPLEVEL_HEAVY ||
							...
							&& !(hitEnt->client->buttons & BUTTON_BLOCK))
						{//knockdown
							...
							else
							{
								...
								{
									G_Knockdown(hitEnt, ent, kickDir, 75, qtrue);
								}
								else
								{
...
static void G_PunchSomeMofos(gentity_t * ent)
{
	...
	{//safety check, this shouldn't ever not be valid
		...
		trap->Trace(&tr, ent->client->ps.origin, mins, maxs, Hand, ent->s.number, MASK_SHOT, qfalse, G2TRFLAG_DOGHOULTRACE | G2TRFLAG_GETSURFINDEX | G2TRFLAG_THICK | G2TRFLAG_HITCORPSES, g_g2TraceLod.integer);
...
static void G_KickSomeMofos(gentity_t * ent)
{
	...
	float kickDist = (ent->r.maxs[0] * 1.5f) + ((m_nerf.integer & (1 << EOC_STAFFKICK)) ? STAFF_KICK_RANGE : 2.25 * STAFF_KICK_RANGE) + 8.0f;//fudge factor of 8
	...
	else
	{
		...
		case BOTH_ARIAL_RIGHT:
			...
			{//lead leg
				int footBolt = (ent->client->ps.legsAnim == BOTH_ARIAL_LEFT) ? ri->footLBolt : ri->footRBolt;//mirrored anims
				doKick = qtrue;
				if (footBolt != -1)
				{//actually trace to a bolt
		...
		case BOTH_CARTWHEEL_RIGHT:
			if (elapsedTime >= 200 && elapsedTime <= 600)
			{//lead leg
				int footBolt = (ent->client->ps.legsAnim == BOTH_CARTWHEEL_LEFT) ? ri->footLBolt : ri->footRBolt;//mirrored anims
				doKick = qtrue;
				if (footBolt != -1)
				{//actually trace to a bolt
					G_GetBoltPosition(ent, footBolt, kickEnd, 0);
		...
		case BOTH_BUTTERFLY_FR1:
			...
			{//lead leg
				int footBolt = (ent->client->ps.legsAnim == BOTH_BUTTERFLY_FL1) ? ri->footLBolt : ri->footRBolt;//mirrored anims
				doKick = qtrue;
				if (footBolt != -1)
				{//actually trace to a bolt
					G_GetBoltPosition(ent, footBolt, kickEnd, 0);
		...
		case BOTH_BUTTERFLY_RIGHT:
			...
			{//lead leg
				int footBolt = (ent->client->ps.legsAnim == BOTH_BUTTERFLY_LEFT) ? ri->footLBolt : ri->footRBolt;//mirrored anims
				doKick = qtrue;
				if (footBolt != -1)
				{//actually trace to a bolt
					G_GetBoltPosition(ent, footBolt, kickEnd, 0);
		...
		case BOTH_A7_SLAP_R:
			if (level.framenum & 1)
			{//back
				int handBolt = (ent->client->ps.legsAnim == BOTH_A7_SLAP_R) ? ri->handRBolt : ri->handLBolt;//mirrored anims
				doKick = qtrue;
				kickDist = 80;
				if (handBolt != -1)//changed to accomadate teh new hand hand anim
				{//actually trace to a bolt
					G_GetBoltPosition(ent, handBolt, kickEnd, 0);//changed to accomadate teh new hand hand anim
					VectorSubtract(kickEnd, ent->r.currentOrigin, kickDir);
					kickDir[2] = 0;//ah, flatten it, I guess...
					VectorNormalize(kickDir);
		...
		case BOTH_A7_SLAP_L:
			if (level.framenum & 1)
			{//back
				int handBolt = (ent->client->ps.legsAnim == BOTH_A7_SLAP_L) ? ri->handLBolt : ri->handRBolt;//mirrored anims
				doKick = qtrue;
				kickDist = 80;
				if (handBolt != -1)//changed to accomadate teh new hand hand anim
				{//actually trace to a bolt
	...
	{ //do the thrown-saber stuff
		...
		if (saberent)
		{
			if (!self->client->ps.saberEntityState && self->client->ps.saberEntityNum)
			{
				...
				if (self->client->ps.fd.saberAnimLevel == SS_DESANN || self->client->ps.fd.saberAnimLevel == SS_TAVION)
				{
					saberent->s.apos.trType = TR_LINEAR;
						...
						while (!gotHit)
						{
							if (!CheckSaberDamage(self, rSaberNum, rBladeNum, boltOrigin, end, qfalse, trMask, qfalse))
							{
								if (!CheckSaberDamage(self, rSaberNum, rBladeNum, boltOrigin, end, qtrue, trMask, qfalse))
								{
									...
									if ((deltaX != 0 || deltaY != 0 || deltaZ != 0) && deltaX < 180 && deltaY < 180 && deltaZ < 180 && (PM_SaberInAttack(self->client->ps.saberMove) || PM_SaberInTransition(self->client->ps.saberMove)))
...
int Manual_Saberblocking(gentity_t * defender)
{
	...
	if (!(defender->client->buttons & BUTTON_BLOCK))
	{
		...
		{//bots just randomly parry to make up for them not intelligently parrying.
			if (defender->client->ps.fd.forcePower <= 50)
			{
				blockFactor = 0.7f;
			}
		...
		else if (defender->s.eType == ET_NPC)
		{//bots just randomly parry to make up for them not intelligently parrying.
			if (defender->client->ps.fd.forcePower <= 50)
			{
				blockFactor = 0.7f;
		...
		else if (defender->s.number >= MAX_CLIENTS && !G_ControlledByPlayer(defender))
		{//bots just randomly parry to make up for them not intelligently parrying.
			if (defender->client->ps.fd.forcePower <= 50)
			{
				blockFactor = 0.7f;
...
int Manual_HandBlocking(gentity_t * defender)
{
	...
	if (defender->client->ps.fd.forcePowerLevel[FP_ABSORB] < FORCE_LEVEL_2)
	{
		return qfalse;
	}
	...
	if (!(defender->client->buttons & BUTTON_BLOCK))
	{
		if (defender->r.svFlags & SVF_BOT)
		{//bots just randomly parry to make up for them not intelligently parrying.
			if (defender->client->ps.fd.forcePower >= 75)
			{
				blockFactor = 0.7f;
			}
			else
...
qboolean WP_SaberBlockNonRandom(gentity_t * self, vec3_t hitloc, qboolean missileBlock)
{
	...
	if (self->client->ps.weaponstate == WEAPON_DROPPING ||
		self->client->ps.weaponstate == WEAPON_RAISING)
	{//don't block
		if (self->health > 0 && self->r.svFlags & SVF_BOT)
		{
			blockFactor = 0.7f;
		}
		else if (self->health > 0 && self->s.eType == ET_NPC)
		{
			blockFactor = 0.7f;
		}
		else
	...
	if (!inFront && self->client->ps.fd.forcePowerLevel[FP_SABER_DEFENSE] >= FORCE_LEVEL_1)
	{
		...
		{
		case SS_STAFF:
			G_SetAnim(self, &self->client->pers.cmd, SETANIM_TORSO, BOTH_P7_S1_B1_, AFLAG_PACE, 0);
			break;
		case SS_DUAL:
			G_SetAnim(self, &self->client->pers.cmd, SETANIM_TORSO, BOTH_P6_S1_B1_, AFLAG_PACE, 0);
			break;
		default:
			G_SetAnim(self, &self->client->pers.cmd, SETANIM_TORSO, BOTH_P1_S1_B1_, AFLAG_PACE, 0);
			break;
		}
	...
	else if (zdiff > -5)
	{
		if (rightdot > 0.3)
		{
			...
			{
			case SS_STAFF:
				G_SetAnim(self, &self->client->pers.cmd, SETANIM_TORSO, BOTH_P7_S7_TR, AFLAG_PACE, 0);
				self->client->ps.weaponTime = Q_irand(300, 600);
				break;
			case SS_DUAL:
				G_SetAnim(self, &self->client->pers.cmd, SETANIM_TORSO, BOTH_P6_S6_TR, AFLAG_PACE, 0);
				self->client->ps.weaponTime = Q_irand(300, 600);
				break;
			default:
				if (saber1->type == SABER_ANAKIN)
				{
					G_SetAnim(self, &self->client->pers.cmd, SETANIM_TORSO, BOTH_BLOCK_BACK_R, AFLAG_PACE, 0);
					self->client->ps.weaponTime = Q_irand(300, 600);
				}
				...
				{
					G_SetAnim(self, &self->client->pers.cmd, SETANIM_TORSO, BOTH_K1_S1_TR, AFLAG_PACE, 0);
				}
				else
				{
					G_SetAnim(self, &self->client->pers.cmd, SETANIM_TORSO, BOTH_P1_S1_TR, AFLAG_PACE, 0);
					self->client->ps.weaponTime = Q_irand(300, 600);
				}
				break;
			}
		...
		else if (rightdot < -0.3)
		{
			...
			{
			case SS_STAFF:
				G_SetAnim(self, &self->client->pers.cmd, SETANIM_TORSO, BOTH_P7_S7_TL, AFLAG_PACE, 0);
				self->client->ps.weaponTime = Q_irand(300, 600);
				break;
			case SS_DUAL:
				G_SetAnim(self, &self->client->pers.cmd, SETANIM_TORSO, BOTH_P6_S6_TL, AFLAG_PACE, 0);
				self->client->ps.weaponTime = Q_irand(300, 600);
				break;
			default:
				if (saber1->type == SABER_ANAKIN)
				{
					G_SetAnim(self, &self->client->pers.cmd, SETANIM_TORSO, BOTH_BLOCK_BACK_L, AFLAG_PACE, 0);
					self->client->ps.weaponTime = Q_irand(300, 600);
				}
				else if ((saber1->type == SABER_DOOKU)
					|| (saber1->type == SABER_REY)
					|| (saber1->type == SABER_PALP))
				{
					G_SetAnim(self, &self->client->pers.cmd, SETANIM_TORSO, BOTH_K1_S1_TL, AFLAG_PACE, 0);
				}
				else
				{
					G_SetAnim(self, &self->client->pers.cmd, SETANIM_TORSO, BOTH_P1_S1_TL, AFLAG_PACE, 0);
					self->client->ps.weaponTime = Q_irand(300, 600);
				}
				break;
			}
		...
		else
		{
			...
			{
			case SS_STAFF:
				G_SetAnim(self, &self->client->pers.cmd, SETANIM_TORSO, BOTH_P7_S7_T_, AFLAG_PACE, 0);
				break;
			case SS_DUAL:
				G_SetAnim(self, &self->client->pers.cmd, SETANIM_TORSO, BOTH_P6_S6_T_, AFLAG_PACE, 0);
				break;
			default:
				if ((saber1->type == SABER_DOOKU)
					...
					|| (saber1->type == SABER_PALP))
				{
					G_SetAnim(self, &self->client->pers.cmd, SETANIM_TORSO, BOTH_P1_S1_T1_, AFLAG_PACE, 0);
				}
				else
				{
					G_SetAnim(self, &self->client->pers.cmd, SETANIM_TORSO, BOTH_P1_S1_T_, AFLAG_PACE, 0);
				}
				break;
			}
			self->client->ps.weaponTime = Q_irand(300, 600);
	...
	else
	{
		if (rightdot >= 0)
		{
			...
			{
			case SS_STAFF:
				G_SetAnim(self, &self->client->pers.cmd, SETANIM_TORSO, BOTH_P7_S7_BR, AFLAG_PACE, 0);
				break;
			case SS_DUAL:
				G_SetAnim(self, &self->client->pers.cmd, SETANIM_TORSO, BOTH_P6_S6_BR, AFLAG_PACE, 0);
				break;
			default:
				G_SetAnim(self, &self->client->pers.cmd, SETANIM_TORSO, BOTH_P1_S1_BR, AFLAG_PACE, 0);
				break;
			}
			self->client->ps.weaponTime = Q_irand(300, 600);
		}
		else
		{
			...
			{
			case SS_STAFF:
				G_SetAnim(self, &self->client->pers.cmd, SETANIM_TORSO, BOTH_P7_S7_BL, AFLAG_PACE, 0);
				break;
			case SS_DUAL:
				G_SetAnim(self, &self->client->pers.cmd, SETANIM_TORSO, BOTH_P6_S6_BL, AFLAG_PACE, 0);
				break;
			default:
				G_SetAnim(self, &self->client->pers.cmd, SETANIM_TORSO, BOTH_P1_S1_BL, AFLAG_PACE, 0);
				break;
			}
			self->client->ps.weaponTime = Q_irand(300, 600);
		}
	...
	if (missileBlock)
	{
		if (self->s.number < MAX_CLIENTS || G_ControlledByPlayer(self)) //controlled by player only
		{
			...
			if (self->client->ps.ManualBlockingFlags & (1 << MBF_MBLOCKING))
			{
				...
				if (self->client->ps.fd.blockPoints < BlockPointsUsedUsed)
				{
				...
				{
					self->client->ps.fd.blockPoints -= BlockPointsUsedUsed;
					return qtrue;
				}
			}
		}
		...
		else if (self->client->ps.fd.blockPoints <= BLOCKPOINTS_FAIL)
		{
			self->client->ps.saberMove = LS_NONE;
			self->client->ps.saberBlocked = BLOCKED_NONE;
			self->client->ps.weaponTime = Q_irand(300, 600);
		...
		else if (self->client->ps.fd.blockPoints <= BLOCKPOINTS_DANGER)
		{//drop that saber
			saberKnockOutOfHand(&g_entities[self->client->ps.saberEntityNum], self, vec3_origin);
		}
		else if (self->client->ps.fd.forcePower <= BLOCKPOINTS_FATIGUE)
		{//heavy slow bounce
			SabBeh_AnimateMassiveBoltBounce(self, qtrue);
			self->client->ps.weaponTime = Q_irand(300, 600);
		}
		else  if (self->client->ps.fd.blockPoints <= BLOCKPOINTS_FATIGUE)
		{//heavy slow bounce
			SabBeh_AnimateMassiveBoltBounce(self, qtrue);
			self->client->ps.weaponTime = Q_irand(300, 600);
		}
...
qboolean WP_SaberCanBlock(gentity_t * self, vec3_t point, int dflags, int mod, qboolean projectile, int attackStr)
{
	...
	if (g_Enhanced_saberTweaks.integer & SABERTWEAK_REDUCEBLOCKS)
	{
		...
		const float parity = g_Enhanced_saberBlockStanceParity.value; // range [0, 3]
		const float chanceMin = g_Enhanced_saberBlockChanceMin.value;
		const float chanceMax = g_Enhanced_saberBlockChanceMax.value;
		const float chanceScalar = g_Enhanced_saberBlockChanceScale.value;
...
{//think function for sabers in ballistics mode
 //G_RunObject(saberEnt);
	...
	{//we're stuck in something
		...
		else if (saberOwner->client->pers.cmd.buttons & BUTTON_ATTACK ||
			saberOwner->client->pers.cmd.buttons & BUTTON_BLOCK && !(saberOwner->r.svFlags & SVF_BOT))
		{//we want to pull the saber back.
			saberEnt->s.eFlags &= ~EF_MISSILE_STICK;
 ...
 //the force into a saber that's just ballastically moving.
	...
	{//otherwise, just move by normal ballistic physics
	 //spin just like we were in our saber throw.
		...
		if (saberOwn->client->ps.fd.saberAnimLevel == SS_DESANN || saberOwn->client->ps.fd.saberAnimLevel == SS_TAVION)
		{
			saberEnt->s.apos.trType = TR_LINEAR;
			saberEnt->s.apos.trDelta[0] = 0;
			saberEnt->s.apos.trDelta[1] = 600;
