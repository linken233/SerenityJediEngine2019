...
qboolean NPC_UpdateAngles ( qboolean doPitch, qboolean doYaw )
{
	...
	if ( !NPCS.NPC->enemy && ( (level.time < NPCS.NPCInfo->aimTime) || NPCS.NPC->client->renderInfo.renderFlags & RF_LOCKEDANGLE) )
	{
		if(doPitch)
	...
	if ( checkAlerts )
	{
		...
		if ( alertEvent >= 0 )
		{
			...
			if ( event->level >= AEL_DISCOVERED )
			{
				...
				if ( ( event->owner->client ) && ( event->owner->client->playerTeam == NPCS.NPC->client->playerTeam ) && NPC_ValidEnemy(event->owner))
