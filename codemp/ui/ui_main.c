		...
		if (scl->weapons & (1<<i))
		{
			...
			{ //we want to see what kind of saber they have, and set the cvar based on that
				...
				else
				{
					...
					{
						if ( !Q_stricmp( buf, "SABER_STAFF" ) || !Q_stricmp(buf, "SABER_STAFF_UNSTABLE") || !Q_stricmp(buf, "SABER_STAFF_SFX") || !Q_stricmp(buf, "SABER_STAFF_THIN"))
						{
...
static void UI_RunMenuScript(char **args)
{
	...
	if (String_Parse(args, &name))
	{
		...
		else if (Q_stricmp(name, "ServerSort") == 0)
		{
			...
			if (uiInfo.teamIndex >= 0 && uiInfo.teamIndex < uiInfo.myTeamCount) {
				trap->Cmd_ExecuteText( EXEC_APPEND, va("callteamvote kick \"%s\"\n",uiInfo.teamNames[uiInfo.teamIndex]) );
			}
