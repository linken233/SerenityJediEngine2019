...
#define DROP_CLOAK      16
#define DROP_JETPACK    32
...
void misc_model_cargo_die( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int mod, int dFlags, int hitLoc )
{
	int		flags;
	vec3_t	org, temp;
	gitem_t *health = NULL, *shields = NULL, *bacta = NULL, *batteries = NULL, *cloak = NULL, *jetpack = NULL;
	...
	if ((flags & DROP_CLOAK))
	{
		...
		if (cloak)
		{
			...
			LaunchItem(cloak, temp, vec3_origin, NULL);
		}
	}

	if ((flags & DROP_JETPACK))
	{
		jetpack = FindItem("item_jetpack");
		...
		if (jetpack)
		{
			...
			LaunchItem(jetpack, temp, vec3_origin, NULL);
		}
	}
