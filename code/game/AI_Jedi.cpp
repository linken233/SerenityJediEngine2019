...
void player_Cloak(gentity_t * self)
{
	if (self && self->client)
	{
		if ((!self->client->ps.SaberActive()) || self->client->ps.weapon == WP_NONE || self->client->ps.weapon == WP_MELEE)
		{
			self->flags |= FL_NOTARGET;
		}
		if (!self->client->ps.powerups[PW_CLOAKED])
...
static void Jedi_AdjustSaberAnimLevel(gentity_t * self, int newLevel)
{
	...
	if (self->client->playerTeam == TEAM_ENEMY)
	{
		...
		{//fast only
			self->client->ps.saberAnimLevel = Q_irand(SS_FAST, SS_MEDIUM);
		}
...
{//for many of these checks, what we really want is horizontal distance to enemy
		...
		&& NPC->enemy->client)
	{
		...
		if ( !NPC->client->ps.weaponTime //not firing
		...
		else if (enemy_dist < 80 && ucmd.forwardmove < 0
			...
			&& !(ucmd.buttons & BUTTON_USE_FORCE))
		{
			...
			if (TIMER_Done(NPC, "TalkTime"))
			{
				if (NPC_IsDarkJedi(NPC))
				{// Do taunt/anger...
					G_AddVoiceEvent(NPC, Q_irand(EV_ANGER1, EV_ANGER1), 0);
				}
				else if (NPC_IsLightJedi(NPC))
				{// Do taunt...
					G_AddVoiceEvent(NPC, Q_irand(EV_COMBAT1, EV_COMBAT3), 0);
				}
				else
				{// Do taunt/anger...
					G_AddVoiceEvent(NPC, Q_irand(EV_TAUNT1, EV_TAUNT3), 0);
				}
			}
			TIMER_Set(NPC, "TalkTime", 3000);
		}
		else if (enemy_dist < 70)
		{
			ucmd.buttons |= BUTTON_ATTACK;
			ucmd.buttons &= ~BUTTON_BLOCK;
		}
		...
		&& !PM_InKnockDown(&NPC->client->ps))
	{
		if (NPC->client->ps.torsoAnim == BOTH_A7_SLAP_R ||
			...
			NPC->client->ps.torsoAnim == BOTH_A7_KICK_B3)
		{
			shoot = qfalse;
	...
	{//maintain distance
		...
		if (NPC->client->ps.weapon == WP_SABER //using saber
			...
			&& ((level.time - NPC->client->ps.saberKnockedTime) > MAX_RETURN_TIME)
		...
		&& NPC->enemy->s.apos.trType == TR_STATIONARY)
	{
		...
		&& ((NPCInfo->scriptFlags & SCF_DONT_FIRE)
			|| (!Q_stricmp("T_yoda", NPC->NPC_type)
				|| (!Q_stricmp("jedi_kdm1", NPC->NPC_type)
					|| (!Q_stricmp("RebornBoss", NPC->NPC_type)
						|| (!Q_stricmp("T_Palpatine_sith", NPC->NPC_type)
	...
	{//we're out of striking range and we are allowed to attack
		...
		{//They're being gripped, rush them!
			...
			if (NPC->client->ps.forcePowerLevel[FP_SABERTHROW] > FORCE_LEVEL_2
				&& TIMER_Done(NPC, "attackDelay") || (NPCInfo->rank >= RANK_LT_JG || WP_ForcePowerUsable(NPC, FP_SABERTHROW, 0))
				&& !Q_irand(0, 5)
				&& !(NPC->client->ps.forcePowersActive & (1 << FP_SPEED))
				&& !(NPC->client->ps.saberEventFlags & SEF_INWATER))//saber not in water
			{//throw saber
		...
		else
		{
			...
			{
				int chanceScale = 0;
				if ((NPC->client->NPC_class == CLASS_KYLE || NPC->client->NPC_class == CLASS_YODA) && (NPC->spawnflags & 1))
				{
					chanceScale = 4;
				}
				else if (NPC->enemy
				...
				{//else randomly try some kind of attack every now and then
						...
						&& (!Q_irand(0, 1) || NPC->s.weapon != WP_SABER))
					{
							...
							&& (((NPCInfo->scriptFlags & SCF_DONT_FIRE) && Q_stricmp("cultist_lightning", NPC->NPC_type)) || Q_irand(0, 1)))
						{
							...
							&& (((NPCInfo->scriptFlags & SCF_DONT_FIRE) && Q_stricmp("md_snoke_tfa", NPC->NPC_type)) || Q_irand(0, 1)))
						{
							ForceLightning(NPC);
							if (NPC->client->ps.forcePowerLevel[FP_LIGHTNING] > FORCE_LEVEL_1)
							{
							...
							&& InFOV(NPC->enemy->currentOrigin, NPC->currentOrigin, NPC->client->ps.viewangles, 20, 30))
						{
							...
							if (TIMER_Done(NPC, "chatter") && jediSpeechDebounceTime[NPC->client->playerTeam] < level.time && NPCInfo->blockedSpeechDebounceTime < level.time)
							{
								...
								jediSpeechDebounceTime[NPC->client->playerTeam] = NPCInfo->blockedSpeechDebounceTime = level.time + 1000;
								if ((NPCInfo->aiFlags& NPCAI_ROSH))
								{
									TIMER_Set(NPC, "chatter", 3000);
								}
		...
		{//either it will miss by a bit (and 25% chance) OR our saber is not in-hand OR saber is off
			...
			{//acrobat or fencer or above
				if (self->client->ps.groundEntityNum != ENTITYNUM_NONE &&//on the ground
					...
					&& self->client->ps.forcePower > FATIGUE_DODGEINGBOT
					&& (self->client->ps.saberInFlight ||
					(self->client->NPC_class == CLASS_REBORN && self->s.weapon != WP_SABER) ||
						(self->client->NPC_class == CLASS_SITHLORD && self->s.weapon != WP_SABER) ||
...
static evasionType_t Jedi_CheckEvadeSpecialAttacks(void)
{
	...
	{//check some other attacks?
		...
		if (NPC->enemy->client->ps.torsoAnim == BOTH_ROLL_STAB
			|| (NPC->enemy->client->ps.torsoAnim == BOTH_ROLL_F || NPC->enemy->client->ps.torsoAnim == BOTH_ROLL_F1 || NPC->enemy->client->ps.torsoAnim == BOTH_ROLL_F2
				&& ((NPC->enemy->client->pers.lastCommand.buttons & BUTTON_ATTACK) || (NPC->enemy->client->ps.pm_flags & PMF_ATTACK_HELD))))
...
static qboolean Jedi_SaberBlock(void)
{
	...
	if (Distance(NPC->currentOrigin, NPC->enemy->currentOrigin) > 64
		|| !(NPC->enemy->client->pers.cmd.buttons & BUTTON_ATTACK))
...
static qboolean Jedi_AttackDecide(int enemy_dist)
{
	...
	if (!(ucmd.buttons & BUTTON_ATTACK) && !(ucmd.buttons & BUTTON_ALT_ATTACK) && !(ucmd.buttons & BUTTON_FORCE_FOCUS && !(ucmd.buttons & BUTTON_DASH)))
	{//not already attacking
		if (CanShoot(NPC->enemy, NPC))
		{
			...
			{//Try to attack
				...
				{
					if (NPC->client->ps.blockPoints < BLOCKPOINTS_HALF)
					...
					else
					{
						...
						{//Fake a force-jump
							...
							if (Jedi_Jump(dest, goal->s.number))
							{
								...
								{
									if (NPC->client->ps.forcePowerLevel[FP_SPEED] < FORCE_LEVEL_3)
									{//short burst
...
static void Jedi_CheckEnemyMovement(float enemy_dist)
{
	...
	if (!(NPCInfo->aiFlags & NPCAI_BOSS_CHARACTER))
	{
		...
		{//enemy is mad at *me*
			...
			else if (NPC->enemy->client->ps.legsAnim == BOTH_A2_STABBACK1 || NPC->enemy->client->ps.legsAnim == BOTH_A2_STABBACK1B)
...
void RT_CheckJump(void)
{
	...
	{//hunt him down
		...
		if ((Jedi_Hunt() || Jedi_Track()) && !(NPCInfo->aiFlags & NPCAI_BLOCKED))
			...
			{//hunt him down
				...
				{//can macro-navigate to him
					if (enemy_dist < 384 && !Q_irand(0, 10) && NPCInfo->blockedSpeechDebounceTime < level.time&& jediSpeechDebounceTime[NPC->client->playerTeam] < level.time && !NPC_ClearLOS(NPC->enemy))
					{
						if (!NPC_IsDarkJedi(NPC))
						{
							G_AddVoiceEvent(NPC, Q_irand(EV_GIVEUP1, EV_GIVEUP4), 2000);
						}
						else
						{
							G_AddVoiceEvent(NPC, Q_irand(EV_JLOST1, EV_JLOST3), 10000);
						}
						jediSpeechDebounceTime[NPC->client->playerTeam] = NPCInfo->blockedSpeechDebounceTime = level.time + 3000;
					}
					if (NPC->client && (NPC->client->NPC_class == CLASS_BOBAFETT || NPC->client->NPC_class == CLASS_MANDO))
	...
	{//saber is not in hand
		...
		{//
			...
			{//fell to the ground, try to pick it up...
				if (Jedi_CanPullBackSaber(NPC) && ((level.time - NPC->client->ps.saberKnockedTime) > MAX_RETURN_TIME))
				{
...
static void Jedi_Attack(void)
{
	...
	if (NPC->enemy)
	{
		if (NPC->enemy->health <= 0
			&& NPC->enemy->enemy == NPC
			&& (NPC->client->playerTeam != TEAM_PLAYER || ((NPC->client->NPC_class == CLASS_KYLE || NPC->client->NPC_class == CLASS_YODA) && (NPC->spawnflags & 1) && NPC->enemy == player)))//good guys don't gloat (unless it's Kyle having just killed his student
	...
	if ((NPCInfo->aiFlags&NPCAI_BOSS_CHARACTER) || (NPCInfo->aiFlags&NPCAI_SUBBOSS_CHARACTER) || NPC->s.weapon == WP_SABER)
	{
			...
			&& NPC->client->NPC_class != CLASS_ROCKETTROOPER)
		{
			...
			{//Tavion will kick in force speed if the player does...
					...
					&& !(NPC->client->ps.forcePowersActive & (1 << FP_SPEED)))
				{
					...
					if (!Q_irand(0, chance))
					{
						if (IsPressingDashButton(NPC))
						{
							ForceSpeedDash(NPC, 0);
						}
						else
						{
							ForceSpeed(NPC, 0);
...
void NPC_BSJedi_Default(void)
{
	...
	{//have an enemy
		...
		if (NPC->client->NPC_class == CLASS_BOBAFETT || NPC->client->NPC_class == CLASS_MANDO)
		{
			if (NPC->enemy->enemy != NPC && NPC->health == NPC->client->pers.maxHealth && DistanceSquared(NPC->currentOrigin, NPC->enemy->currentOrigin) > (800 * 800))
			{
		...
		{// Normal non-jedi enemy
			...
			{// Try to heal...
				ForceHeal(NPC);
				TIMER_Set(NPC, "heal", Q_irand(5000, 10000));
