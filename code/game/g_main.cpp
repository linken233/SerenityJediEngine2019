...
extern void WP_SaberLoadParms2(void);
...
void G_InitCvars(void) {
	...
	g_spskill = gi.cvar("g_spskill", "1", CVAR_ARCHIVE | CVAR_SAVEGAME | CVAR_NORESTART);
	...
	g_saberRealisticCombat = gi.cvar("g_saberRealisticCombat", "2", CVAR_ARCHIVE);//makes collision more precise, increases damage
	...
	g_saberDamageCapping = gi.cvar("g_saberDamageCapping", "1", CVAR_ARCHIVE);//caps damage of sabers vs players and NPC who use sabers
	g_saberMoveSpeed = gi.cvar("g_saberMoveSpeed", "1", CVAR_ARCHIVE);//how fast you run while attacking with a saber
	g_saberAnimSpeed = gi.cvar("g_saberAnimSpeed", "0.97", CVAR_ARCHIVE);//how fast saber animations run
	...
	com_outcast = gi.cvar("com_outcast", "0", CVAR_ARCHIVE);
	// Items
	...
	g_saberDarkSideSaberColor = gi.cvar("g_saberDarkSideSaberColor", "1", CVAR_ARCHIVE);	//when you turn evil, it turns your saber red!
	...
	g_InvertedHolsteredSabers = gi.cvar("g_InvertedHolsteredSabers", "0", CVAR_ARCHIVE);//if 1, saber faces up when holstered not down
			...
			if (ent->client->ps.blockPoints < BLOCK_POINTS_MAX)
			{
				if (ent->client->ps.weapon == WP_SABER && !Manual_Saberblocking(ent))
				{
					if (ent->client->ps.saberBPDebRecharge < level.time)
					{
						...
						int rechargeDifference = 150 - ent->client->ps.saber[0].BPregenRate;
						if (ent->client->ps.SaberActive())
						{
							rechargeDifference += 300 - ent->client->ps.saber[1].BPregenRate;
						}
						else if (PM_CrouchAnim(ent->client->ps.legsAnim))
						{
							rechargeDifference += 5 - ent->client->ps.saber[1].BPregenRate;
						}
						else if (PM_RestAnim(ent->client->ps.legsAnim))
						{
							rechargeDifference += 1 - ent->client->ps.saber[1].BPregenRate;
						}
