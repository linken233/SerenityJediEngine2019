...
void Svcmd_SaberAttackCycle_f(void)
{
	...
	if ((self->client->ps.saber[0].type == SABER_STAFF_MAUL) && self->client->ps.saberAnimLevel == SS_STAFF)
	{
		return;
	}
	...
	if (self->s.clientNum >= MAX_CLIENTS//not the player
		&& !PM_ControlledByPlayer() && (self->client->ps.saber[0].type == SABER_STAFF) && self->client->ps.saberAnimLevel == SS_STAFF)
	{
		return;
	}

	if (self->s.clientNum >= MAX_CLIENTS//not the player
		&& !PM_ControlledByPlayer() && (self->client->ps.saber[0].type == SABER_ELECTROSTAFF) && self->client->ps.saberAnimLevel == SS_STAFF)
	{
		return;
	}

	if (self->s.clientNum >= MAX_CLIENTS//not the player
		&& !PM_ControlledByPlayer() && (self->client->ps.saber[0].type == SABER_STAFF_UNSTABLE) && self->client->ps.saberAnimLevel == SS_STAFF)
	{
		return;
	}

	if (self->s.clientNum >= MAX_CLIENTS//not the player
		&& !PM_ControlledByPlayer() && (self->client->ps.saber[0].type == SABER_STAFF_MAUL) && self->client->ps.saberAnimLevel == SS_STAFF)
	{
		return;
	}

	if (self->s.clientNum >= MAX_CLIENTS//not the player
		&& !PM_ControlledByPlayer() && (self->client->ps.saber[0].type == SABER_SINGLE) && self->client->ps.saberAnimLevel == SS_DUAL)
	{
		return;
	}
		...
		&& sanityCheck < SS_NUM_SABER_STYLES + 1)
	{
		...
		if (saberAnimLevel > SS_STAFF)
		{
			...
			else if (self->client->ps.saber[0].type == SABER_BACKHAND)//saber backhand
			{
				saberAnimLevel = SS_STAFF;
			}
			else if (self->client->ps.saber[0].type == SABER_ASBACKHAND)//saber backhand
			{
				saberAnimLevel = SS_STAFF;
			}
			else if (self->client->ps.saber[0].type == SABER_STAFF_MAUL)//saber backhand
			{
				saberAnimLevel = SS_STAFF;
			}
			else if (self->client->ps.saber[0].type == SABER_ELECTROSTAFF)//saber backhand
			{
				saberAnimLevel = SS_STAFF;
			}
			else
...
static void Svcmd_ForceSpeed_f(void)
{
	...
	else if (g_entities[0].client->ps.forcePowerLevel[FP_SPEED] > FORCE_LEVEL_1)
	{
		g_entities[0].client->ps.forcePowerLevel[FP_SPEED] = FORCE_LEVEL_1;
	}
}
...
static svcmd_t svcmds[] = {
	...
	//{ "weather",				    Svcmd_Weather_f,					     	CMD_NONE },
