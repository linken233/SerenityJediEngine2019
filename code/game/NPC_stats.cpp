...
qboolean NPC_ParseParms(const char* NPCName, gentity_t * NPC)
{
	...
	if (NPC->NPC)
	{
		...
		if (NPC->client->NPC_class == CLASS_KYLE || com_outcast->integer == 1)
		{
...
void NPC_LoadParms1(void)//jko version
{
	...
	//now load in the .npc definitions
	fileCnt = gi.FS_GetFileList("ext_data/jkonpcs", ".npc", npcExtensionListBuf, sizeof(npcExtensionListBuf));
	...
	for (i = 0; i < fileCnt; i++, holdChar += npcExtFNLen + 1)
	{
		...
		len = gi.FS_ReadFile(va("ext_data/jkonpcs/%s", holdChar), (void**)& buffer);
...
void NPC_LoadParms2(void)//mod version
{
	...
	//now load in the .npc definitions
	fileCnt = gi.FS_GetFileList("ext_data/dfnpcs", ".npc", npcExtensionListBuf, sizeof(npcExtensionListBuf));
	...
	for (i = 0; i < fileCnt; i++, holdChar += npcExtFNLen + 1)
	{
		...
		len = gi.FS_ReadFile(va("ext_data/dfnpcs/%s", holdChar), (void**)& buffer);
...
void NPC_LoadParms3(void)//yav version
{
	...
	//now load in the .npc definitions
	fileCnt = gi.FS_GetFileList("ext_data/yavnpcs", ".npc", npcExtensionListBuf, sizeof(npcExtensionListBuf));
	...
	for (i = 0; i < fileCnt; i++, holdChar += npcExtFNLen + 1)
	{
		...
		len = gi.FS_ReadFile(va("ext_data/yavnpcs/%s", holdChar), (void**)& buffer);
...
void NPC_LoadParms4(void)//eoc version
{
	...
	//now load in the .npc definitions
	fileCnt = gi.FS_GetFileList("ext_data/kotornpcs", ".npc", npcExtensionListBuf, sizeof(npcExtensionListBuf));
	...
	for (i = 0; i < fileCnt; i++, holdChar += npcExtFNLen + 1)
	{
		...
		len = gi.FS_ReadFile(va("ext_data/kotornpcs/%s", holdChar), (void**)& buffer);
