...
int Manual_Saberblocking(gentity_t* defender);
int WP_SaberMustBlock(gentity_t* self, gentity_t* atk, qboolean checkBBoxBlock, vec3_t point, int rSaberNum, int rBladeNum);
...
int WP_AbsorbLightningConversion(gentity_t* attacked, int atdAbsLevel, gentity_t* attacker, int atPower, int atPowerLevel, int atForceSpent);
...
extern qboolean G_ActiveParry(gentity_t* self, gentity_t* attacker, vec3_t hitLoc);
...
extern qboolean SabBeh_BlockvsAttack(gentity_t* blocker, gentity_t* attacker, int saberNum, int bladeNum);
extern qboolean SabBeh_AttackVsBlockAdvanced(gentity_t* attacker, gentity_t* blocker, int saberNum, int bladeNum);
extern qboolean SabBeh_AttackVsBlock(gentity_t* attacker, gentity_t* blocker, int saberNum, int bladeNum);
extern qboolean SabBeh_AttackvsAttack(gentity_t* self, gentity_t* otherowner, int saberNum, int bladeNum);
extern void Advanced_Saberblocking(gentity_t* self, gentity_t* otherOwner, int saberNum, int bladeNum);
...
int forcePowerNeededlevel1[NUM_FORCE_POWERS] =
{
	0,//FP_HEAL,//instant
	10,//FP_LEVITATION,//hold/duration
...
int forcePowerNeededlevel2[NUM_FORCE_POWERS] =
{
	0,//FP_HEAL,//instant
	10,//FP_LEVITATION,//hold/duration
...
void WP_SaberAddHolsteredG2SaberModels(gentity_t* ent, int specificSaberNum)
{
	...
	for (; saberNum <= maxSaber; saberNum++)
	{
		...
		if (saberNum == 0)
		{
			...
			{
				if ((ent->client->ps.saber[0].type == SABER_STAFF) || (ent->client->ps.saber[0].type == SABER_STAFF_MAUL) || (ent->client->ps.saber[0].type == SABER_STAFF_SFX)
...
void WP_SaberLockSound(gentity_t * ent, gentity_t * hitEnt, int saberNum, int bladeNum)
{
	int index = 1;
	if (!ent || !ent->client)
	{
...
void WP_SaberKnockSound(gentity_t * ent, gentity_t * hitEnt, int saberNum, int bladeNum)
{
	int index = 1;
	if (!ent || !ent->client)
	{
		...
		for (int saberNum = 0; saberNum < MAX_SABERS; saberNum++)
		{
			for (int bladeNum = 0; bladeNum < MAX_BLADES; bladeNum++)
			{
				...
				if (!ent->client->ps.saber[saberNum].blade[bladeNum].lengthMax)
				{
					...
					{//shorter saber
						ent->client->ps.saber[saberNum].blade[bladeNum].lengthMax = 38;
					}
					else if (ent->client->NPC_class == CLASS_YODA)
					{//shorter saber
						ent->client->ps.saber[saberNum].blade[bladeNum].lengthMax = 28;
					}
					else
...
static int G_SaberAttackPower(gentity_t *ent, qboolean attacking)
{
	...
	baseLevel = ent->client->ps.saberAnimLevel;
	...
	{ //the attacker gets a boost to help penetrate defense.
		...
		VectorSubtract(ent->client->renderInfo.muzzlePoint, ent->client->renderInfo.muzzlePointOld, vSub);
...
qboolean WP_GetSaberDeflectionAngle(gentity_t * attacker, gentity_t * defender)
{
	...
	if (Manual_Saberblocking(defender) && defender->client->ps.saberBlockingTime > level.time)
	{
	...
	else if (hitDot < 0.25f && hitDot > -0.25f)
...
qboolean WP_SaberApplyDamage(gentity_t * ent, float baseDamage, int baseDFlags, qboolean brokenParry, int saberNum, int bladeNum, qboolean thrownSaber)
{
	...
	if (ent->client->buttons & BUTTON_BLOCK)//jacesolaris2019 test for idlekill
	{
		return qfalse;
	}
				...
				{//actually want to do *some* damage here
					...
					if (totalDmg[i] > 0)
					{
						...
						if (victim->client)
						{
							...
							{
								if (debug_subdivision->integer || g_saberRealisticCombat->integer > 1)
								{
						...
						if (damage > 0 && cg.time)
						{
							...
							G_SaberBounce(ent, victim, qfalse);
							//if not hitting the first model on the enemy, don't do this!
					...
					else
					{
						...
						{//only do these effects if actually trying to damage the thing...
							...
							else
							{
								...
								{
									hitEffect = G_EffectIndex(hit_sparks2);
								}
							}
						}
					}
...
void WP_SaberKnockaway(gentity_t * attacker, trace_t * tr)
{
	...
	WP_SaberKnockSound(attacker, NULL, 0, 0);
...
qboolean G_InCinematicLightningAnim(gentity_t * self)
{
...
#define SABER_COLLISION_DIST 14//was 2//was 4//was 8//was 16
extern qboolean InFront(vec3_t spot, vec3_t from, vec3_t fromAngles, float threshHold = 0.0f);

qboolean WP_SaberDamageForTrace(int ignore, vec3_t start, vec3_t end, float dmg, vec3_t bladeDir, qboolean noGhoul, int attackStrength, saberType_t saberType, qboolean extrapolate, int saberNum, int bladeNum)
{
	trace_t		tr;
	vec3_t		dir;
	...
	if (!noGhoul)
	{
		...
		{//do normal check for larger-size saber traces
			...
			{
				useRadiusForDamage = 14;
			}
		}

		if (useRadiusForDamage > 0)
		{//player,. player allies, shadowtroopers, tavion and desann use larger traces
			...
			gi.trace(&tr, start, traceMins, traceMaxs, end2, ignore, mask, G2_COLLIDE, 10);
		}
		else
		{//reborn use smaller traces
			vec3_t	traceMins = { -14,-14,-14 }, traceMaxs = { 14,14,14 };
			gi.trace(&tr, start, traceMins, traceMaxs, end2, ignore, mask, G2_COLLIDE, 10);
		}
	}
	else
	...
	if (&g_entities[tr.entityNum])
	{
		...
		if (WP_SaberMustBlock(&g_entities[tr.entityNum], owner, qfalse, tr.endpos, -1, -1))
		...
		else if (hitEnt->contents & CONTENTS_LIGHTSABER)
		{
			...
			{//thrown saber hit something
					...
					&& owner->health > 0)
				{
					...
					else if (Manual_Saberblocking(owner))
					{//player takes less damage
						dmg *= 0.20f;
					}
				}
			}
			qboolean sabersIntersect = WP_SabersIntersect(attacker, saberNum, bladeNum, owner, qfalse);
			float sabersDist;

			if (attacker && attacker->client && attacker->client->ps.saberInFlight
				&& owner && owner->s.number == 0 && (g_saberAutoBlocking->integer || Manual_Saberblocking(owner) || attacker->client->ps.saberBlockingTime > level.time))
			{//players have g_saberAutoBlocking or Manual_Saberblocking on, do the more generous check against flying sabers
				sabersDist = 0;
			}
			else
			...
			if (g_saberRealisticCombat->integer > 1 && !in_camera)
			{
		...
		{//thrown saber hit something
			if ((hitEnt && hitEnt->client && hitEnt->health > 0
				&& (hitEnt->client->NPC_class == CLASS_DESANN || hitEnt->client->NPC_class == CLASS_VADER
				...
				|| (owner && owner->client && owner->health > 0
					&& (owner->client->NPC_class == CLASS_DESANN || owner->client->NPC_class == CLASS_VADER
		...
		{
			{
				...
				{//we weren't doing a ghoul trace
					...
					{
						G_PlayEffect(hitEffect, tr.endpos, dir);
						G_SaberBounce(attacker, hitEnt, qfalse);
					}
				}
				else
...
void AnimateoldKnockaway(gentity_t* self, gentity_t* inflictor, vec3_t impact)
{//do an knockaway.
	if (!PM_SaberInKnockaway(self->client->ps.saberMove) && !PM_InKnockDown(&self->client->ps))
	{
		if (!PM_SaberInParry(self->client->ps.saberMove))
...
void SabBeh_AnimateMassiveBoltBounce(gentity_t *hitEnt, qboolean dothebounce)
{
	...
	if (TIMER_Done(hitEnt, "TalkTime"))
	{
		G_AddVoiceEvent(hitEnt, Q_irand(EV_PUSHED1, EV_PUSHED3), Q_irand(3000, 5000));
		TIMER_Set(hitEnt, "TalkTime", Q_irand(3000, 5000));
	}

	int animChoice = Q_irand(0, 6);
	int useAnim = -1;
	...
	if (PM_SaberInMassiveBounce(hitEnt->client->ps.torsoAnim))
	{
		...
		hitEnt->client->ps.weaponTime = Q_irand(300, 600);
...
qboolean WP_SaberParry(gentity_t* blocker, gentity_t* attacker, int saberNum, int bladeNum)
{
	...
	if (blocker->s.number || Manual_Saberblocking(blocker) || CheckManualBlocking(attacker, blocker) || blocker->client->ps.saberBlockingTime > level.time)
...
qboolean WP_BrokenParryKnockDown(gentity_t * victim)
{
	...
	if (victim->client->ps.saberMove == LS_PARRY_UP
		|| victim->client->ps.saberMove == LS_PARRY_FRONT
...
void WP_SaberDamageTrace(gentity_t * ent, int saberNum, int bladeNum)
{
			...
			{//normal attack swing swinging/spinning (or if using strong set), do normal damage
				...
				else
				{
					...
					else
					{
						...
						default:
						case FORCE_LEVEL_5:
							baseDamage = 7.5f * (float)AttackerPowerLevel;
						case FORCE_LEVEL_4: //Staff, medium, duals all do same damage
						case FORCE_LEVEL_3:
							baseDamage = 5.0f * (float)AttackerPowerLevel;
							break;
						case FORCE_LEVEL_2:
							baseDamage = 2.0f * (float)AttackerPowerLevel;
							break;
						case FORCE_LEVEL_1: // Fast damage is so low...
						case FORCE_LEVEL_0:
							baseDamage = 1.5f * (float)AttackerPowerLevel;
							break;
						}
					}
				}
	...
	{//always do damage, and lots of it
		if (g_saberRealisticCombat->integer > 2 && !in_camera)
		{//always do damage, and lots of it
			baseDamage = 20.0f;
		}
	...
	{
		hit_wall = WP_SaberDamageForTrace(ent->s.number, mp2, endNew, baseDamage * 4, md2, qfalse, AttackerPowerLevel, ent->client->ps.saber[saberNum].type, qfalse, saberNum, bladeNum);
	}
	else
	{
		...
		hit_wall = WP_SaberDamageForTrace(ent->s.number, baseOld, baseNew, baseDamage, md2, qfalse, AttackerPowerLevel, ent->client->ps.saber[saberNum].type, qtrue, saberNum, bladeNum);
		...
		while (1)
		{
			...
			for (step = stepsize; step < ent->client->ps.saber[saberNum].blade[bladeNum].length && step < ent->client->ps.saber[saberNum].blade[bladeNum].lengthOld; step += 12)
			{
				...
				if (WP_SaberDamageForTrace(ent->s.number, bladePointOld, bladePointNew, baseDamage, curMD2, qfalse, AttackerPowerLevel, ent->client->ps.saber[saberNum].type, qtrue, saberNum, bladeNum))
				{
					hit_wall = qtrue;
				}
		...
		if (WP_SaberDamageForTrace(ent->s.number, endOld, endNew, tipDmgMod * baseDamage, md2, qfalse, AttackerPowerLevel, ent->client->ps.saber[saberNum].type, qfalse, saberNum, bladeNum))
		{
			hit_wall = qtrue;
		}
	}
	...
	{// The saber (in-hand) hit another saber, mano.
		...
		{
			BlockerPowerLevel = 2 * hitOwner->client->ps.forcePowerLevel[FP_SABER_DEFENSE] + PM_PowerLevelForSaberAnim(&hitOwner->client->ps);
		}
		...
		{//2 in-hand sabers hit
			...
			{//there is damage involved, not just effects
				...
				{//add twoHanded bonus and dualSaber bonus and parryBonus to AttackerPowerLevel here
					AttackerPowerLevel += Q_irand(ent->client->ps.saber[saberNum].parryBonus, ent->client->ps.forcePowerLevel[FP_SABER_DEFENSE]);
				}
				...
				{//add twoHanded bonus and dualSaber bonus and parryBonus to AttackerPowerLevel here
					BlockerPowerLevel += Q_irand(hitOwner->client->ps.saber[saberNum].parryBonus, hitOwner->client->ps.forcePowerLevel[FP_SABER_DEFENSE]);

					if (hitOwner->client->ps.dualSabers && Q_irand(0, 1))
					{//assumes both sabers are defending at same time...?
						BlockerPowerLevel += 1 + Q_irand(hitOwner->client->ps.saber[saberNum].parryBonus, hitOwner->client->ps.forcePowerLevel[FP_SABER_DEFENSE]);
					}
				}
				...
				else if (entAttacking && hitOwnerAttacking
					&& !Q_irand(0, g_saberLockRandomNess->integer)
					&& (g_debugSaberLock->integer || forceLock
						|| AttackerPowerLevel == BlockerPowerLevel
						|| (AttackerPowerLevel > FORCE_LEVEL_2 && BlockerPowerLevel > FORCE_LEVEL_2)
						|| (AttackerPowerLevel < FORCE_LEVEL_3 && BlockerPowerLevel < FORCE_LEVEL_3 && hitOwner->client->ps.forcePowerLevel[FP_SABER_OFFENSE] > FORCE_LEVEL_2 && Q_irand(0, 3))
						|| (AttackerPowerLevel < FORCE_LEVEL_2 && BlockerPowerLevel < FORCE_LEVEL_3 && hitOwner->client->ps.forcePowerLevel[FP_SABER_OFFENSE] > FORCE_LEVEL_1 && Q_irand(0, 2))
						|| (BlockerPowerLevel < FORCE_LEVEL_3 && AttackerPowerLevel < FORCE_LEVEL_3 && ent->client->ps.forcePowerLevel[FP_SABER_OFFENSE] > FORCE_LEVEL_2 && !Q_irand(0, 1))
						|| (BlockerPowerLevel < FORCE_LEVEL_2 && AttackerPowerLevel < FORCE_LEVEL_3 && ent->client->ps.forcePowerLevel[FP_SABER_OFFENSE] > FORCE_LEVEL_1 && !Q_irand(0, 1)))
					&& WP_SabersCheckLock(ent, hitOwner))
				{
					collisionResolved = qtrue;
				}
				else if (hitOwnerAttacking && entDefending
					&& !Q_irand(0, g_saberLockRandomNess->integer * 3)
					&& (g_debugSaberLock->integer || forceLock ||
					((ent->client->ps.saberMove != LS_READY || (BlockerPowerLevel - ent->client->ps.forcePowerLevel[FP_SABER_DEFENSE]) < Q_irand(-6, 0)) &&
					((BlockerPowerLevel < FORCE_LEVEL_3 && ent->client->ps.forcePowerLevel[FP_SABER_DEFENSE] > FORCE_LEVEL_2) ||
					(BlockerPowerLevel < FORCE_LEVEL_2 && ent->client->ps.forcePowerLevel[FP_SABER_DEFENSE] > FORCE_LEVEL_1) ||
					(BlockerPowerLevel < FORCE_LEVEL_3 && ent->client->ps.forcePowerLevel[FP_SABER_DEFENSE] > FORCE_LEVEL_0 && !Q_irand(0, (BlockerPowerLevel - ent->client->ps.forcePowerLevel[FP_SABER_DEFENSE] + 1) * 2)))))
				...
				{//I'm attacking hit, they're parrying
					...
					qboolean ActiveDefenseDefender = (qboolean)(hitOwner->s.number || (g_saberAutoBlocking->integer && Manual_Saberblocking(hitOwner)) || Manual_Saberblocking(hitOwner) || hitOwner->client->ps.ManualBlockingFlags & (1 << MBF_BLOCKING) || hitOwner->client->ps.saberBlockingTime > level.time);

					if (saberHitFraction < 1.0f)
					{//an actual collision

						//The blocker is pressing block
						if (ActiveDefenseDefender) 
						{//JaceSolaris making it feel like EoC MP/OJP.
							...
							ent->client->ps.userInt3 |= (1 << FLAG_PARRIED);

							if (BlockerPowerLevel > AttackerPowerLevel)
							{//blocker is more powerfull than the attacker and blocker is actively blocking

								//make me parry	-(Im the blocker)
								SabBeh_BlockvsAttack(hitOwner, ent, saberNum, bladeNum);

								//The attack is blocked -(Im the attacker)
								SabBeh_AttackVsBlockAdvanced(ent, hitOwner, saberNum, bladeNum);
							}
							else
							{
								//make me parry -(Im the blocker)
								...
								SabBeh_AttackVsBlock(ent, hitOwner, saberNum, bladeNum);
							}							
						}
						else if ((!deflected && Q_irand(0, PM_PowerLevelForSaberAnim(&ent->client->ps) - hitOwner->client->ps.forcePowerLevel[FP_SABER_DEFENSE]) > 0))
						{//broke their parry altogether
							if (Q_irand(0, ent->client->ps.forcePowerLevel[FP_SABER_OFFENSE] - hitOwner->client->ps.forcePowerLevel[FP_SABER_DEFENSE]))
						...
						else //Not pressing block but the sabers contacted
						{//JaceSolaris making it feel like EoC MP/OJP
							if (hitOwner->s.number >= MAX_CLIENTS && !G_ControlledByPlayer(hitOwner))
							{
								SabBeh_BlockvsAttack(hitOwner, ent, saberNum, bladeNum);
							}
							else
							{
								Advanced_Saberblocking(hitOwner, ent, saberNum, bladeNum);
							}
						}
				...
				{//I'm parrying, they're attacking
					...
					qboolean ActiveDefenseAttacker = (qboolean)(ent->s.number || (g_saberAutoBlocking->integer && Manual_Saberblocking(ent)) || Manual_Saberblocking(ent) || ent->client->ps.ManualBlockingFlags & (1 << MBF_BLOCKING) || ent->client->ps.saberBlockingTime > level.time);

					if (saberHitFraction < 1.0f)
					{//an actual collision

						//The attacker is pressing block
						if (ActiveDefenseAttacker)
						{
							deflected = WP_GetSaberDeflectionAngle(hitOwner, ent);
							//just so Jedi knows that he was blocked
							hitOwner->client->ps.saberEventFlags |= SEF_BLOCKED;
							hitOwner->client->ps.userInt3 |= (1 << FLAG_PARRIED);

							if (AttackerPowerLevel > BlockerPowerLevel)
							{
								//make me parry	-(Im the blocker)
								SabBeh_BlockvsAttack(ent, hitOwner, saberNum, bladeNum);

								//if the attack is blocked -(Im the attacker)
								SabBeh_AttackVsBlockAdvanced(hitOwner, ent, saberNum, bladeNum);
							}
							else
							{
								//make me parry -(Im the blocker)
								...
								//if the attack is blocked -(Im the attacker)
								SabBeh_AttackVsBlock(hitOwner, ent, saberNum, bladeNum);
							}
						}
						else if ((!deflected && Q_irand(0, PM_PowerLevelForSaberAnim(&hitOwner->client->ps) - ent->client->ps.forcePowerLevel[FP_SABER_DEFENSE]) > 0))
						{//broke their parry altogether
							if (Q_irand(0, hitOwner->client->ps.forcePowerLevel[FP_SABER_OFFENSE] - ent->client->ps.forcePowerLevel[FP_SABER_DEFENSE]))
							{//chance of continuing with the attack (not bouncing back)
								hitOwner->client->ps.saberEventFlags &= ~SEF_BLOCKED;
								hitOwner->client->ps.saberBounceMove = LS_NONE;
								brokenParry = qtrue;
						...
						{//JaceSolaris making it feel like EoC MP/OJP
							...
							{
								Advanced_Saberblocking(ent, hitOwner, saberNum, bladeNum);
							}
						}
		...
		if (saberHitFraction < 1.0f)
		{
			qboolean activeDefenseent = (qboolean)(ent->s.number || (g_saberAutoBlocking->integer && Manual_Saberblocking(ent)) || Manual_Saberblocking(ent) || ent->client->ps.ManualBlockingFlags & (1 << MBF_BLOCKING) || ent->client->ps.saberBlockingTime > level.time);
			...
			{//some other kind of in-hand saber collision
				...
				{//their saber is in hand
					if (PM_SaberInAttack(hitOwner->client->ps.saberMove)
						|| PM_SaberInSpecialAttack(hitOwner->client->ps.torsoAnim) ||
						(hitOwner->client->ps.saberAnimLevel > SS_MEDIUM && !PM_SaberInIdle(hitOwner->client->ps.saberMove)
						&& !PM_SaberInParry(hitOwner->client->ps.saberMove)	&& !PM_SaberInReflect(hitOwner->client->ps.saberMove)))
					{//in the middle of attacking
		...
		{//either actually hit or locked
			if (ent->client->ps.saberLockTime < level.time)
			{
				...
				else
				{
					...
					{
						WP_SaberKnockSound(ent, hitOwner, saberNum, bladeNum);
					}
				}
				if (!g_saberNoEffects)
				{
			...
			{//we threw a saber and it was blocked, do any effects, etc.
				...
				{//knock it aside and turn it off
					...
					if (hitEnt)
					{
						...
						if (ent->s.number >= MAX_CLIENTS && !G_ControlledByPlayer(ent))
						{
							G_ReflectMissileNPC(ent, &g_entities[ent->client->ps.saberEntityNum], newDir);
						}
						else
						{
							if (Manual_Saberblocking(ent) && ent->client->ps.blockPoints > BLOCKPOINTS_FATIGUE)
							{
								G_ManualBlockMissile(ent, &g_entities[ent->client->ps.saberEntityNum], newDir, FP_SABER_DEFENSE);
							}
							else
							{
	...
	else
	{
		if (hit_wall
			...
			&& (PM_SaberInAttackPure(ent->client->ps.saberMove) //only in a normal attack anim
				|| ent->client->ps.saberMove == LS_A_JUMP_T__B_ || ent->client->ps.saberMove == LS_A_JUMP_PALP_)) //or in the strong jump-fwd-attack "death from above" move
		{//bounce off walls
			ent->client->ps.saberBlocked = BLOCKED_ATK_BOUNCE;
...
void WP_SaberImpact(gentity_t * owner, gentity_t * saber, trace_t * trace)
{
	...
	{//Luke, Desann and Tavion slap thrown sabers aside
		...
		WP_SaberKnockSound(owner, other, 0, 0);
		...
		if (ent_count)
		{
			for (int x = 0; x < ent_count; x++)
			{
				...
				{//bounce it
					...
					{
						if (Manual_Saberblocking(self) && self->client->ps.blockPoints > BLOCKPOINTS_FATIGUE)
						{
							G_ManualBlockMissile(self, missile_list[x], forward, FP_SABER_DEFENSE);
						}
						else
						{
...
{//FIXME: probably need a debounce time
	...
	if (self->client->ps.saberAnimLevel == SS_DESANN || self->client->ps.saberAnimLevel == SS_TAVION)
	{
		saber->s.apos.trDelta[1] = 600;
	}
	else if (self->client->ps.saberAnimLevel == SS_STAFF)
	{
		saber->s.apos.trDelta[1] = 1200;
	}
	else if (self->client->ps.saberAnimLevel == SS_FAST)
	{
	...
	self->client->ps.saberKnockedTime = level.time;
...
{//FIXME: probably need a debounce time
	if (self->health > 0 && !PM_SaberInBrokenParry(self->client->ps.saberMove) && self->client->ps.saberBlocked != BLOCKED_PARRY_BROKEN)
	{
		...
		//play catch sound
		G_Sound(saber, G_SoundIndex("sound/weapons/saber/sabercatch.mp3"));
	...
	{//switched away
		...
		{//it's on the ground, see if it's inside us (touching)
			...
			{//it's in us, pick it up automatically
				if (self->NPC && !G_ControlledByPlayer(self))
				{					
					if ((level.time - self->client->ps.saberKnockedTime) > MAX_RETURN_TIME &&
						!PM_SaberInBrokenParry(self->client->ps.saberMove) && self->client->ps.saberBlocked != BLOCKED_PARRY_BROKEN)
					{
	...
	{//weapon is saber and not flying
		...
		{//we dropped it
			...
			{//we actively want to pick it up or we just switched to it, so pull it back
				...
				if (self->NPC && !G_ControlledByPlayer(self))
				{
					if ((level.time - self->client->ps.saberKnockedTime) > MAX_LEAVE_TIME &&
						!PM_SaberInBrokenParry(self->client->ps.saberMove) && self->client->ps.saberBlocked != BLOCKED_PARRY_BROKEN)
					{
						WP_SaberPull(self, saberent);
					}
...
int WP_SaberBlockCost(gentity_t * defender, gentity_t * attacker, vec3_t hitLocs)
{//returns the cost to block this attack for this attacker/defender combo.
	...
	{//standard bolt block!
			...
			|| attacker->activator && attacker->activator->s.weapon == WP_FLECHETTE)
		{
			...
			{
				float distance = VectorBlockDistance(attacker->activator->currentOrigin, defender->currentOrigin);
				...
				if (defender->client->ps.forcePowerLevel[FP_SABER_OFFENSE] >= FORCE_LEVEL_3 && defender->client->ps.saberAnimLevel != SS_MEDIUM)
				{
					saberBlockCost++;
				}
			...
			{
				saberBlockCost = DODGE_TUSKENBLOCK * 3;
			}
	...
	{//lunge attacks
		saberBlockCost = .75 * BasicSaberBlockCost(attacker->client->ps.saberAnimLevel);
	}
	else if (attacker->client->ps.saberMove == LS_ROLL_STAB)
	{//roll stab
		saberBlockCost = 2 * BasicSaberBlockCost(attacker->client->ps.saberAnimLevel);
	}
	else if (attacker->client->ps.saberMove == LS_A_JUMP_T__B_)
	{//DFA moves
		saberBlockCost = 4 * BasicSaberBlockCost(attacker->client->ps.saberAnimLevel);
	}
	...
	{//"normal" swing moves
		...
		{//attacker is in an attack fake
			if (attacker->client->ps.saberAnimLevel == SS_STRONG
				&& !G_ActiveParry(defender, attacker, hitLocs))
			{//Red does additional DP damage with attack fakes if they aren't parried.
				saberBlockCost = (BasicSaberBlockCost(attacker->client->ps.saberAnimLevel) * 1.35);
			}
			else
			{
				saberBlockCost = (BasicSaberBlockCost(attacker->client->ps.saberAnimLevel) * 1.25);
			}
		}
		else
		{//normal saber block
			saberBlockCost = BasicSaberBlockCost(attacker->client->ps.saberAnimLevel);
		}
	...
	{//attacker is a player so he must have just hit you with a saber blow.
		if (G_ActiveParry(defender, attacker, hitLocs))
		...
		{//player is behind us, costs more to block
		 //staffs back block at normal cost.
			if (defender->client->ps.saberAnimLevel == SS_STAFF && defender->client->ps.forcePowerLevel[FP_SABER_DEFENSE] == FORCE_LEVEL_3)
			{// Having both staff and defense 3 allow no extra back hit damage
				saberBlockCost *= 1;
			}
			else if (defender->client->ps.saberAnimLevel == SS_STAFF && defender->client->ps.forcePowerLevel[FP_SABER_DEFENSE] == FORCE_LEVEL_2)
			{//level 2 defense lowers back damage more
				saberBlockCost *= 1.50;
			}
			else if (defender->client->ps.saberAnimLevel == SS_STAFF && defender->client->ps.forcePowerLevel[FP_SABER_DEFENSE] == FORCE_LEVEL_1)
		...
		if (saberBlockCost > BasicWeaponBlockCosts[MOD_SABER])
		{
			saberBlockCost = BasicWeaponBlockCosts[MOD_SABER];
		}
	}
...
qboolean CheckStagger(gentity_t *defender, gentity_t *attacker)
{
	...
	{// This stuff here is more about timing, allowing levels to make timing easier.
		...
		if (defender->client->ps.forcePowerLevel[FP_SABER_DEFENSE] == FORCE_LEVEL_3)
		{
			defenderStanceStaggerChanceMod *= 1.666f;
		}
		else if (defender->client->ps.forcePowerLevel[FP_SABER_DEFENSE] == FORCE_LEVEL_2)
		{//level 2 defense lowers back damage more
			defenderStanceStaggerChanceMod *= 1.333f;
		}
		else if (defender->client->ps.forcePowerLevel[FP_SABER_DEFENSE] == FORCE_LEVEL_1)
		{//level 1 defense lowers back damage a bit
		...
		{
			float fpPercent = (float)attacker->client->ps.forcePower / (float)attacker->client->ps.forcePowerMax;
...
int WP_SaberMustBlock(gentity_t* self, gentity_t* atk, qboolean checkBBoxBlock, vec3_t point, int rSaberNum, int rBladeNum)
{
	...
	{//player is attacking with saber
		...
		if (PM_SuperBreakWinAnim(atk->client->ps.torsoAnim) && self->client->ps.forcePower < 35)
	...
	{//player attacker, if they are here they're using their saber to attack.  
	 ...
	 //create a line seqment thru the center of the player.
		...
		bodyMax[2] += self->maxs[2];
		bodyMin[2] -= self->mins[2];
		...
		VectorSubtract(atk->client->ps.saber[rSaberNum].blade[rBladeNum].muzzlePoint, atk->client->ps.saber[rSaberNum].blade[rBladeNum].muzzlePointOld, saberMoveDir);
...
int Manual_Saberblocking(gentity_t * defender)
{
	...
	if (!(defender->client->buttons & BUTTON_BLOCK))
	{
		...
		{//npc just randomly block to make up for them not intelligently blocking
			if (defender->client->ps.forcePower <= 50)
			{
				blockFactor = 0.7f;
			}
			else if (defender->methodOfDeath == MOD_FORCE_LIGHTNING)
			{
				return qtrue;
			}
			else
...
int Manual_HandBlocking(gentity_t * defender)
{
	float blockFactor = 0;
	...
	if (defender->client->ps.weapon != WP_NONE
		&& defender->client->ps.weapon != WP_MELEE)
	{
		return qfalse;
	}

	if (defender->client->ps.forcePowerLevel[FP_ABSORB] < FORCE_LEVEL_2)
	{
		return qfalse;
	}
	...
	if (!(defender->client->buttons& BUTTON_BLOCK))
	{
		if (defender->s.number >= MAX_CLIENTS && !G_ControlledByPlayer(defender))
		{//npc just randomly block to make up for them not intelligently blocking
			if (defender->client->ps.forcePower >= 75)
			{
				blockFactor = 0.7f;
			}
			else
...
qboolean WP_SaberBlockNonRandom(gentity_t * self, vec3_t hitloc, qboolean missileBlock)
{
	...
	float rightdot;
	float zdiff;
	qboolean inFront = InFront(hitloc, self->client->ps.origin, self->client->ps.viewangles, -0.7f);
	...
	if (self->client->ps.blockPoints <= BLOCKPOINTS_FAIL)
	{
		return qfalse;
	}
	...
	if (!inFront && self->client->ps.forcePowerLevel[FP_SABER_DEFENSE] >= FORCE_LEVEL_1)
	{
		...
		{
		case SS_STAFF:
			NPC_SetAnim(self, SETANIM_TORSO, BOTH_P7_S1_B_, AFLAG_PACE);
			break;
		case SS_DUAL:
			NPC_SetAnim(self, SETANIM_TORSO, BOTH_P6_S1_B_, AFLAG_PACE);
			break;
		default:
			NPC_SetAnim(self, SETANIM_TORSO, BOTH_P1_S1_B_, AFLAG_PACE);
			break;
		}
	...
	else if (zdiff > -5)
	{
		if (rightdot > 0.3)
		{
			...
			{
			case SS_STAFF:
				NPC_SetAnim(self, SETANIM_TORSO, BOTH_P7_S7_TR, AFLAG_PACE);
				self->client->ps.weaponTime = Q_irand(300, 600);
				break;
			case SS_DUAL:
				NPC_SetAnim(self, SETANIM_TORSO, BOTH_P6_S6_TR, AFLAG_PACE);
				self->client->ps.weaponTime = Q_irand(300, 600);
				break;
			default:
				if (self->client->ps.saber[0].type == SABER_ANAKIN)
				{
					NPC_SetAnim(self, SETANIM_TORSO, BOTH_BLOCK_BACK_R, AFLAG_PACE);
					self->client->ps.weaponTime = Q_irand(300, 600);
				}
				else if ((self->client->ps.saber[0].type == SABER_DOOKU)
					|| (self->client->ps.saber[0].type == SABER_REY)
					|| (self->client->ps.saber[0].type == SABER_PALP))
				{
					NPC_SetAnim(self, SETANIM_TORSO, BOTH_K1_S1_TR, AFLAG_PACE);
				}
				else
				{
					NPC_SetAnim(self, SETANIM_TORSO, BOTH_P1_S1_TR, AFLAG_PACE);
					self->client->ps.weaponTime = Q_irand(300, 600);
				}
				break;
			}
		...
		else if (rightdot < -0.3)
		{
			...
			{
			case SS_STAFF:
				NPC_SetAnim(self, SETANIM_TORSO, BOTH_P7_S7_TL, AFLAG_PACE);
				self->client->ps.weaponTime = Q_irand(300, 600);
				break;
			case SS_DUAL:
				NPC_SetAnim(self, SETANIM_TORSO, BOTH_P6_S6_TL, AFLAG_PACE);
				self->client->ps.weaponTime = Q_irand(300, 600);
				break;
			default:
				if (self->client->ps.saber[0].type == SABER_ANAKIN)
				{
					NPC_SetAnim(self, SETANIM_TORSO, BOTH_BLOCK_BACK_L, AFLAG_PACE);
					self->client->ps.weaponTime = Q_irand(300, 600);
				}
				else if ((self->client->ps.saber[0].type == SABER_DOOKU)
					|| (self->client->ps.saber[0].type == SABER_REY)
					|| (self->client->ps.saber[0].type == SABER_PALP))
				{
					NPC_SetAnim(self, SETANIM_TORSO, BOTH_K1_S1_TL, AFLAG_PACE);
				}
				else
				{
					NPC_SetAnim(self, SETANIM_TORSO, BOTH_P1_S1_TL, AFLAG_PACE);
					self->client->ps.weaponTime = Q_irand(300, 600);
				}
				break;
			}
		...
		else
		{
			...
			{
			case SS_STAFF:
				NPC_SetAnim(self, SETANIM_TORSO, BOTH_P7_S7_T_, AFLAG_PACE);
				break;
			case SS_DUAL:
				NPC_SetAnim(self, SETANIM_TORSO, BOTH_P6_S6_T_, AFLAG_PACE);
				break;
			default:
				if ((self->client->ps.saber[0].type == SABER_DOOKU)
					|| (self->client->ps.saber[0].type == SABER_ANAKIN)
					|| (self->client->ps.saber[0].type == SABER_REY)
					|| (self->client->ps.saber[0].type == SABER_PALP))
				{
					NPC_SetAnim(self, SETANIM_TORSO, BOTH_P1_S1_T1_, AFLAG_PACE);
				}
				else
				{
					NPC_SetAnim(self, SETANIM_TORSO, BOTH_P1_S1_T_, AFLAG_PACE);
				}
				break;
			}
			self->client->ps.weaponTime = Q_irand(300, 600);
	...
	else
	{
		if (rightdot >= 0)
		{
			...
			{
			case SS_STAFF:
				NPC_SetAnim(self, SETANIM_TORSO, BOTH_P7_S7_BR, AFLAG_PACE);
				break;
			case SS_DUAL:
				NPC_SetAnim(self, SETANIM_TORSO, BOTH_P6_S6_BR, AFLAG_PACE);
				break;
			default:
				NPC_SetAnim(self, SETANIM_TORSO, BOTH_P1_S1_BR, AFLAG_PACE);
				break;
			}
			self->client->ps.weaponTime = Q_irand(300, 600);
		}
		else
		{
			...
			{
			case SS_STAFF:
				NPC_SetAnim(self, SETANIM_TORSO, BOTH_P7_S7_BL, AFLAG_PACE);
				break;
			case SS_DUAL:
				NPC_SetAnim(self, SETANIM_TORSO, BOTH_P6_S6_BL, AFLAG_PACE);
				break;
			default:
				NPC_SetAnim(self, SETANIM_TORSO, BOTH_P1_S1_BL, AFLAG_PACE);
				break;
			}
			self->client->ps.weaponTime = Q_irand(300, 600);
		}
	...
	if (missileBlock)
	{
		if (self->s.number < MAX_CLIENTS || G_ControlledByPlayer(self)) //controlled by player only
		{
			int BlockPointsUsedUsed = BLOCKPOINTS_BOLT;

			// modify blockpointsused by how fast we activated projectile block mode, if we did
			if (self->client->ps.ManualBlockingFlags & (1 << MBF_MBLOCKING))
			{
				...
				if (self->client->ps.blockPoints < BlockPointsUsedUsed)
				{
				...
				{
					self->client->ps.blockPoints -= BlockPointsUsedUsed;
					return qtrue;
				}
			}
		}
		else if (self && self->client &&
			(self->client->MassiveBounceAnimTime > level.time ||
		...
		else if (self->client->ps.blockPoints <= BLOCKPOINTS_FAIL)
		{
			self->client->ps.saberMove = LS_NONE;
			self->client->ps.saberBlocked = BLOCKED_NONE;
			self->client->ps.weaponTime = Q_irand(300, 600);
		...
		else if (self->client->ps.blockPoints <= BLOCKPOINTS_DANGER)
		{//drop that saber
			WP_SaberLose(self, throwDir);
		}
		else if (self->client->ps.forcePower <= BLOCKPOINTS_FATIGUE)
		{//heavy slow bounce
			SabBeh_AnimateMassiveBoltBounce(self, qtrue);
			self->client->ps.weaponTime = Q_irand(300, 600);
		}
		else if (self->client->ps.blockPoints <= BLOCKPOINTS_FATIGUE)
		{//heavy slow bounce
			SabBeh_AnimateMassiveBoltBounce(self, qtrue);
			self->client->ps.weaponTime = Q_irand(300, 600);
		}
...
void WP_SaberStartMissileBlockCheck(gentity_t * self, usercmd_t * ucmd)
{
	...
	if (!self->s.number && !Manual_Saberblocking(self) && self->client->ps.saberBlockingTime < level.time)
	{
		doFullRoutine = qfalse;
	}

	if (!Manual_Saberblocking(self) && self->client->ps.saberBlockingTime < level.time)
	{
		return;
	}
	...
	for (e = 0; e < numListedEntities; e++)
	{
		...
		{//not a normal projectile
			...
			{//active saber blade, treat differently.//allow the blocking of normal saber swings
				...
				{//attacking
					swingBlockQuad = InvertQuad(saberMoveData[self->client->ps.saberMove].startQuad);
				}
				else if (PM_SaberInStart(self->client->ps.saberMove) || PM_SaberInTransition(self->client->ps.saberMove))
				{//preparing to attack
					swingBlockQuad = InvertQuad(saberMoveData[self->client->ps.saberMove].endQuad);
				}
				else
				{//not attacking
					continue;
			...
			if ((self->NPC) && dist < ent->splashRadius)
			{
				...
				{//TD is close enough to hurt me, I'm on the ground and the thing is at rest or behind me and about to blow up, or I don't have force-push so force-jump!
					...
					{//jump out of the way
						...
						BG_AddFatigue(&self->client->ps, FORCE_LONGJUMP_POWER);
					}
				}
				else if (Manual_Saberblocking(self)
					...
					&& self->client->NPC_class != CLASS_ROCKETTROOPER)
				{
					if (!ent->owner || !OnSameTeam(self, ent->owner))
					{
						...
						BG_AddFatigue(&self->client->ps, FORCE_DEFLECT_PUSH);
					}
				}
			}
			continue;
...
void WP_SaberUpdate(gentity_t * self, usercmd_t * ucmd)
{
	...
	{	// It isn't, which means we can update its position as we will.
		...
		{//keep bbox in front of player, even when wide?
			...
			if (!forceBlock
				&& ((self->s.number && !Jedi_SaberBusy(self)
					&& !g_saberRealisticCombat->integer)
					|| (self->s.number == 0
						&& self->client->ps.saberBlocking == BLK_WIDE
						&& (Manual_Saberblocking(self) || self->client->ps.saberBlockingTime > level.time)))
				&& self->client->ps.weaponTime <= 0
				&& !G_InCinematicSaberAnim(self))
			{//full-size blocking for non-attacking player with Manual_Saberblocking on
				...
				{//not a lightsaber
					...
					{//and not glass
						...
						{//not a force-usable door
							if (Q_stricmp("func_static", ent->classname) != 0 || (!(ent->spawnflags & 1) && !(ent->spawnflags & 2)) || (ent->spawnflags & 32))
...
{//FIXME: pass in a target ent so we (an NPC) can push/pull just one targeted ent.
	...
	else
	{
		...
		{
			anim = BOTH_FORCEPUSH;
			if (self->client->ps.saber[0].type == SABER_UNSTABLE//saber kylo
		...
		for (int x = 0; x < ent_count; x++)
		{
			if (push_list[x]->client)
			{
				...
				{//Jedi don't get pushed, they resist as long as they aren't already attacking and are on the ground
					...
					if (!push_list[x]->s.number && Manual_Saberblocking(self))
					{
						resistChance = 1;
					}
			...
			{//not a fake push/pull
				...
				{//a thrown saber, just send it back
					if (push_list[x]->owner && push_list[x]->owner->client && push_list[x]->owner->client->ps.SaberActive() && push_list[x]->s.pos.trType == TR_LINEAR && push_list[x]->owner->client->ps.saberEntityState != SES_RETURNING)
					{//it's on and being controlled
						...
						{//certain chance of throwing it aside and turning it off?
							...
							if (self->s.number >= MAX_CLIENTS && !G_ControlledByPlayer(self))
							{
								G_ReflectMissileNPC(self, push_list[x], right);
							}
							else
							{
								if (Manual_Saberblocking(self) && self->client->ps.blockPoints > BLOCKPOINTS_FATIGUE)
								{
									G_ManualBlockMissile(self, push_list[x], right, FP_SABER_DEFENSE);
								}
								else
								{
									G_ReflectMissile(self, push_list[x], right, FP_SABER_DEFENSE);
								}
							}
					...
					&& (push_list[x]->s.pos.trType != TR_INTERPOLATE || push_list[x]->s.weapon != WP_THERMAL))//rolling and stationary thermal detonators are dealt with below
				{
					...
					else
					{
						...
						{//it's heading towards me
							...
							if (self->s.number >= MAX_CLIENTS && !G_ControlledByPlayer(self))
							{
								G_ReflectMissileNPC(self, push_list[x], forward);
							}
							else
							{
								if (Manual_Saberblocking(self) && self->client->ps.blockPoints > BLOCKPOINTS_FATIGUE)
...
{//if these are interruptable, should they also drain power at a constant rate
	if (self && self->client)
	{
		if (self->client->ps.forcePowersActive & (1 << FP_SPEED)
			|| self->client->ps.forcePowersActive & (1 << FP_PROTECT)
...
void ForceSpeedAnim(gentity_t * self)
{
	if (self->client->pers.cmd.rightmove > 0)
	{
		if (self->client->ps.saber[0].type == SABER_UNSTABLE//saber kylo
			...
			|| self->client->ps.saber[0].type == SABER_ANAKIN
			|| self->client->ps.saber[0].type == SABER_PALP
			|| self->client->ps.saber[0].type == SABER_DOOKU
			|| self->client->ps.saber[0].type == SABER_YODA)//saber yoda
		{
...
void ForceSpeedDash(gentity_t * self, int duration)
{
	vec3_t dir;
	...
	if (self->client->ps.forcePowersActive & (1 << FP_SPEED) && (!(IsPressingDashButton(self))))
	{//it's already turned on.  turn it off.
		WP_ForcePowerStop(self, FP_SPEED);
		return;
	}

	if (self->client->ps.forcePowersActive& (1 << FP_SPEED) &&	self->client->ps.groundEntityNum == ENTITYNUM_NONE)
	{
	...
	else if (self->client->ps.forcePowersActive& (1 << FP_SPEED) &&	self->client->ps.groundEntityNum == ENTITYNUM_NONE)
	{
		NPC_SetAnim(self, SETANIM_BOTH, BOTH_FORCEINAIR1, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
		WP_ForcePowerStop(self, FP_SPEED);
	}
...
void player_StopBurn(gentity_t* self)
{
	if (self && self->client)
	{
	...
	{//I have an enemy
		...
		{//grip them all!
			...
			for (e = 0; e < numListedEntities; e++)
			{
				...
				gi.trace(&tr, self->client->renderInfo.eyePoint, vec3_origin, vec3_origin, ent_org, self->s.number, MASK_SHOT, G2_NOCOLLIDE, 0);
	...
	{//okay, trace straight ahead and see what's there
		...
		if (tr.entityNum >= ENTITYNUM_WORLD || tr.fraction == 1.0 || tr.allsolid || tr.startsolid)
		{
			if (g_gripitems->integer)
			{
				...
				trace_t tr2;
				gi.trace(&tr2, self->client->renderInfo.eyePoint, vec3_origin, vec3_origin, traceend, self->s.number, MASK_OPAQUE | CONTENTS_SOLID | CONTENTS_BODY | CONTENTS_ITEM | CONTENTS_CORPSE | MASK_SHOT, (EG2_Collision)0, 0);
...
//rww - RAGDOLL_END
	...
	{//just a duration
		self->client->ps.forcePowerDebounce[FP_GRIP] = level.time + 250;
		self->client->ps.forcePowerDuration[FP_GRIP] = level.time + 3000;
...
void ForceLightningAnim(gentity_t * self)
{
	...
	int bobaholdAnim = BOTH_FLAMETHROWER;
	...
	if (self->client->ps.torsoAnim == startAnim)
	{
		if (!self->client->ps.torsoAnimTimer)
		{
			if (self->client->NPC_class == CLASS_BOBAFETT || self->client->NPC_class == CLASS_MANDO)
			{
				NPC_SetAnim(self, SETANIM_TORSO, bobaholdAnim, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
			}
			else
			{
...
void ForceLightning(gentity_t * self)
{
	...
	{//holding it
		G_SoundOnEnt(self, CHAN_BODY, "sound/weapons/force/lightning.mp3");
	}
...
void ForceLightningDamagemd(gentity_t * self, gentity_t * traceEnt, vec3_t dir, float dist, float dot, vec3_t impactPoint)
{
	...
	if (traceEnt && traceEnt->takedamage)
	{
		if (!traceEnt->client || traceEnt->client->playerTeam != self->client->playerTeam || self->enemy == traceEnt || traceEnt->enemy == self || traceEnt->client->playerTeam == TEAM_SOLO || self->client->playerTeam == TEAM_SOLO)
		{
			...
			if (traceEnt->client
				&& traceEnt->health > 0
				&& traceEnt->NPC
				&& (traceEnt->NPC->aiFlags & NPCAI_BOSS_MOVIEDUELS))
			{
				...
				{//saber can block lightning
					float chanceOfFizz = Q_flrand(0.0f, 1.0f);
					int npcSaberNum = 0;
					int npcBladeNum = 0;
					int parts;
					...
					if (chanceOfFizz > 0)
					{
						VectorMA(traceEnt->client->ps.saber[npcSaberNum].blade[npcBladeNum].muzzlePoint, traceEnt->client->ps.saber[npcSaberNum].blade[npcBladeNum].length * Q_flrand(0, 1), traceEnt->client->ps.saber[npcSaberNum].blade[npcBladeNum].muzzleDir, end);
						G_PlayEffect(G_EffectIndex("saber/saber_Lightninghit.efx"), end, fwd);
					}
					NPC_SetAnim(traceEnt, parts, BOTH_P1_S1_T_, AFLAG_PACE);
					traceEnt->client->ps.weaponTime = Q_irand(300, 600);
					if (traceEnt->client->ps.blockPoints < BLOCKPOINTS_HALF)
					{
						WP_ForcePowerDrain(traceEnt, FP_ABSORB, fpBlockCost);
					}
					traceEnt->client->ps.blockPoints--;
				}
				else
				{
					dmg = 1;
				...
				&& (traceEnt->client->ps.weapon == WP_NONE || traceEnt->client->ps.weapon == WP_MELEE))
			{
				if ((Manual_HandBlocking(traceEnt) || traceEnt->client->ps.forcePowerLevel[FP_ABSORB] > FORCE_LEVEL_1)
					&& traceEnt->client->ps.forcePower > 20
				...
				{
					blockedWithHand = qtrue;
					dmg = 0;
					NPC_SetAnim(traceEnt, SETANIM_TORSO, BOTH_FORCE_2HANDEDLIGHTNING_NEW, AFLAG_PACE);
					traceEnt->client->ps.weaponTime = Q_irand(300, 600);
					if (traceEnt->client->ps.blockPoints < BLOCKPOINTS_HALF)
					{
						WP_ForcePowerDrain(traceEnt, FP_ABSORB, fpBlockCost);
			...
			{
				if ((Manual_Saberblocking(traceEnt)
					|| traceEnt->client->ps.forcePowersActive & (1 << FP_ABSORB)
			...
			else if (traceEnt->s.weapon == WP_SABER && (Manual_Saberblocking(traceEnt)
				|| traceEnt->client->ps.forcePowersActive & (1 << FP_ABSORB)
				&& traceEnt->client->ps.forcePowerLevel[FP_ABSORB] > FORCE_LEVEL_2))
			{//saber can block lightning
				if (traceEnt->client->ps.forcePower > 20
					&& traceEnt->client->ps.SaberActive()
					&& !traceEnt->client->ps.saberInFlight//saber in hand
					&& InFOV(self->currentOrigin, traceEnt->currentOrigin, traceEnt->client->ps.viewangles, 20, 35) //I'm in front of them
					&& !PM_InKnockDown(&traceEnt->client->ps))
				{
					...
					if (Q_irand(0, traceEnt->client->ps.forcePowerLevel[FP_SABER_DEFENSE] * 3) > 0)
					{
						saberLightningBlocked = qtrue;
						float chanceOfFizz = Q_flrand(0.0f, 1.0f);
						int npcSaberNum = 0;
					...
					if ((traceEnt->client->ps.forcePowersActive & (1 << FP_ABSORB)) && traceEnt->client->ps.forcePowerLevel[FP_ABSORB] > FORCE_LEVEL_2)
			...
			{
				if ((Manual_HandBlocking(traceEnt)
					|| traceEnt->client->ps.forcePowersActive & (1 << FP_ABSORB))
					&& traceEnt->client->ps.forcePowerLevel[FP_ABSORB] > FORCE_LEVEL_2
					&& traceEnt->client->ps.forcePower > 20
					&& InFOV(self->currentOrigin, traceEnt->currentOrigin, traceEnt->client->ps.viewangles, 20, 35)
					...
					&& !PM_InSpecialJump(traceEnt->client->ps.torsoAnim))
				{
					...
					NPC_SetAnim(traceEnt, SETANIM_TORSO, BOTH_FORCE_2HANDEDLIGHTNING_NEW, AFLAG_PACE);
					traceEnt->client->ps.weaponTime = Q_irand(300, 600);
					WP_ForcePowerRegenerate(traceEnt, fpBlockCost);
					dmg = 0;
				}
				else
				{
			...
			if (traceEnt->client)
			{
				modPowerLevel = WP_AbsorbLightningConversion(traceEnt, traceEnt->client->ps.forcePowerLevel[FP_ABSORB], self, FP_LIGHTNING, self->client->ps.forcePowerLevel[FP_LIGHTNING], 1);
			}
			...
			if (dmg && (!saberLightningBlocked && !blockedWithHand))
			{
				G_Damage(traceEnt, self, self, dir, impactPoint, dmg, 0, MOD_FORCE_LIGHTNING);

				if ((traceEnt->NPC || traceEnt->s.eType == ET_PLAYER) && !(traceEnt->s.weapon == WP_EMPLACED_GUN))
				{
					...
					else
					{
						if (!PM_RunningAnim(traceEnt->client->ps.legsAnim) && traceEnt->client->ps.stats[STAT_HEALTH] > 1)
						{
							...
							{
								NPC_SetAnim(traceEnt, SETANIM_TORSO, BOTH_SONICPAIN_HOLD, AFLAG_PACE);
							}
							else
							{
								NPC_SetAnim(traceEnt, SETANIM_TORSO, BOTH_WIND, AFLAG_PACE);
							}
						}
					}
				}
			...
			if (traceEnt->client)
			{
				...
				else if (blockedWithHand)
				{
					...
					if (traceEnt->ghoul2.size())
					{
						...
						{
							G_PlayEffect(G_EffectIndex("force/confusion.efx"), traceEnt->playerModel, traceEnt->handRBolt, traceEnt->s.number, traceEnt->currentOrigin, 1000, qtrue);
						}
						if (traceEnt->handLBolt != -1)
						{
							G_PlayEffect(G_EffectIndex("force/confusion.efx"), traceEnt->playerModel, traceEnt->handLBolt, traceEnt->s.number, traceEnt->currentOrigin, 1000, qtrue);
						}
					}
				}
				else
				{
					traceEnt->client->ps.powerups[PW_SHOCKED] = level.time + 3000;
				}
			}
		}
	}
				...
				if ((traceEnt->NPC || traceEnt->s.eType == ET_PLAYER) && !(traceEnt->s.weapon == WP_EMPLACED_GUN))
				{
					...
					else
					{
						if ((traceEnt->NPC || traceEnt->s.eType == ET_PLAYER)
							&& !PM_RunningAnim(traceEnt->client->ps.legsAnim) && traceEnt->client->ps.stats[STAT_HEALTH] > 1)
						{
							if (traceEnt->client->ps.stats[STAT_HEALTH] < 70)
							{
								NPC_SetAnim(traceEnt, SETANIM_TORSO, BOTH_SONICPAIN_HOLD, AFLAG_PACE);
						...
						{
							if ((traceEnt->NPC || traceEnt->s.eType == ET_PLAYER)
								&& traceEnt->client->ps.stats[STAT_HEALTH] < 2)
							{
								vec3_t	defaultDir;
...
void ForceLightningDamagecin(gentity_t * self, gentity_t * traceEnt, vec3_t dir, float dist, float dot, vec3_t impactPoint)
{
	if (traceEnt->NPC && traceEnt->NPC->scriptFlags & SCF_NO_FORCE)
	{
		return;
		...
		{//an enemy or object
			...
			{//saber can block lightning
				if (traceEnt->client //a client
					&& traceEnt->client->ps.forcePowerLevel[FP_ABSORB] > FORCE_LEVEL_1
					&& !traceEnt->client->ps.saberInFlight//saber in hand
					&& (traceEnt->client->ps.saberMove == LS_READY || PM_SaberInParry(traceEnt->client->ps.saberMove)
						|| PM_SaberInReturn(traceEnt->client->ps.saberMove))//not attacking with saber
					&& InFOV(self->currentOrigin, traceEnt->currentOrigin, traceEnt->client->ps.viewangles, 20, 35) //I'm in front of them
					...
					&& (!traceEnt->s.number || (traceEnt->NPC && traceEnt->NPC->rank >= RANK_LT_COMM)))//the player or a tough jedi/reborn
				{
					...
					{//make them do a parry
						...
						if ((traceEnt->client->ps.saber[0].type == SABER_KENOBI))
						{
							NPC_SetAnim(traceEnt, SETANIM_TORSO, BOTH_KENOBI_LIGHTNINGBLOCK, AFLAG_PACE);
						}
						else
						{
							traceEnt->client->ps.saberBlocked = BLOCKED_TOP;
						}
						traceEnt->client->ps.weaponTime = Q_irand(100, 300);
					}
					else
					{//make them do a parry
...
void ForceShootLightning(gentity_t * self)
{
	...
	{//arc
		vec3_t	center, mins, maxs, dir, ent_org, size, v;
		float	radius = FORCE_LIGHTNING_RADIUS_WIDE, dot, dist;
		gentity_t* entityList[MAX_GENTITIES];
		int		e, numListedEntities, i;
		...
		for (e = 0; e < numListedEntities; e++)
		{
			...
			else if (com_outcast->integer == 10)//MOVIEDUELS version
			{
				if (!strcmp(s, "md_gd_jedi")
					|| !strcmp(s, "md_gd_sith")
					|| !strcmp(s, "md_gd_bonus")
					|| (in_camera || G_InCinematicLightningAnim(self)))//special case for this map!
				{// in cinematic
					ForceLightningDamagecin(self, traceEnt, dir, dist, dot, ent_org);
				}
				else
				{
					ForceLightningDamagemd(self, traceEnt, dir, dist, dot, ent_org);
				}
			}
			else
			{
	...
	{//trace-line
		...
		else if (com_outcast->integer == 10)//jko version
		{
			if (!strcmp(s, "md_gd_jedi")
				...
				|| (in_camera || G_InCinematicLightningAnim(self)))//special case for this map!
			{// in cinematic
				ForceLightningDamagecin(self, traceEnt, forward, 0, 0, tr.endpos);
			}
			else
			{
				ForceLightningDamagemd(self, traceEnt, forward, 0, 0, tr.endpos);
			}
		}
		else
		{
...
void ForceJump(gentity_t * self, usercmd_t * ucmd)
{
	...
	case FJ_FORWARD:
		if (((self->client->NPC_class == CLASS_BOBAFETT ||
			self->client->NPC_class == CLASS_MANDO || self->client->NPC_class == CLASS_ROCKETTROOPER) && self->client->ps.forceJumpCharge > 300)
...
void ForceDestruction(gentity_t * self)
{
	...
	soundIndex = G_SoundIndex("sound/weapons/force/destruction.mp3");
...
int WP_AbsorbLightningConversion(gentity_t * attacked, int atdAbsLevel, gentity_t * attacker, int atPower, int atPowerLevel, int atForceSpent)
{
	...
	G_SoundOnEnt(attacked, CHAN_ITEM, "sound/weapons/force/lightninghit2.mp3");
...
void WP_ForcePowerDrain(gentity_t * self, forcePowers_t forcePower, int overrideAmt)
{
	...
	if (!drain)
	{
		if (self->client->ps.forcePowerLevel[FP_SPEED] == FORCE_LEVEL_1 &&
			self->client->ps.forcePowersActive & (1 << FP_SPEED))
		{
			drain = forcePowerNeededlevel1[forcePower];
		}
		else if (self->client->ps.forcePowerLevel[FP_SPEED] == FORCE_LEVEL_2 &&
			self->client->ps.forcePowersActive & (1 << FP_SPEED))
		{
			drain = forcePowerNeededlevel2[forcePower];
		}
...
void WP_ForcePowerStart(gentity_t * self, forcePowers_t forcePower, int overrideAmt)
{
	...
	case FP_SPEED:
		...
		if (self->client->ps.forcePowerLevel[FP_SPEED] == FORCE_LEVEL_1)
		{
			...
			{
				duration = ceil(FORCE_SPEED_DURATION_FORCE_LEVEL_1 * forceDashValue[self->client->ps.forcePowerLevel[FP_SPEED]]);
			}
			else
			{
				duration = ceil(FORCE_SPEED_DURATION_FORCE_LEVEL_1 * forceSpeedValue[self->client->ps.forcePowerLevel[FP_SPEED]]);
			}
		}
		else if (self->client->ps.forcePowerLevel[FP_SPEED] == FORCE_LEVEL_2)
		{
			...
			{
				duration = ceil(FORCE_SPEED_DURATION_FORCE_LEVEL_2 * forceSpeedValue[self->client->ps.forcePowerLevel[FP_SPEED]]);
			}
		}
		else if (self->client->ps.forcePowerLevel[FP_SPEED] == FORCE_LEVEL_3)
		{
			...
			{
				duration = ceil(FORCE_SPEED_DURATION_FORCE_LEVEL_3 * forceSpeedValue[self->client->ps.forcePowerLevel[FP_SPEED]]);
			}
		}
		self->client->ps.forcePowersActive |= (1 << forcePower);
...
//rww - RAGDOLL_END
	...
	case FP_LIGHTNING:
		...
		if (self->client->ps.torsoAnim == BOTH_FORCELIGHTNING_HOLD || self->client->ps.torsoAnim == BOTH_FLAMETHROWER || self->client->ps.torsoAnim == BOTH_FORCELIGHTNING_START)
		{
			NPC_SetAnim(self, SETANIM_TORSO, BOTH_FORCELIGHTNING_RELEASE, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
		}
		else if (self->client->ps.torsoAnim == BOTH_FORCE_2HANDEDLIGHTNING_HOLD || self->client->ps.torsoAnim == BOTH_FORCE_2HANDEDLIGHTNING_START)
		{
...
static void WP_ForcePowerRun(gentity_t * self, forcePowers_t forcePower, usercmd_t * cmd)
{
	...
	case FP_GRIP:
			...
			{//invalid or freed ent
				...
				{//either invalid ent, or dead ent
					...
					{//flying creature broke free
						...
						&& (gripEnt->client->NPC_class == CLASS_BOBAFETT || gripEnt->client->NPC_class == CLASS_MANDO || gripEnt->client->NPC_class == CLASS_ROCKETTROOPER)
					...
					else
					{
						...
						if (gripEnt->client && gripEnt->health > 0)
						{
							...
							{//still on ground, only set anim on torso
								if (gripEnt->health > 50)
								{
									NPC_SetAnim(gripEnt, SETANIM_TORSO, BOTH_CHOKE4, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
								}
								else
								{
							...
							{//in air, set on whole body
								if (gripEnt->health > 50)
								{
									NPC_SetAnim(gripEnt, SETANIM_BOTH, BOTH_CHOKE4, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
								}
								else
								{
		...
		break;
	case FP_LIGHTNING:
		if (!ForceLightningCheckattack(self))
		...
		if (!WP_ForcePowerAvailable(self, forcePower, 0)
			|| self->client->ps.forcePowerDuration[FP_LIGHTNING] < level.time
		...
		{//holding someone
			...
			else
			{
				...
				{//flying creature broke free
					...
					&& (drainEnt->client->NPC_class == CLASS_BOBAFETT || drainEnt->client->NPC_class == CLASS_MANDO || drainEnt->client->NPC_class == CLASS_ROCKETTROOPER)
...
void WP_ForcePowersUpdate(gentity_t * self, usercmd_t * ucmd)
{
	...
	{//when not using the force, regenerate at 10 points per second
		if (self->client->ps.forcePowerRegenDebounceTime < level.time)
		{
			...
			self->client->ps.forcePowerRegenDebounceTime = level.time + 200;
			...
			else if (ucmd->buttons & BUTTON_BLOCK)
			{//regen half as fast
...
void WP_InitForcePowers(gentity_t * ent)
{
	...
	{//player
		...
		{
			ent->client->ps.forcePowersKnown = (1 << FP_HEAL) | (1 << FP_LEVITATION) | (1 << FP_SPEED) | (1 << FP_PUSH) | (1 << FP_PULL) | (1 << FP_TELEPATHY) | (1 << FP_GRIP) | (1 << FP_LIGHTNING) | (1 << FP_SABERTHROW) | (1 << FP_SABER_DEFENSE) | (1 << FP_SABER_OFFENSE) | (1 << FP_RAGE) | (1 << FP_DRAIN) | (1 << FP_PROTECT) | (1 << FP_ABSORB) | (1 << FP_SEE) | (1 << FP_STASIS) | (1 << FP_DESTRUCTION);
...
qboolean WP_DoingForcedAnimationForForcePowers(gentity_t * self)
{
	if (!self->client)
	{
		return qfalse;
...
{//place self into a stunned state.
	...
	else if (!PM_SaberInBrokenParry(self->client->ps.saberMove)	&& !PM_InKnockDown(&self->client->ps))
	{
		...
		{// Do taunt/anger...
			...
			{
			case 3:
				G_AddVoiceEvent(self, Q_irand(EV_JCHASE1, EV_JCHASE3), Q_irand(2000, 4000));
				break;
			case 2:
				G_AddVoiceEvent(self, Q_irand(EV_COMBAT1, EV_COMBAT3), Q_irand(2000, 4000));
				break;
			case 1:
				G_AddVoiceEvent(self, Q_irand(EV_ANGER1, EV_ANGER1), Q_irand(2000, 4000));
				break;
			default:
				G_AddVoiceEvent(self, Q_irand(EV_TAUNT1, EV_TAUNT3), Q_irand(2000, 4000));
				break;
			}
		}
		...
		{// Do taunt/anger...
			...
			case 1:
				...
				break;
			default:
				G_AddVoiceEvent(self, Q_irand(EV_PUSHED1, EV_PUSHED3), Q_irand(2000, 4000));
				break;
			}
		}
	}
