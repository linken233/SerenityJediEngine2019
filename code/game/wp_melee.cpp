...
//---------------------------------------------------------
{
	...
	qboolean	isMandoPlayer = (!ent->s.number && ent->client->NPC_class == CLASS_MANDO && ent->client->ps.forcePowerDuration[missileStates[BOBA_MISSILE_VIBROBLADE].dummyForcePower]) ? qtrue : qfalse;
	...
	if ( tr_ent && tr_ent->takedamage )
	{
		...
		else if(ent->client->ps.torsoAnim == BOTH_WOOKIE_SLAP)
		{ //do a tad bit more damage IF WOOKIE CLASS // SERENITY
			damage = MELEE_SWING_WOOKIE_DAMAGE;
		}
		else if (ent->client->ps.torsoAnim == BOTH_MELEEUP)
		{ //do a tad bit more damage IF WOOKIE CLASS // SERENITY
