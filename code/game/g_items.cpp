...
void ClearRegisteredItems( void )
{
	...
	RegisterItem( FindItemForInventory( INV_JETPACK));
...
void ItemUse_UseCloak(gentity_t *ent)
{
	...
	if ((ent->client->ps.powerups[PW_CLOAKED] && ent->client->ps.cloakFuel < 0))
	{ //using cloak, drain force
		player_Decloak(ent);
	}

	if (ent->client->ps.forcePower <= 30)
	...
	{//cloak
		...
		{ //using cloak, drain force
			Self_CheckCloak(ent);
			WP_ForcePowerDrain(ent, FP_SABER_DEFENSE, 25);
		}		
	}	
