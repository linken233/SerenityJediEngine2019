...
void G_Give( gentity_t *ent, const char *name, const char *args, int argc )
{
	...
	if (give_all || Q_stricmp(name, "inventory") == 0)
	{
		...
		{
			ent->client->ps.inventory[INV_JETPACK] = 1;
		}
...
void G_SetTauntAnim( gentity_t *ent, int taunt )
{
	...
	if (!ent->client->ps.weaponTime	&& ent->client->ps.saberLockTime < level.time)
	{
		int anim = -1;
		switch ( taunt )
		{
		case TAUNT_TAUNT:
			...
			else if (ent->client->NPC_class == CLASS_VADER && ent->client->ps.weapon == WP_SABER && ent->client->ps.SaberActive())
			{
		...
		if ( anim != -1 )
		{
			if ( ent->client->ps.groundEntityNum != ENTITYNUM_NONE )
			{
				...
				if ( anim != BOTH_ENGAGETAUNT && anim != BOTH_VADERTAUNT)
				{
					parts = SETANIM_BOTH;
