...
static void PM_Friction(void)
{
	...
	else if (Flying != FLY_NORMAL)
	{
		...
		&& pm->gent
		&& pm->gent->client
		&& (pm->gent->client->NPC_class == CLASS_BOBAFETT || pm->gent->client->NPC_class == CLASS_MANDO || pm->gent->client->NPC_class == CLASS_ROCKETTROOPER) && pm->gent->client->moveType == MT_FLYSWIM)
	{//player as Boba
...
static qboolean PM_CheckJump(void)
{
	...
	else if (pm->waterlevel < 3)
	{
		...
		{//can't do this in zero-G
			...
			{//in the middle of a force long-jump
				...
				&& !(pm->ps->pm_flags & PMF_JUMP_HELD)
				&& (level.time - pm->ps->forcePowerDebounce[FP_SPEED]) <= 500//have to have just started the force speed within the last half second
				&& pm->gent)
	...
	{//in low grav, you push in the dir you're facing as long as there is something behind you to shove off of
		...
		if (trace.fraction < 1.0f)
		{
			if (pm->ps->weapon == WP_SABER)
			{
				...
				PM_SetAnim(pm, SETANIM_LEGS, BOTH_FORCEJUMP2, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD | SETANIM_FLAG_RESTART);
			}
			else
			{
				VectorMA(pm->ps->velocity, JUMP_VELOCITY / 2, forward, pm->ps->velocity);
		...
		&& ((pm->ps->clientNum && !PM_ControlledByPlayer()) || ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) && cg.renderingThirdPerson && !cg.zoomMode && !(pm->gent->flags & FL_LOCK_PLAYER_WEAPONS))))// yes this locked weapons check also includes force powers, if we need a separate check later I'll make one
	{
		...
		{//on the ground
			...
			if ((pm->ps->weapon == WP_SABER && G_TryingCartwheel(pm->gent, &pm->cmd) && (pm->cmd.buttons & BUTTON_ATTACK))//using saber and holding focus + attack
				&& (((pm->ps->clientNum >= MAX_CLIENTS && !PM_ControlledByPlayer()) && pm->cmd.upmove > 0 && pm->ps->velocity[2] >= 0)//jumping NPC, going up already
					|| ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) && G_TryingCartwheel(pm->gent, &pm->cmd)))//focus-holding player
	...
	if (pm->ps->groundEntityNum == ENTITYNUM_NONE)
	{
		if (!(pm->waterlevel == 1 && pm->cmd.upmove > 0 && pm->ps->forcePowerLevel[FP_LEVITATION] == FORCE_LEVEL_3 && pm->ps->forcePower >= 5))
		{
			return qfalse;
		}
		else
...
static void PM_GrappleMove(void)
{
	...
	{
		if (vlen > pm->ps->userFloat1 && !(PM_InSpecialJump(pm->ps->torsoAnim) || (pm->ps->pm_flags & PMF_STUCK_TO_WALL)))
		{
...
{
	NPC_SetAnim(ent, SETANIM_BOTH, anim, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_RESTART | SETANIM_FLAG_HOLD | SETANIM_FLAG_HOLDLESS);
...
static qboolean PM_BFIIMove(void)
{
	vec3_t dir;
	int		anim = -1;
	...
	if (pm->ps->groundEntityNum != ENTITYNUM_NONE)
	{
		...
		pm->ps->velocity[0] = pm->ps->velocity[0] * 6;
		pm->ps->velocity[1] = pm->ps->velocity[1] * 6;
	}
	else
	{
...
static qboolean PM_TryRoll(void)
{
	...
	vec3_t fwd, right, traceto, mins = { pm->mins[0], pm->mins[1], pm->mins[2] + STEPSIZE },
		maxs = { pm->maxs[0], pm->maxs[1], (float)pm->gent->client->crouchheight }, fwdAngles = { 0, pm->ps->viewangles[YAW], 0 };
	trace_t	trace;
	int		anim = -1;
...
static void PM_CrashLand(void)
{
	...
	else
	{
		...
		{//have to do death on impact if we are falling to our death, FIXME: should we avoid any additional damage this func?
			...
			&& pm->gent->client
			&& (pm->gent->client->NPC_class == CLASS_BOBAFETT || pm->gent->client->NPC_class == CLASS_MANDO || pm->gent->client->NPC_class == CLASS_ROCKETTROOPER))
		{
			if (JET_Flying(pm->gent))
			{
										...
										if (!pm->ps->forceJumpZStart || pm->ps->forceJumpZStart > pm->ps->origin[2])
										{
											...
											else if (!trace.allsolid && !trace.startsolid && (pm->ps->origin[2] - trace.endpos[2]) >= 128)//>=128 so we don't die on steps!
											{
												...
												{//didn't hit, we're probably going to die
													...
													&& pm->ps->weapon != WP_SABER
													&& (!pm->gent || !pm->gent->client || (pm->gent->client->NPC_class != CLASS_BOBAFETT && pm->gent->client->NPC_class != CLASS_MANDO && pm->gent->client->NPC_class != CLASS_REBORN && pm->gent->client->NPC_class != CLASS_ROCKETTROOPER)))
							...
							else if (!(pm->ps->forcePowersActive & (1 << FP_LEVITATION)))
							{
								if (pm->cmd.forwardmove >= 0)
								{
									if (!PM_InDeathAnim())
									{
										if (pm->ps->weapon == WP_SABER && pm->ps->SaberActive())//saber out
										{
											PM_SetAnim(pm, SETANIM_LEGS, BOTH_JUMP2, SETANIM_FLAG_OVERRIDE, 100);	// Only blend over 100ms
										}
										else
										{
...
qboolean PM_GettingUpFromKnockDown(float standheight, float crouchheight)
{
	...
	{//in a knockdown
		...
		if (!pm->ps->legsAnimTimer || (pm->ps->legsAnimTimer <= minTimeLeft
		...
		{//done with the knockdown - FIXME: somehow this is allowing an *instant* getup...???
			...
			else
			{
				...
				{//stand up
					...
					case BOTH_SLAPDOWNLEFT:
						if ((pm->ps->clientNum
							&& pm->ps->forcePowerLevel[FP_SABER_DEFENSE] > FORCE_LEVEL_0)
							|| ((pm->ps->clientNum < MAX_CLIENTS
								&& !(pm->ps->userInt3 & (1 << FLAG_FATIGUED))  //player isn't fatigued.
								|| PM_ControlledByPlayer())
								&& pm->cmd.upmove > 0
								&& pm->ps->forcePowerLevel[FP_LEVITATION] > FORCE_LEVEL_0))//FORCE_LEVEL_1) )FORCE_LEVEL_1) )
						{
...
//melee kungfu==========================
qboolean PM_MeleeblockHoldAnim(int anim)
{
	switch (anim)
	{
...
qboolean PM_MeleeblockAnim(int anim)
{
	switch (anim)
	{
...
void PM_JetPackAnim(void)
{
	...
	if (pm->cmd.rightmove > 0)
	{
		...
		{
			PM_SetAnim(pm, SETANIM_TORSO, BOTH_FLAMETHROWER, AFLAG_PACE);
		}
		else
		{
			anim = BOTH_FORCEJUMPRIGHT1;
...
static void PM_Footsteps(void)
{
	...
	int			setAnimOverride = AFLAG_PACE;
				...
				if (!PM_AdjustStandAnimForSlope())
				{
					if (pm->ps->legsAnim != BOTH_LOSE_SABER || !pm->ps->legsAnimTimer)
					{
						...
						case SS_FAST:
							...
							break;
						case SS_TAVION:
							PM_SetAnim(pm, SETANIM_LEGS, BOTH_SABERPULL_ALT, SETANIM_FLAG_NORMAL);
							break;
						case SS_MEDIUM:
							PM_SetAnim(pm, SETANIM_LEGS, BOTH_SABERPULL, SETANIM_FLAG_NORMAL);
							break;
				...
				if (!PM_AdjustStandAnimForSlope())
				{
					...
					else
					{
						...
						case SS_FAST:
							...
							{
								legsAnim = BOTH_SABERYODA_STANCE;
							}
							else if (pm->ps->saber[0].type == SABER_BACKHAND)//saber backhand
							{
								legsAnim = BOTH_SABERBACKHAND_STANCE;
							}
							else if (pm->ps->saber[0].type == SABER_ASBACKHAND)//saber backhand
							{
								legsAnim = BOTH_SABERBACKHAND_STANCE;
							}
							else if (pm->ps->saber[0].type == SABER_UNSTABLE)//saber kylo
							{
								legsAnim = BOTH_SABERSTANCE_STANCE_ALT;
							}
						...
						case SS_DUAL:
							...
							else if (pm->ps->saber[0].type == SABER_GRIE)//saber
							{
								legsAnim = BOTH_SABERDUAL_STANCE_GRIEVOUS;
							}
							else if (pm->ps->saber[0].type == SABER_GRIE4)//saber
							{
								legsAnim = BOTH_SABERDUAL_STANCE_GRIEVOUS;
							}
							else
						...
						case SS_MEDIUM:
							...
							{
								legsAnim = BOTH_SABEREADY_STANCE;
							}
							else if (pm->ps->saber[0].type == SABER_WINDU)//saber kylo
							{
								legsAnim = BOTH_SABEREADY_STANCE;
							}
							else
						...
						default:
							...
							{
								legsAnim = BOTH_SABERSTANCE_STANCE;
							}
							break;
						}
					}
...
#define OVERRIDE_ROLL_CHECK g_entities[pm->ps->clientNum].npc_roll_start
	...
	if (pm->ps->pm_flags & PMF_DUCKED || OVERRIDE_ROLL_CHECK)
	{
			...
			&& (!PM_InRollIgnoreTimer(pm->ps) || (!pm->ps->legsAnimTimer && pm->cmd.upmove < 0)))
		{
			...
			if (PM_RunningAnim(pm->ps->legsAnim)
				...
				|| pm->cmd.buttons & BUTTON_USE || OVERRIDE_ROLL_CHECK)
			{//roll!
				rolled = PM_TryRoll();
			}
			if (!rolled)
	...
	{//Moving backwards
		...
		{//running backwards
			...
			{
				if (pm->ps->forcePowersActive & (1 << FP_SPEED) && pm->cmd.buttons & BUTTON_DASH)
				{
					if (pm->cmd.rightmove > 0)
					{
						if (pm->ps->saber[0].type == SABER_UNSTABLE//saber kylo
							|| pm->ps->saber[0].type == SABER_STAFF_UNSTABLE
							...
							|| pm->ps->saber[0].type == SABER_DOOKU
							|| pm->ps->saber[0].type == SABER_YODA)//saber yoda
						{
							PM_SetAnim(pm, SETANIM_BOTH, BOTH_DASH_R, setAnimOverride);
						}
						else
						{
							PM_SetAnim(pm, SETANIM_BOTH, BOTH_HOP_R, setAnimOverride);
						}
					}
					else if (pm->cmd.rightmove < 0)
					{
						if (pm->ps->saber[0].type == SABER_UNSTABLE//saber kylo
							...
							|| pm->ps->saber[0].type == SABER_YODA)//saber yoda
						{
							PM_SetAnim(pm, SETANIM_BOTH, BOTH_DASH_L, setAnimOverride);
						}
						else
						{
							PM_SetAnim(pm, SETANIM_BOTH, BOTH_HOP_L, setAnimOverride);
						}
					}
					else if (pm->cmd.forwardmove < 0)
					{
						if (pm->ps->saber[0].type == SABER_UNSTABLE//saber kylo
							...
							|| pm->ps->saber[0].type == SABER_YODA)//saber yoda
						{
							PM_SetAnim(pm, SETANIM_BOTH, BOTH_DASH_B, setAnimOverride);
						}
						else
						{
							PM_SetAnim(pm, SETANIM_BOTH, BOTH_HOP_B, setAnimOverride);
						}
					}
					else
					{
						if (pm->ps->saber[0].type == SABER_UNSTABLE//saber kylo
							...
							|| pm->ps->saber[0].type == SABER_YODA)//saber yoda
						{
							PM_SetAnim(pm, SETANIM_BOTH, BOTH_DASH_F, setAnimOverride);
						}
						else
						{
							PM_SetAnim(pm, SETANIM_BOTH, BOTH_HOP_F, setAnimOverride);
						}
					}
				}
				else
		...
		{//walking backwards
			...
			if (pm->ps->weapon == WP_SABER && pm->ps->SaberActive())
			{
				if (pm->ps->saberAnimLevel == SS_STAFF)
				{
					if ((pm->ps->SaberStaff() && (!pm->ps->saber[0].singleBladeStyle || pm->ps->saber[0].blade[1].active)))//saber staff with more than first blade active
					{
						if (pm->ps->forcePowersActive & (1 << FP_SPEED) && pm->cmd.buttons & BUTTON_DASH)
						{
							if (pm->cmd.rightmove > 0)
		...
		else
		{
			...
			{//running
				...
				if (pm->ps->weapon == WP_SABER && pm->ps->SaberActive())
				{
					if (pm->ps->saberAnimLevel == SS_STAFF)
					{
						if ((pm->ps->SaberStaff() && (!pm->ps->saber[0].singleBladeStyle || pm->ps->saber[0].blade[1].active)))//saber staff with more than first blade active
						{
							...
							else if (pm->ps->saber[0].type == SABER_STAFF_UNSTABLE)//saber kylo
							{
								PM_SetAnim(pm, SETANIM_BOTH, BOTH_SPRINT, setAnimFlags);
							}
					...
					else if (pm->ps->saberAnimLevel == SS_DUAL)
					{
						if (pm->ps->dualSabers && !pm->ps->saber[1].Active())
						{
							if (pm->cmd.buttons & BUTTON_BLOCK)
							{
								if (pm->ps->saber[0].type == SABER_BACKHAND)//saber backhand
								{
									PM_SetAnim(pm, SETANIM_BOTH, BOTH_RUN_STAFF, setAnimFlags);
								}
								else if (pm->ps->saber[0].type == SABER_ASBACKHAND)//saber backhand
						...
						|| pm->ps->saberAnimLevel == SS_TAVION)
					{
						...
						else
						{
							...
							else if (pm->ps->stats[STAT_HEALTH] <= 70 && pm->ps->stats[STAT_HEALTH] >= 40)
							{
								PM_SetAnim(pm, SETANIM_BOTH, BOTH_RUN9, setAnimFlags);
							}
							else if (pm->ps->stats[STAT_HEALTH] <= 40)
				...
				else if (pm->ps->weapon == WP_BLASTER_PISTOL)
				{
					...
					{
						if (pm->gent->client->NPC_class == CLASS_REBORN && pm->ps->weapon == WP_BLASTER_PISTOL)//they do this themselves
				...
				else
				{
					...
					/*else if (pm->gent && pm->gent->client && pm->gent->client->NPC_class == CLASS_DESANN)
					{
						PM_SetAnim(pm, SETANIM_BOTH, BOTH_VADERRUN1, setAnimFlags);
					}*/
					...
					else if ((pm->gent && pm->gent->client && pm->gent->client->ps.forcePowersActive & (1 << FP_SPEED) && pm->gent->client->ps.forcePowerLevel[FP_SPEED] > FORCE_LEVEL_1))
					{//in force speed
					...
					else if (pm->gent && pm->gent->client && pm->ps->weapon == WP_MELEE)
					{
						if (pm->ps->stats[STAT_HEALTH] <= 70 && pm->ps->stats[STAT_HEALTH] >= 40)
						{
							PM_SetAnim(pm, SETANIM_LEGS, BOTH_RUN7, setAnimFlags);
					...
					else if (pm->ps->stats[STAT_HEALTH] <= 40 && pm->ps->weapon == WP_SABER)
					{
						if ((pm->ps->forcePowersActive & (1 << FP_SPEED) && pm->ps->forcePowerLevel[FP_SPEED] > FORCE_LEVEL_1) || pm->ps->weapon == WP_MELEE)
						{
							if (pm->ps->forcePowersActive & (1 << FP_SPEED) && pm->ps->forcePowerLevel[FP_SPEED] < FORCE_LEVEL_2)
							{
								if (pm->cmd.rightmove > 0)
								{
									PM_SetAnim(pm, SETANIM_BOTH, BOTH_HOP_R, setAnimOverride);
					...
					else if (pm->ps->stats[STAT_HEALTH] <= 70 && pm->ps->stats[STAT_HEALTH] >= 40 && pm->ps->weapon == WP_SABER)
					{
						if ((pm->ps->forcePowersActive & (1 << FP_SPEED) && pm->ps->forcePowerLevel[FP_SPEED] > FORCE_LEVEL_1) || pm->ps->weapon == WP_MELEE)
						{
							if (pm->ps->forcePowersActive & (1 << FP_SPEED) && pm->cmd.buttons & BUTTON_DASH)
						...
						else
						{
							...
							{
								PM_SetAnim(pm, SETANIM_LEGS, BOTH_RUN9, setAnimFlags);
							}
						}
					}
					else if (pm->ps->stats[STAT_HEALTH] <= 70 && pm->ps->stats[STAT_HEALTH] >= 40)
					...
					{
						if (pm->cmd.buttons & BUTTON_BLOCK && pm->ps->weapon == WP_SABER)
						{
							if (pm->ps->saber[0].type == SABER_BACKHAND)//saber backhand
							{
								PM_SetAnim(pm, SETANIM_BOTH, BOTH_RUN_STAFF, setAnimFlags);
			...
			{//walking forward
				...
				if (pm->ps->weapon == WP_SABER && pm->ps->SaberActive())
				{
					...
					else if (pm->ps->saberAnimLevel == SS_DESANN)
					{
						if (pm->cmd.buttons & BUTTON_BLOCK)
						{
							PM_SetAnim(pm, SETANIM_LEGS, BOTH_WALK2, setAnimFlags);
		...
		{//going to switch away from saber
			...
			if (!G_IsRidingVehicle(pm->gent))
			{
				if (!g_noIgniteTwirl->integer)
				{
					...
					{
						//just pull it in if walking like in mp
					}
				}
				else
				{
	...
	{//turn on the lightsaber
		...
		{//actually did switch weapons, play anim
			if (!G_IsRidingVehicle(pm->gent))
			{
				if (!g_noIgniteTwirl->integer)
				{
					if (PM_RunningAnim(pm->ps->legsAnim) || PM_WalkingAnim(pm->ps->legsAnim)
						|| (pm->ps->groundEntityNum == ENTITYNUM_NONE) || in_camera)
					{
					...
					else
					{
						...
						else if (pm->ps->saber[0].type == SABER_SFX)
						{
							switch (pm->gent->client->ps.saberAnimLevel)
							{
							case SS_FAST:
								PM_SetAnim(pm, SETANIM_TORSO, BOTH_SHOWOFF_FAST, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
								break;
							case SS_TAVION:
								PM_SetAnim(pm, SETANIM_TORSO, BOTH_VICTORY_MEDIUM, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
								break;
							case SS_MEDIUM:
								PM_SetAnim(pm, SETANIM_TORSO, BOTH_STAND1TO2, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
								break;
							case SS_STRONG:
								PM_SetAnim(pm, SETANIM_TORSO, BOTH_SHOWOFF_MEDIUM, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
								break;
							case SS_DESANN:
								PM_SetAnim(pm, SETANIM_TORSO, BOTH_SHOWOFF_STRONG, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
								break;
							case SS_DUAL:
								PM_SetAnim(pm, SETANIM_TORSO, BOTH_SHOWOFF_DUAL, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
								break;
							case SS_STAFF:
								PM_SetAnim(pm, SETANIM_TORSO, BOTH_SHOWOFF_STAFF, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
								break;
							}
						}
						else
...
int PM_MediumStanceForSingleStance(int anim)
{
	switch (anim)
	{
	case BOTH_P1_S1_T_:
...
int PM_ReadyPoseForBlockanimation(void)
{
	...
	if (pm->cmd.forwardmove > 0)
	{// Use a directional set of animations for blocking				
		if (pm->cmd.rightmove < 0)
		{//lower left block
			anim = BOTH_P1_S1_BL;
		}
		...
		{
			anim = BOTH_P1_S1_T_; // use top as default
		}
		if (pm->ps->saberAnimLevel == SS_DUAL)
		{
		...
		{
			return PM_MediumStanceForSingleStance(anim);
		}
	}
...
{
	int anim = BOTH_SABERSTANCE_STANCE;
	...
	if ((pm->cmd.buttons & BUTTON_BLOCK) && (pm->cmd.buttons & BUTTON_ATTACK))
	{
		return anim;
	}
		...
		pm->cmd.rightmove < 0)
	{
		if (pm->ps->saberAnimLevel == SS_DUAL)
		{
			return BOTH_RUNBACK2;
		}
		else if (pm->ps->saberAnimLevel == SS_STAFF)
		{
			if ((pm->ps->SaberStaff() && (!pm->ps->saber[0].singleBladeStyle || pm->ps->saber[0].blade[1].active)))//saber staff with more than first blade active
			{
				return BOTH_RUNBACK_STAFF;
			}
			else
			{
				return BOTH_RUNBACK2;
	...
	case SS_DUAL:
		if ((pm->cmd.buttons & BUTTON_BLOCK) && !(pm->cmd.buttons& BUTTON_ATTACK))
		{
			...
			else if (pm->ps->saber[0].type == SABER_GRIE)//saber backhand
			{
		...
		else
		{
			...
			{
				if (pm->ps->saber[0].type == SABER_OBIWAN)//saber kylo
				{
					anim = BOTH_SABEROBI_STANCE;
				}
				else if (pm->ps->saber[0].type == SABER_REY)//saber kylo
				{
	...
	case SS_STAFF:
		...
		else if (pm->ps->blockPoints < BLOCKPOINTS_FULL)
		{
			if (pm->ps->saber[0].type == SABER_OBIWAN)//saber kylo
			{
				anim = BOTH_SABEROBI_STANCE;
		...
		else
		{
			...
			{//simple saber attack
				...
				{//simple saber attack
					if (pm->ps->saber[0].type == SABER_STAFF_MAUL)//saber MAUL STAFF
					{
						anim = BOTH_SABERSTAFF_STANCE_ALT;
					}
					else if (pm->ps->saber[0].type == SABER_ELECTROSTAFF)//saber MAUL STAFF
					{
						anim = BOTH_SABERSTAFF_STANCE_ALT;
					}
					else
	...
	case SS_FAST:
		...
		{//simple saber attack
			...
			{
				if (pm->ps->saber[0].type == SABER_DOOKU)//saber backhand
				{
					anim = BOTH_SABERSTANCE_STANCE_ALT;
				}
				else if (pm->ps->saber[0].type == SABER_UNSTABLE)//saber kylo
	...
	default:
		...
		else if (pm->ps->saber[0].type == SABER_DOOKU)//saber backhand
		{
			anim = BOTH_SABERSTANCE_STANCE_ALT;
		}
		else if (pm->ps->saber[0].type == SABER_UNSTABLE)//saber kylo
...
int PM_ReadyPoseForSaberAnimLevelNPC(void)
{
	...
	case SS_STAFF:
		...
		else if (pm->ps->saber[0].type == SABER_STAFF_MAUL)//saber MAUL STAFF
		{
			anim = BOTH_SABERSTAFF_STANCE_ALT;
		}
		else if (pm->ps->saber[0].type == SABER_ELECTROSTAFF)//saber MAUL STAFF
...
void PM_SetSaberMove(saberMoveName_t newMove)
{
	...
	else if (newMove == LS_DRAW || newMove == LS_DRAW2 || newMove == LS_DRAW3)
	{
		...
		{
			PM_SetAnim(pm, SETANIM_TORSO, BOTH_DOOKU_SMALLDRAW, AFLAG_PACE);
		}
		...
		{
			PM_SetAnim(pm, SETANIM_TORSO, BOTH_S1_S7, AFLAG_PACE);
		}
		else if (pm->ps->saber[0].stylesLearned == (1 << SS_STAFF))
		{
			if (pm->ps->saber[0].type == SABER_BACKHAND)
			{
				PM_SetAnim(pm, SETANIM_TORSO, BOTH_SABER_BACKHAND_IGNITION, AFLAG_PACE);
			}
			else if (pm->ps->saber[0].type == SABER_ASBACKHAND)//saber backhand
			{
				PM_SetAnim(pm, SETANIM_TORSO, BOTH_SABER_BACKHAND_IGNITION, AFLAG_PACE);
			...
			&& !(pm->ps->saber[1].stylesForbidden & (1 << SS_DUAL)))
		{
			if (pm->ps->saber[0].type == SABER_GRIE || pm->ps->saber[0].type == SABER_GRIE4)
			{
				PM_SetAnim(pm, SETANIM_TORSO, BOTH_GRIEVOUS_SABERON, AFLAG_PACE);
			}
			else
			{
				PM_SetAnim(pm, SETANIM_TORSO, BOTH_S1_S6, AFLAG_PACE);
			}
		}
	...
	else if (newMove == LS_PUTAWAY)
	{
			...
			&& pm->ps->saber[1].Active())
		{
			if (pm->ps->saber[0].type == SABER_GRIE || pm->ps->saber[0].type == SABER_GRIE4)
			{
				PM_SetAnim(pm, SETANIM_BOTH, BOTH_STAND2TO1, AFLAG_PACE);
			}
			else
			{
				PM_SetAnim(pm, SETANIM_BOTH, BOTH_S6_S1, AFLAG_PACE);
			}
		}
		...
		{
			PM_SetAnim(pm, SETANIM_BOTH, BOTH_S7_S1, AFLAG_PACE);
		}
		else if (pm->ps->saber[0].stylesLearned == (1 << SS_STAFF)
	...
	else if (newMove == LS_A_JUMP_T__B_ && pm->ps->saberAnimLevel == SS_DESANN)
	{
	...
	if ((anim == BOTH_STAND1 && (pm->ps->saber[0].type == SABER_ARC || (pm->ps->dualSabers && pm->ps->saber[1].Active())))
		...
		|| (0 && anim == BOTH_SABERSTAFF_STANCE_ALT)
		|| (0 && anim == BOTH_SABERSTAFF_STANCE_IDLE)
		|| anim == BOTH_SABERDUAL_STANCE
		|| anim == BOTH_SABERDUAL_STANCE_GRIEVOUS
		|| anim == BOTH_SABERDUAL_STANCE_IDLE
		|| anim == BOTH_SABERFAST_STANCE
...
int G_SaberLockStrength(gentity_t * gent)
{
	...
	{//player
		strength += gent->client->ps.forcePowerLevel[FP_SABER_OFFENSE] + Q_irand(0, g_spskill->integer) + Q_irand(8, pm->cmd.buttons & BUTTON_ATTACK);
	}
	return strength;
}
...
qboolean PM_CanDoKata(void)
{
	...
	if (!pm->ps->saberInFlight//not throwing saber
		...
		&& G_EnoughPowerForSpecialMove(pm->ps->forcePower, SABER_ALT_ATTACK_POWER, qtrue))// have enough power
...
int PM_SetStandingSaberBlock(void)
{
		...
		&& !(pm->ps->pm_flags & PMF_DUCKED))
	{
			...
			&& !PM_WalkingOrRunningAnim(pm->ps->legsAnim))  //pushing a direction
		{
			if (pm->cmd.rightmove > 0)
			{//lean right
				if ((pm->ps->saber[0].type == SABER_STAFF) || (pm->ps->saber[0].type == SABER_STAFF_MAUL) || (pm->ps->saber[0].type == SABER_STAFF_SFX)
					|| (pm->ps->saber[0].type == SABER_STAFF_UNSTABLE) || (pm->ps->saber[0].type == SABER_ELECTROSTAFF) || (pm->ps->saber[0].type == SABER_STAFF_THIN)
					|| (pm->ps->saber[0].type == SABER_PALP) || (pm->ps->saber[0].type == SABER_ANAKIN))//saber staff
				{
	...
	{//player kicks
		...
		{//kick front/back
			...
			{//kick fwd
				if (pm->ps->groundEntityNum == ENTITYNUM_NONE || pm->cmd.upmove > 0)
				{
					PM_TryAirKick(LS_KICK_F_AIR2);
				}
				else if (pm->ps->weapon == WP_SABER && pm->gent && G_CheckEnemyPresence(pm->gent, DIR_FRONT, 64, 0.8f))
				{
...
{
	if (!PM_KickMove(pm->ps->saberMove))//not already in a kickk
	{//player kicks
		if (pm->cmd.rightmove)
		{//kick to side
			if (pm->cmd.rightmove > 0)
		...
		{//auto-pick?
			...
			else
			{
				...
				{//Matrix?
					...
					switch (kickMove)
					{
					case LS_KICK_B2://just pull back
					case LS_KICK_B_AIR://just pull back
						meFlags = MEF_NO_SPIN;
						break;
...
qboolean PM_SaberBlocking(void)
{
	...
	if (pm->ps->saberBlocked)
	{
		...
		case BLOCKED_UPPER_RIGHT:
			...
			else if (pm->ps->saber[0].type == SABER_ASBACKHAND)
			{
				PM_SetAnim(pm, SETANIM_TORSO, BOTH_BLOCK_BACK_HAND, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
				pm->ps->torsoAnimTimer += Q_irand(200, 1000);
				pm->ps->weaponTime = pm->ps->torsoAnimTimer;
...
void PM_WeaponLightsaber(void)
{
	...
	{//in knockdown
			...
			&& pm->ps->legsAnimTimer <= 250)
		{
			...
			{
				if (G_EnoughPowerForSpecialMove(pm->ps->forcePower, FATIGUE_JUMPATTACK))
				{
					...
					{//okay to do roll-stab
						...
						if (pm->gent)
						{
							G_DrainPowerForSpecialMove(pm->gent, FP_SABER_OFFENSE, FATIGUE_JUMPATTACK);
						}
					}
				}
			}
	...
	{//Just selected the weapon
		...
		if (pm->gent && (pm->gent->s.number < MAX_CLIENTS || G_ControlledByPlayer(pm->gent)))
		{
			...
			default:
				if (pm->ps->pm_flags & PMF_DUCKED)
				{
					int anim = PM_ReadyPoseForSaberAnimLevelDucked();
				}
				else
				{
					if (pm->gent->s.number >= MAX_CLIENTS && !G_ControlledByPlayer(pm->gent))
					{
						int anim = PM_ReadyPoseForSaberAnimLevelNPC();
					}
					else
					{
						int anim = PM_ReadyPoseForSaberAnimLevel();
	...
	if (!delayed_fire)
	{
		...
		{	// Last attack is not yet complete.
			...
			{
				PM_SetAnim(pm, SETANIM_TORSO, PM_ReadyPoseForSaberAnimLevel(), SETANIM_FLAG_OVERRIDE);
				return;
			}
			else
			{
...
//---------------------------------------
{
	...
	{
		//------------------
		//case WP_BRYAR_PISTOL://SBD WEAPON
...
static void PM_Weapon(void)
{
	...
	else
	{
		...
		if (pm->cmd.buttons & BUTTON_BLOCK && pm->cmd.buttons & BUTTON_ALT_ATTACK)
		{//set up the block position
			PM_SetMeleeBlock();
			pm->cmd.forwardmove = 0;
			pm->cmd.rightmove = 0;
	...
	if (pm->ps->weaponstate == WEAPON_RAISING)
	{
		...
		else
		{
			...
			case WP_BRYAR_PISTOL://SBD WEAPON
				...
				{//dual pistols
					if (pm->gent && pm->gent->client && pm->gent->client->NPC_class == CLASS_REBORN)
					{
						PM_SetAnim(pm, SETANIM_TORSO, BOTH_DUELPISTOL_STAND, SETANIM_FLAG_NORMAL);
					}
					else if (pm->gent && pm->gent->client && pm->gent->client->NPC_class == CLASS_BOBAFETT)
					{
						PM_SetAnim(pm, SETANIM_TORSO, BOTH_DUELPISTOL_STAND, SETANIM_FLAG_NORMAL);
					}
					else if (pm->gent&& pm->gent->client&& pm->gent->client->NPC_class == CLASS_MANDO)
					{
						PM_SetAnim(pm, SETANIM_TORSO, BOTH_DUELPISTOL_STAND, SETANIM_FLAG_NORMAL);
					}
					else
	...
	if (!delayed_fire)
	{
		...
		else
		{
			...
			case WP_BRYAR_PISTOL://SBD WEAPON
				...
				{//dual pistols
					if (pm->gent && pm->gent->client && pm->gent->client->NPC_class == CLASS_BOBAFETT)
					{
						PM_SetAnim(pm, SETANIM_TORSO, BOTH_DUELPISTOL_FIRE, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_RESTART | SETANIM_FLAG_HOLD);
					}
					else if (pm->gent && pm->gent->client && pm->gent->client->NPC_class == CLASS_MANDO)
					{
						PM_SetAnim(pm, SETANIM_TORSO, BOTH_DUELPISTOL_FIRE, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_RESTART | SETANIM_FLAG_HOLD);
					}
					else if (pm->gent&& pm->gent->client&& pm->gent->client->NPC_class == CLASS_REBORN)
					{
						PM_SetAnim(pm, SETANIM_TORSO, BOTH_DUELPISTOL_FIRE, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_RESTART | SETANIM_FLAG_HOLD);
					}
					else
				...
				{//single pistol
					if (pm->gent&& pm->gent->client&& pm->gent->client->NPC_class == CLASS_SBD)
					{
						PM_SetAnim(pm, SETANIM_TORSO, SBD_WEAPON_OUT_STANDING, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_RESTART | SETANIM_FLAG_HOLD);
					}
					else
					{//single pistol
						PM_SetAnim(pm, SETANIM_TORSO, BOTH_ATTACK2, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_RESTART | SETANIM_FLAG_HOLD);
			...
			case WP_MELEE:
				...
				if (pm->gent && pm->gent->client)
				{
					...
					else if (pm->gent && pm->gent->client && pm->gent->client->NPC_class == CLASS_WOOKIE)
					{
						if (rand() & 1)
						{
							PM_SetAnim(pm, SETANIM_TORSO, BOTH_WOOKIE_SLAP, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
						}
						else
						{
