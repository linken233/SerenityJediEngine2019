...
#define		BOBA_FLAMETHROWDAMAGEMAX	2//40
...
qboolean Boba_Flying( gentity_t *self )
{
	assert(self && self->client && (self->client->NPC_class == CLASS_BOBAFETT || self->client->NPC_class == CLASS_MANDO));
...
bool	Boba_CanSeeEnemy( gentity_t *self )
{
	assert(self && self->NPC && self->client && (self->client->NPC_class == CLASS_BOBAFETT || self->client->NPC_class == CLASS_MANDO || self->client->NPC_class == CLASS_COMMANDO));
...
void Boba_FireFlameThrower( gentity_t *self )
{
	...
	int			damage		= BOBA_FLAMETHROWDAMAGEMIN;
	...
	{
		G_Damage( traceEnt, self, self, dir, tr.endpos, damage, DAMAGE_NO_ARMOR|DAMAGE_NO_KNOCKBACK|DAMAGE_NO_HIT_LOC|DAMAGE_IGNORE_TEAM, MOD_BURNING, HL_NONE );
...
void Boba_StartFlameThrower( gentity_t *self )
{
	if (!(NPCInfo->aiFlags&NPCAI_FLAMETHROW))
	{
		...
		G_PlayEffect( G_EffectIndex("flamethrower/flamethrower"), self->playerModel, self->genericBolt3, self->s.number, self->s.origin, 1 );
...
void	Boba_TacticsSelect()
{
	...
	else if (enemyInFlameRange)
	{
		...
		//-----------------------------------------------------------------------------
		if (TIMER_Done(NPC, "nextFlameDelay") && (NPC->client->NPC_class == CLASS_BOBAFETT || NPC->client->NPC_class == CLASS_MANDO))
		{
...
void	Boba_Update()
{
	...
	//--------------------------------------------------------------------------------------------------
	if (!BobaHadDeathScript && NPC->behaviorSet[BSET_DEATH]!=0 && NPC->client->NPC_class != CLASS_MANDO)
	{
