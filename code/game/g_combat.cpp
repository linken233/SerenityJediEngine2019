...
qboolean G_GetHitLocFromSurfName(gentity_t * ent, const char* surfName, int* hitLoc, vec3_t point, vec3_t dir, vec3_t bladeDir, int mod, saberType_t saberType)
{
	...
	*hitLoc = HL_NONE;
	...
	if (debug_subdivision->integer && !in_camera)
	{
		dismember = qtrue;
	}
	...
	else if (debug_subdivision->integer && !in_camera || !ent->client->dismembered)
	{
...
qboolean G_DoDismembermentnormal(gentity_t * self, vec3_t point, int mod, int damage, int hitLoc, qboolean force = qfalse)
{
...
void player_die(gentity_t * self, gentity_t * inflictor, gentity_t * attacker, int damage, int meansOfDeath, int dflags, int hitLoc)
{
	...
	if (attacker)
	{
		...
		if (d_slowmodeath->integer)
		{
			...
			{//any jedi killed by player-saber
				...
				{//Matrix!
					if (attacker->client->ps.torsoAnim == BOTH_A6_SABERPROTECT || attacker->client->ps.torsoAnim == BOTH_GRIEVOUS_PROTECT || attacker->client->ps.torsoAnim == BOTH_GRIEVOUS_SPIN)
	...
	else if (self->s.weapon != WP_BRYAR_PISTOL)//SBD WEAPON
...
void PlayerPain(gentity_t * self, gentity_t * inflictor, gentity_t * other, const vec3_t point, int damage, int mod, int hitLoc)
{
	...
	else
	{
		...
		if (damage != -1 && (mod == MOD_MELEE || damage == 0 || (Q_irand(0, 10) <= damage && self->client->damage_blood)))
...
int CheckArmor(gentity_t * ent, int damage, int dflags, int mod)
{
	...
	if (client->NPC_class == CLASS_ASSASSIN_DROID)
	{
		...
		//-----------------------------------------------------------
		if (mod == MOD_GAS || mod == MOD_IMPACT || mod == MOD_LAVA || mod == MOD_BURNING || mod == MOD_SLIME || mod == MOD_WATER ||
		...
		//------------------------------------------------------
		if (mod == MOD_GAS || mod == MOD_IMPACT || mod == MOD_LAVA || mod == MOD_SLIME || mod == MOD_WATER || mod == MOD_BURNING)
		{
			return damage / 2;
		}
...
void G_Damage(gentity_t * targ, gentity_t * inflictor, gentity_t * attacker, const vec3_t dir, const vec3_t point, int damage, int dflags, int mod, int hitLoc)
{
	...
	{//it acts like a stun hit
		...
		{//knockdown
			...
			else if (targ->client->ps.forcePower < 25)
			{
				G_Knockdown(targ, attacker, dir, 50, qtrue);
			}
		}
	...
	if (mod == MOD_DEMP2 || mod == MOD_DEMP2_ALT)
	{
		if (client)
		{
				...
				client->NPC_class == CLASS_MOUSE || client->NPC_class == CLASS_GONK)
			{
				...
				client->NPC_class == CLASS_ATST || client->NPC_class == CLASS_SBD || client->NPC_class == CLASS_BATTLEDROID ||
				client->NPC_class == CLASS_DROIDEKA)
			{
...
{//get a small bonus for killing an enemy
	...
	attacker->client->ps.blockPoints += FATIGUE_KILLBONUS;
}
...
{//get a small bonus for killing an enemy
	...
	attacker->client->ps.blockPoints += FATIGUE_DAMAGEBONUS;
}
...
{//get a small bonus for killing an enemy
	...
	attacker->client->ps.blockPoints += FATIGUE_HURTBONUS;
}
...
{//get a small bonus for killing an enemy
	...
	attacker->client->ps.blockPoints += FATIGUE_HURTBONUSMAX;
}
