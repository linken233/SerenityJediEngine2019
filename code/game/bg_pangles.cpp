...
qboolean PM_AdjustAnglesForBackAttack( gentity_t *ent, usercmd_t *ucmd )
{
		...
		&& PM_InAnimForSaberMove( ent->client->ps.torsoAnim, ent->client->ps.saberMove ) )
	{
		...
		else if (ent->client->ps.saberMove != LS_A_BACKSTAB_B || !ent->enemy || (ent->s.number >= MAX_CLIENTS && !G_ControlledByPlayer(ent)))
		{
			if (ent->client->ps.viewEntity <= 0 || ent->client->ps.viewEntity >= ENTITYNUM_WORLD)
			{//don't clamp angles when looking through a viewEntity
				SetClientViewAngle(ent, ent->client->ps.viewangles);
...
void PM_UpdateViewAngles( playerState_t *ps, usercmd_t *cmd, gentity_t *gent )
{
	...
	if (gent && gent->client && gent->s.weapon == WP_MELEE)
	{	//only in the real pmove
		if ((cmd->buttons & BUTTON_BLOCK && cmd->buttons & BUTTON_ALT_ATTACK))
		{//check leaning
			if (cg.renderingThirdPerson)
			{//third person lean
				if ((cmd->rightmove || (cmd->forwardmove)))//pushing a direction
				{
					...
					if (anim != -1)
					{
						int extraHoldTime = 0;
						if (PM_MeleeblockAnim(ps->torsoAnim)
							&& !PM_MeleeblockHoldAnim(ps->torsoAnim))
						{//already in a dodge
						 //use the hold pose, don't start it all over again
