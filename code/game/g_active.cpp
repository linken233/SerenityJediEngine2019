...
void P_WorldEffects(gentity_t * ent)
{
	...
	if (ent->waterlevel && (ent->watertype & (CONTENTS_LAVA | CONTENTS_SLIME)))
	{
		if (ent->health > 0 && ent->painDebounceTime < level.time)
		{
			...
			{
				G_Damage(ent, NULL, NULL, NULL, NULL, 15 * ent->waterlevel, 0, MOD_LAVA);
			}
		...
		TIMER_Done(ent, "AcidPainDebounce"))
	{
		...
		{
			G_Damage(ent, NULL, NULL, NULL, NULL, 5, 0, MOD_SLIME);
		}
	}
	...
	if ((ent->client->poisonDamage) && (ent->client->poisonTime < level.time))
	{
		...
		G_Damage(ent, NULL, NULL, 0, 0, 4, DAMAGE_NO_KNOCKBACK | DAMAGE_NO_ARMOR, MOD_UNKNOWN);
	...
	if ((ent->client->stunDamage) && (ent->client->stunTime < level.time))
	{
		...
		G_Damage(ent, NULL, NULL, 0, 0, 3, DAMAGE_NO_KNOCKBACK | DAMAGE_NO_ARMOR, MOD_UNKNOWN);
	...
	{ //we're in space, check for suffocating and for exiting
		...
		if (!spacetrigger->inuse || !G_PointInBounds(ent->client->ps.origin, spacetrigger->absmin, spacetrigger->absmax))
	...
	if (!self->client || self->client->ps.lastOnGround + 300 < level.time || (self->client->ps.lastOnGround + 100 < level.time))
	{
		...
		if (damageSelf && self->takedamage && !(self->flags & FL_NO_IMPACT_DMG))
		{
			...
			if ((magnitude >= 100 + self->health
				...
				|| self->client->NPC_class == CLASS_VEHICLE || (magnitude >= 700))//health here is used to simulate structural integrity
			{
				if ((self->s.weapon == WP_SABER || self->s.number < MAX_CLIENTS || (self->client && (self->client->NPC_class == CLASS_BOBAFETT || self->client->NPC_class == CLASS_MANDO || self->client->NPC_class == CLASS_ROCKETTROOPER))) && self->client && self->client->ps.groundEntityNum < ENTITYNUM_NONE && magnitude < 1000)
...
qboolean WP_AbsorbKick(gentity_t * ent, gentity_t * pusher, const vec3_t pushDir)
{
	...
	if (ent->client->ps.forcePowerLevel[FP_SABER_DEFENSE] < FORCE_LEVEL_1)
	{//only force-users
		return qfalse;
	}
	...
	if (ent->client->ps.BlasterAttackChainCount >= BLASTERMISHAPLEVEL_HEAVY)
	{
		return qfalse;
	}

	if (ent->client->ps.saberAttackChainCount >= MISHAPLEVEL_HEAVY)
	{
		return qfalse;
	}

	if (ent->client->ps.saberAttackChainCount >= MISHAPLEVEL_LIGHT && pusher->client->ps.saberAnimLevel == SS_DESANN)
	{
		return qfalse;
	}

	if (ent->client->ps.forcePower < FATIGUE_DODGEING)
	...
	VectorSet(pLAngles, 0, ent->client->ps.viewangles[YAW], 0);
	...
	if (pusher->client->ps.torsoAnim == BOTH_MELEEUP)//for the front kick
	{
		if (pusher->client->ps.forcePower >= 75)
		{
			NPC_SetAnim(ent, SETANIM_BOTH, BOTH_KNOCKDOWN1, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
			WP_ForcePowerDrain(ent, FP_SABER_DEFENSE, FATIGUE_KICKHIT);
		}
		else if (Manual_Saberblocking(ent))
		{
			NPC_SetAnim(ent, SETANIM_BOTH, BOTH_FLIP_BACK1, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
			WP_ForcePowerDrain(ent, FP_SABER_DEFENSE, FATIGUE_KICKHIT);
		}
		else
		{
			NPC_SetAnim(ent, SETANIM_BOTH, BOTH_FLIP_BACK2, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
			WP_ForcePowerDrain(ent, FP_SABER_DEFENSE, FATIGUE_KICKHIT);
		}
		AddFatigueDamageBonus(pusher, ent);
	}
	else if (pusher->client->ps.torsoAnim == BOTH_WOOKIE_SLAP)//for the front kick
	{
		if (pusher->client->ps.forcePower >= 75)
		{
	...
	else if (pusher->client->ps.legsAnim == BOTH_A7_KICK_F)//for the front kick
	{
		if (pusher->client->ps.forcePower >= 75)
		{
			NPC_SetAnim(ent, SETANIM_BOTH, BOTH_KNOCKDOWN1, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
	...
	else if (pusher->client->ps.legsAnim == BOTH_A7_KICK_F2)//for the front kick
	{
		if (pusher->client->ps.forcePower >= 75)
		{
	...
	else if (pusher->client->ps.legsAnim == BOTH_A7_KICK_R)//for the r slap
	{
		if (pusher->client->ps.forcePower >= 75)
		{
			NPC_SetAnim(ent, SETANIM_BOTH, BOTH_KNOCKDOWN2, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
	...
	else if (pusher->client->ps.torsoAnim == BOTH_A7_SLAP_R)//for the r slap
	{
		if (pusher->client->ps.forcePower >= 75)
		{
	...
	else if (pusher->client->ps.legsAnim == BOTH_A7_KICK_L)//for the l slap
	{
		if (pusher->client->ps.forcePower >= 75)
		{
			NPC_SetAnim(ent, SETANIM_BOTH, BOTH_KNOCKDOWN2, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
	...
	else if (pusher->client->ps.torsoAnim == BOTH_A7_SLAP_L)//for the l slap
	{
		if (pusher->client->ps.forcePower >= 75)
		{
			NPC_SetAnim(ent, SETANIM_BOTH, BOTH_SLAPDOWNLEFT, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
	...
	else if (pusher->client->ps.torsoAnim == BOTH_TUSKENLUNGE1)//for the l slap
	{
		if (pusher->client->ps.forcePower >= 75)
		{
			NPC_SetAnim(ent, SETANIM_BOTH, BOTH_SLAPDOWNRIGHT, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
	...
	else if (pusher->client->ps.torsoAnim == BOTH_TUSKENATTACK1)//for the l slap
	{
		if (pusher->client->ps.forcePower >= 75)
		{
			NPC_SetAnim(ent, SETANIM_BOTH, BOTH_KNOCKDOWN1, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
	...
	else if (pusher->client->ps.torsoAnim == BOTH_TUSKENATTACK2)//for the l slap
	{
		if (pusher->client->ps.forcePower >= 75)
		{
			NPC_SetAnim(ent, SETANIM_BOTH, BOTH_KNOCKDOWN2, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
	...
	{
		if (pusher->client->ps.forcePower >= 50)
		{
			NPC_SetAnim(ent, SETANIM_BOTH, BOTH_KNOCKDOWN5, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
			WP_ForcePowerDrain(ent, FP_SABER_DEFENSE, FATIGUE_KICKHIT);
		}
	...
	{//if anything else other than kick/slap/roundhouse or karate
		NPC_SetAnim(ent, SETANIM_BOTH, BOTH_FLIP_BACK3, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
		WP_ForcePowerDrain(ent, FP_SABER_DEFENSE, FATIGUE_KICKHIT);
		AddFatigueDamageBonus(pusher, ent);
	}
	return qtrue;
...
qboolean G_CheckClampUcmd(gentity_t * ent, usercmd_t * ucmd)
{
	...
	else if (ent->client->ps.legsAnim == BOTH_GRIEVOUS_SPIN || ent->client->ps.torsoAnim == BOTH_GRIEVOUS_SPIN || ent->client->ps.saberMove == LS_GRIEVOUS_SPECIAL)
	{
		ucmd->forwardmove = ucmd->rightmove = ucmd->upmove = 0;
	}
	...
	{//backflip attack
		...
		{//middle of anim
			...
			{//still on ground?
				...
				if (ent->client->ps.forcePowerLevel[FP_SPEED] < FORCE_LEVEL_3)
				{//short burst
	...
	else if (BG_InSlowBounce(&ent->client->ps))
	{//can't move during a slow bounce
		ucmd->rightmove = 0;
		ucmd->upmove = 0;
		ucmd->forwardmove = 0;
	}
	else if (BG_InBlockedAttackBounce(&ent->client->ps))
	...
	else if (BG_FullBodyTauntAnim(ent->client->ps.legsAnim))
	{
		...
		{//stop the anim
			...
			else if (ent->client->ps.legsAnim == BOTH_MEDITATE1 && ent->client->ps.torsoAnim == BOTH_MEDITATE1)
			{
		...
		else
		{
			...
			if (ent->client->ps.torsoAnim == BOTH_MEDITATE1)
			{
				if (ent->client->ps.torsoAnimTimer < 100)
				{
					ent->client->ps.legsAnimTimer = 100;
...
{
	gentity_t* tracedEnemy;
	trace_t tr;
	vec3_t endTrace, forward;
	vec3_t eyes;
	VectorCopy(caller->currentOrigin, eyes);
	eyes[2] += 20.0f;
	AngleVectors(caller->client->ps.viewangles, forward, NULL, NULL);
	...
	tracedEnemy = &g_entities[tr.entityNum];
	...
	{
		G_AddEvent(caller, EV_CALLOUT_FAR, 0);
	}
	else if (ourFriend->client->ps.stats[STAT_ARMOR] < 70)
	{
		G_AddEvent(caller, EV_CALLOUT_VC_REQ_ASSIST, 0);
	}
	else if (ourFriend->client->ps.stats[STAT_HEALTH] < 70)
	{
		G_AddEvent(caller, EV_CALLOUT_VC_REQ_MEDIC, 0);
	}
	else
	{
		vec3_t dir;
		...
		angle = DotProduct(ourFriend->currentAngles, dir);

		if (angle >= 0.2 && angle <= 1.0)
		{
			G_AddEvent(caller, EV_CALLOUT_BEHIND, 0);
		}
		else
		{
			G_AddEvent(caller, EV_CALLOUT_NEAR, 0);
		}
	}

	tracedEnemy->markTime = level.time + 20000;
...
void ClientAlterSpeed(gentity_t * ent, usercmd_t * ucmd, qboolean	controlledByPlayer, float vehicleFrameTimeModifier)
{
	...
	if (ent->NPC != NULL && ent->client && (ent->s.m_iVehicleNum != 0))
...
void ClientThink_real(gentity_t * ent, usercmd_t * ucmd)
{
	...
	if (ent->s.number == 0)
	{
		...
		if (ent->client->renderInfo.lookTargetClearTime < level.time //NOTE: here this is used as a debounce, not an actual timer
			...
			&& (!ent->enemy || ent->client->ps.saberMove != LS_A_BACKSTAB_B))//don't update if in backstab unless don't currently have an enemy
	...
	if (ent->client->ps.weapon == WP_SABER && !ent->NPC && ent->client->ps.SaberActive())	// NPCs don't use this method, they do it on their own terms 
	{
...
void ClientThink(int clientNum, usercmd_t * ucmd)
{
	...
	if (ent->s.number < MAX_CLIENTS)
	{
		...
		if ((ucmd->buttons& BUTTON_BLOCK) && !g_saberAutoBlocking->integer)
		{
