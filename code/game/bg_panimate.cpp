...
saberMoveData_t	saberMoveData[LS_MOVE_MAX] = {//							NB:randomized
	...
	{ "Jump Att",	BOTH_FORCELEAP2_T__B_,Q_T,	Q_B,	AFLAG_ACTIVE,	100,	BLK_TIGHT,	LS_READY,		LS_READY,		200 },	// LS_A_JUMP_T__B__
	...
	{ "Reflect Front",BOTH_P1_S1_T1_,	Q_R,	Q_T,	AFLAG_ACTIVE,	50,		BLK_WIDE,	LS_R_BL2TR,		LS_A_T2B,		150 },	// LS_REFLECT_FRONT,
	...
	{//alt+back+attack using fast, dual or staff attacks
		...
		{//player
			if (G_TryingLungeAttack(pm->gent, &pm->cmd)
				&& G_EnoughPowerForSpecialMove(pm->ps->forcePower, FATIGUE_JUMPATTACK))//have enough force power to pull it off
			{
				return qtrue;
			}
		}
...
saberMoveName_t PM_SaberJumpForwardAttackMove(void)
{
	...
	if (pm->ps->saberAnimLevel == SS_DUAL || pm->ps->saberAnimLevel == SS_STAFF)
	{
		...
		if (pm->ps->saber[0].type == SABER_GRIE4 || pm->ps->saber[0].type == SABER_GRIE || pm->ps->saber[0].type == SABER_PALP
			&& pm->ps->saberAnimLevel == SS_DUAL)
		{
				...
				&& (pm->cmd.buttons & BUTTON_ATTACK))//want to attack
			{
				...
				{//PLAYER
					if (G_TryingJumpForwardAttack(pm->gent, &pm->cmd)
						&& G_EnoughPowerForSpecialMove(pm->ps->forcePower, FATIGUE_JUMPATTACK))//have enough power to attack
					{
						return qtrue;
					}
				}
...
void PM_SaberStartTransAnim(int saberAnimLevel, int anim, float* animSpeed, gentity_t * gent, int fatigued)
{
	char	buf[128];
...
void PM_SetAnimFinal(int* torsoAnim, int* legsAnim,	int setAnimParts, int anim, int setAnimFlags, int* torsoAnimTimer, int* legsAnimTimer, gentity_t * gent, int blendTime)		// default blendTime=350
{
	...
	{//make me parry
		if (Manual_Saberreadyanim(anim))
		{
			blendTime *= 1.8f;
		}
		if (Manual_Saberblockinganim(anim))
		{
			blendTime *= 1.8f;
		}
		if (gent->client->ps.ManualBlockingFlags & (1 << MBF_BLOCKING) && anim == LS_READY)
		{
			blendTime *= 1.8f;
		}
		if (gent->client->ps.ManualBlockingFlags & (1 << MBF_MBLOCKING))
		{
			blendTime *= 1.8f;
		}
	}
	...
	//--------------------------------------------------------------------
	PM_SaberStartTransAnim(gent->client->ps.saberAnimLevel, anim, &timeScaleMod, gent, gent->userInt3);
			...
			if (gi.WE_GetWindVector(windDir, ent->currentOrigin))
			{
				...
				if (DotProduct(fwd, windDir) > 0.65f)
				{
					if (!Manual_Saberblocking(ent))
					{
						...
						{
							NPC_SetAnim(ent, SETANIM_TORSO, BOTH_WIND, SETANIM_FLAG_OVERRIDE, 400);
						}
					}
...
//	int	oldAnim;
	...
	if (pm->ps->taunting > level.time)
	{
		...
		else if (PM_HasAnimation(pm->gent, BOTH_GESTURE1))
		{
			...
			{//2-handed PUSH
				PM_SetAnim(pm, SETANIM_BOTH, BOTH_TUSKENTAUNT1, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);//SETANIM_FLAG_NORMAL
			}
			else
			{
