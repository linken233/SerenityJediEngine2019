...
void NPC_BSST_Attack(void)
{
	...
	if (NPC->client->NPC_class == CLASS_STORMCOMMANDO
		...
		&& !PM_InKnockDown(&NPC->client->ps))
	{
		if (NPC->client->ps.torsoAnim == BOTH_TUSKENLUNGE1)
		{
			shoot = qfalse;
		...
		&& !PM_InKnockDown(&NPC->client->ps))
	{
		...
		{//enemy within 80, if very close, use melee attack to slap away
			...
			{//animate me
				...
				G_Sound(NPC->enemy, G_SoundIndex("sound/chars/ultraclones/misc/taunt3.wav"));
				NPC_SetAnim(NPC, SETANIM_BOTH, swingAnim, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
				TIMER_Set(NPC, "slapattackDelay", NPC->client->ps.torsoAnimTimer + Q_irand(15000, 20000));
				//delay the hurt until the proper point in the anim
				TIMER_Set(NPC, "smackTime", 300);
		...
		&& !PM_InKnockDown(&NPC->client->ps))
	{
		...
		{//enemy within 80, if very close, use melee attack to slap away
			...
			{//animate me
				int swingAnim;
				swingAnim = BOTH_TUSKENATTACK1;
				G_Sound(NPC->enemy, G_SoundIndex("sound/chars/ultraclones/misc/taunt.wav"));
				NPC_SetAnim(NPC, SETANIM_BOTH, swingAnim, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
				TIMER_Set(NPC, "slapattackDelay", NPC->client->ps.torsoAnimTimer + Q_irand(15000, 20000));
				//delay the hurt until the proper point in the anim
				TIMER_Set(NPC, "smackTime", 300);
			...
			{//time to smack
			 //recheck enemyDist and InFront
					...
					&& InFront(NPC->enemy->currentOrigin, NPC->currentOrigin, NPC->client->ps.viewangles, 0.3f))
				{
					...
					VectorNormalize(smackDir);
					//hurt them
					G_Sound(NPC->enemy, G_SoundIndex("sound/SBD/ArmBash.mp3"));
					G_Damage(NPC->enemy, NPC, NPC, smackDir, NPC->currentOrigin, (g_spskill->integer + 1) * Q_irand(2, 5), DAMAGE_NO_KNOCKBACK, MOD_MELEE);
					...
					else if (NPC->client->ps.torsoAnim == BOTH_A7_SLAP_L)
					{
						G_Throw(NPC->enemy, smackDir, 65);
		...
		&& !PM_InKnockDown(&NPC->client->ps))
	{
		...
		{//enemy within 80, if very close, use melee attack to slap away
			...
			{//animate me
				int swingAnim;
				swingAnim = Q_irand(BOTH_A7_SLAP_R, BOTH_A7_SLAP_L);
				G_Sound(NPC->enemy, G_SoundIndex("sound/SBD/ArmBash.mp3"));
				NPC_SetAnim(NPC, SETANIM_BOTH, swingAnim, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
				TIMER_Set(NPC, "slapattackDelay", NPC->client->ps.torsoAnimTimer + Q_irand(15000, 20000));
				//delay the hurt until the proper point in the anim
	...
	if (NPC->client->NPC_class == CLASS_WOOKIE
		&& NPC->client->playerTeam == TEAM_PLAYER
		&& !PM_InKnockDown(&NPC->client->ps))
	{
		if (NPC->client->ps.torsoAnim == BOTH_WOOKIE_SLAP
			...
			{//time to smack
			 //recheck enemyDist and InFront
					...
					&& InFront(NPC->enemy->currentOrigin, NPC->currentOrigin, NPC->client->ps.viewangles, 0.3f))
				{
					...
					G_Damage(NPC->enemy, NPC, NPC, smackDir, NPC->currentOrigin, (g_spskill->integer + 1) * Q_irand(10, 20), DAMAGE_NO_KNOCKBACK, MOD_MELEE);
					if (NPC->client->ps.torsoAnim == BOTH_WOOKIE_SLAP
						|| NPC->client->ps.torsoAnim == BOTH_MELEEUP)
					{//smackdown
						int knockAnim = BOTH_KNOCKDOWN1;
		...
		&& !PM_InKnockDown(&NPC->client->ps))
	{
		...
		{//enemy within 80, if very close, use melee attack to slap away
			...
			{//animate me
				int swingAnim;
				swingAnim = Q_irand(BOTH_WOOKIE_SLAP, BOTH_MELEEUP);
				G_Sound(NPC->enemy, G_SoundIndex("sound/weapons/melee/punch1.mp3"));
				NPC_SetAnim(NPC, SETANIM_BOTH, swingAnim, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
				TIMER_Set(NPC, "attackDelay", NPC->client->ps.torsoAnimTimer + Q_irand(15000, 20000));
				//delay the hurt until the proper point in the anim
	...
	if (NPC->client->NPC_class == CLASS_NOGHRI
		...
		&& !PM_InKnockDown(&NPC->client->ps))//not knocked down ) )
	{
		if (NPC->client->ps.torsoAnim == BOTH_A7_HILT)
		{
			shoot = qfalse;
			if (TIMER_Done(NPC, "smackTime") && !NPCInfo->blockedDebounceTime)
			{//time to smack
		...
		&& !PM_InKnockDown(&NPC->client->ps))
	{
		...
		{//enemy within 80, if very close, use melee attack to slap away
			...
			{//animate me
				...
				if (NPC->health > 70)
				{
					swingAnim = BOTH_A7_HILT;//smackdown or uppercut			
				}
				G_Sound(NPC->enemy, G_SoundIndex("sound/weapons/melee/punch2.mp3"));
				NPC_SetAnim(NPC, SETANIM_BOTH, swingAnim, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
				TIMER_Set(NPC, "attackDelay", NPC->client->ps.torsoAnimTimer + Q_irand(15000, 20000));
				//delay the hurt until the proper point in the anim
				TIMER_Set(NPC, "smackTime", 300);
			...
			{//time to smack
			 //recheck enemyDist and InFront
					...
					&& InFront(NPC->enemy->currentOrigin, NPC->currentOrigin, NPC->client->ps.viewangles, 0.3f))
				{
					...
					//hurt them
					G_Sound(NPC->enemy, G_SoundIndex("sound/chars/%s/misc/pain0%d"));
					G_Damage(NPC->enemy, NPC, NPC, smackDir, NPC->currentOrigin, (g_spskill->integer + 1) * Q_irand(1, 3), DAMAGE_NO_KNOCKBACK, MOD_MELEE);
					if (NPC->client->ps.torsoAnim == BOTH_A7_KICK_F)
					{//smackdown
						int knockAnim = BOTH_KNOCKDOWN1;
						if (PM_CrouchAnim(NPC->enemy->client->ps.legsAnim))
						...
						smackDir[2] = 1;
						VectorNormalize(smackDir);
						G_Throw(NPC->enemy, smackDir, 25);
						NPC_SetAnim(NPC->enemy, SETANIM_BOTH, knockAnim, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
					}
					else if (NPC->client->ps.torsoAnim == BOTH_A7_KICK_F2)
					{
			...
			{//time to smack
			 //recheck enemyDist and InFront
					...
					&& InFront(NPC->enemy->currentOrigin, NPC->currentOrigin, NPC->client->ps.viewangles, 0.3f))
				{
					...
					//hurt them
					G_Sound(NPC->enemy, G_SoundIndex("sound/chars/%s/misc/pain0%d"));
					G_Damage(NPC->enemy, NPC, NPC, smackDir, NPC->currentOrigin, (g_spskill->integer + 1) * 1, DAMAGE_NO_KNOCKBACK, MOD_MELEE);

					if (NPC->client->ps.torsoAnim == BOTH_MELEE_R)
					{//smackdown
						...
						smackDir[2] = 1;
						VectorNormalize(smackDir);
						G_Throw(NPC->enemy, smackDir, 10);
						NPC_SetAnim(NPC->enemy, SETANIM_BOTH, knockAnim, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
					}
					else if (NPC->client->ps.torsoAnim == BOTH_MELEE_L)
					{
						G_Throw(NPC->enemy, smackDir, 10);
						NPC_SetAnim(NPC->enemy, SETANIM_BOTH, BOTH_PAIN3, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
					}
					else if (NPC->client->ps.torsoAnim == BOTH_MELEEUP)
					{
						G_Throw(NPC->enemy, smackDir, 10);
						NPC_SetAnim(NPC->enemy, SETANIM_BOTH, BOTH_PAIN4, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
					}
					else
					{//uppercut
					 //throw them
						G_Throw(NPC->enemy, smackDir, 10);
						//make them backflip
						NPC_SetAnim(NPC->enemy, SETANIM_BOTH, BOTH_PAIN1, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
					}
					//done with the damage
					NPCInfo->blockedDebounceTime = 1;
				}
			...
			{//time to smack
			 //recheck enemyDist and InFront
					...
					&& InFront(NPC->enemy->currentOrigin, NPC->currentOrigin, NPC->client->ps.viewangles, 0.3f))
				{
					...
					//hurt them
					G_Sound(NPC->enemy, G_SoundIndex("sound/chars/%s/misc/pain0%d"));
					G_Damage(NPC->enemy, NPC, NPC, smackDir, NPC->currentOrigin, (g_spskill->integer + 1) * Q_irand(5, 10), DAMAGE_NO_KNOCKBACK, MOD_MELEE);
					if (NPC->client->ps.torsoAnim == BOTH_A7_KICK_F)
					{//smackdown
						int knockAnim = BOTH_KNOCKDOWN1;
						if (PM_CrouchAnim(NPC->enemy->client->ps.legsAnim))
	...
	{//have an enemy
		if (NPC->enemy->client //enemy is a client
			...
			&& (!NPC->enemy->enemy || !NPC->enemy->enemy->client || (NPC->enemy->enemy->client->NPC_class != CLASS_RANCOR
				&& NPC->enemy->enemy->client->NPC_class != CLASS_WAMPA)))//enemy's enemy is not a client or is not a wampa or rancor (which is scarier than me)
		...
		{//have an enemy
			...
			if (TIMER_Done(NPC, "TalkTime"))
			{
				...
				{// Do taunt...
					...
					{
					case 2:
						G_AddVoiceEvent(NPC, Q_irand(EV_JCHASE1, EV_JCHASE3), 5000 + Q_irand(0, 15000));
						break;
					case 1:
						G_AddVoiceEvent(NPC, Q_irand(EV_COMBAT1, EV_COMBAT3), 5000 + Q_irand(0, 15000));
						break;
					default:
						G_AddVoiceEvent(NPC, Q_irand(EV_TAUNT1, EV_TAUNT3), 5000 + Q_irand(0, 15000));
					}
				}
