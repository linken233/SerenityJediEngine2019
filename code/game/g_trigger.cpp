	...
	if (actualDmg)
	{
		...
		{//falling death
			if (other->NPC && other->client &&
				(other->client->NPC_class == CLASS_BOBAFETT || other->client->NPC_class == CLASS_MANDO || other->client->NPC_class == CLASS_ROCKETTROOPER))
...
{//move asteroid to a new start position
	...
	{//move it
		...
		for (axis = 0; axis < 3; axis++)
		{
			...
			{
				startSpot[axis] = ownerTrigger->mins[axis] + (flrand(0, 1.0f) * (ownerTrigger->maxs[axis] - ownerTrigger->mins[axis]));
				endSpot[axis] = ownerTrigger->mins[axis] + (flrand(0, 1.0f) * (ownerTrigger->maxs[axis] - ownerTrigger->mins[axis]));
			}
		}
