	...
	do
	{
		...
		for ( search = fs_searchpaths ; search ; search = search->next ) {
			...
			if ( search->pack && search->pack->hashTable[hash] ) {
				// autoexec_sp.cfg and SerenityJediEngine2019-SP.cfg can only be loaded outside of pk3 files.
				if ( isUserConfig ) {
					continue;
				}
...
void FS_Startup( const char *gameName ) {
	...
	fs_gamedirvar = Cvar_Get ("fs_game", "SerenityJediEngine2019", CVAR_INIT|CVAR_SYSTEMINFO );

	fs_dirbeforepak = Cvar_Get("fs_dirbeforepak", "0", CVAR_INIT|CVAR_PROTECTED);
	Cvar_Get("com_outcast", "", CVAR_ARCHIVE);
...
void FS_InitFilesystem( void ) {
	...
	if ( FS_ReadFile( "spdefault.cfg", NULL ) <= 0 )
	{
