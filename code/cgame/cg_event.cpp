...
void CG_PainEvent(centity_t * cent, int health)
{
	...
	CG_TryPlayCustomSound(NULL, cent->currentState.number, CHAN_VOICE, snd, CS_BASIC);
...
//extern void CG_ToggleCloak(gentity_t *self);
...
void CG_EntityEvent(centity_t * cent, vec3_t position) {
	...
	case EV_FOOTSTEP:
		...
		if (cg_footsteps.integer)
		{
			...
			{//Everyone else has keyframed footsteps in animsounds.cfg
				cgi_S_StartSound(NULL, es->number, CHAN_BODY, cgs.media.footsteps[FOOTSTEP_NORMAL][rand() & 3]);
			}
		}
		break;
	...
	case EV_JUMP:
		...
		{
			CG_TryPlayCustomSound(NULL, es->number, CHAN_VOICE, "*jump1.wav", CS_BASIC);//CHAN_VOICE //play all jump sounds
		}
		else if (cg_jumpSounds.integer == 2 && cg.snap->ps.clientNum != es->number)
		{
			CG_TryPlayCustomSound(NULL, es->number, CHAN_VOICE, "*jump1.wav", CS_BASIC);//CHAN_VOICE //only play other players' jump sounds
		}
		else if (cg_jumpSounds.integer > 2 && cg.snap->ps.clientNum == es->number)
		{
			CG_TryPlayCustomSound(NULL, es->number, CHAN_VOICE, "*jump1.wav", CS_BASIC);//CHAN_VOICE//only play my jump sounds
		}
		break;
	...
	case EV_CALLOUT_BEHIND:
		...
		CG_TryPlayCustomSound(NULL, es->number, CHAN_VOICE, va("*confuse%i.wav", event - EV_CONFUSE1 + 1), CS_CALLOUT);
		break;

	case EV_CALLOUT_NEAR:
		DEBUGNAME("EV_CALLOUT_NEAR");
		CG_TryPlayCustomSound(NULL, es->number, CHAN_VOICE, va("*sight%i.wav", event - EV_SIGHT1 + 1), CS_CALLOUT);
		break;

	case EV_CALLOUT_FAR:
		DEBUGNAME("EV_CALLOUT_FAR");
		CG_TryPlayCustomSound(NULL, es->number, CHAN_VOICE, va("*escaping%i.wav", event - EV_ESCAPING1 + 1), CS_CALLOUT);
		break;

	case EV_CALLOUT_VC_REQ_ASSIST:
		DEBUGNAME("EV_CALLOUT_VC_REQ_ASSIST");
		CG_TryPlayCustomSound(NULL, es->number, CHAN_VOICE, "*req_assist.mp3", CS_CALLOUT);
		break;

	case EV_CALLOUT_VC_REQ_MEDIC:
		DEBUGNAME("EV_CALLOUT_VC_REQ_MEDIC");
		CG_TryPlayCustomSound(NULL, es->number, CHAN_VOICE, "*req_medic.mp3", CS_CALLOUT);
		break;
	...
	case EV_USE_INV_JETPACK:
		DEBUGNAME("EV_USE_INV_JETPACK");
		UseItem(INV_JETPACK);
		break;
