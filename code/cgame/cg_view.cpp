...
static void CG_CalcIdealThirdPersonViewTarget(void)
{
	...
	if ( usesViewEntity )
	{
		...
		{	// sort of a nasty hack in order to get this to work. Don't tell Ensiform, or I'll have to kill him. 
...
static void CG_CalcIdealThirdPersonViewLocation(void)
{
	...
	if (IsPressingDashButton(player))
	{
		if (cg.renderingThirdPerson && (cg.snap->ps.forcePowersActive&(1 << FP_SPEED)) && player->client->ps.forcePowerDuration[FP_SPEED])
		{
			...
			float length = FORCE_SPEED_DURATION_FORCE_LEVEL_1 * forceDashValue[player->client->ps.forcePowerLevel[FP_SPEED]];
			float amt = forceDashRangeMod[player->client->ps.forcePowerLevel[FP_SPEED]];
			if (timeLeft < 500)
			{//start going back
	...
	else
	{
		...
		{
			float timeLeft = player->client->ps.forcePowerDuration[FP_SPEED] - cg.time;
			float length = FORCE_SPEED_DURATION_FORCE_LEVEL_3 * forceSpeedValue[player->client->ps.forcePowerLevel[FP_SPEED]];
...
qboolean CG_CalcFOVFromX( float fov_x )
{
	...
	{
		float perc = (float)(cg.stunnedTime - cg.time) / 5000.0f; // goes for 5 seconds
...
float CG_ForceSpeedFOV( void )
{
	...
	if (IsPressingDashButton(player))
	{
		float timeLeft = player->client->ps.forcePowerDuration[FP_SPEED] - cg.time;
		float length = FORCE_SPEED_DURATION_FORCE_LEVEL_1 * forceDashValue[player->client->ps.forcePowerLevel[FP_SPEED]];
		float amt = forceDashFOVMod[player->client->ps.forcePowerLevel[FP_SPEED]];
