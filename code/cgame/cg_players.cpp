...
extern qboolean Manual_Saberblockinganim(int anim);
	...
	case CS_CALLOUT:
		...
		if (ci)
		{
			for (i = 0; i < MAX_CUSTOM_CALLOUT_SOUNDS && cg_customCalloutSoundNames[i]; i++)
			{
				...
				{
					return ci->sounds[i + MAX_CUSTOM_BASIC_SOUNDS + MAX_CUSTOM_COMBAT_SOUNDS + MAX_CUSTOM_EXTRA_SOUNDS + MAX_CUSTOM_JEDI_SOUNDS];
				}
			}
		}
		break;
	...
	default:
		...
		if (ci)
		{
			...
			for (i = 0; i < MAX_CUSTOM_CALLOUT_SOUNDS && cg_customCalloutSoundNames[i]; i++)
			{
				if (!Q_stricmp(soundName, cg_customCalloutSoundNames[i]))
				{
					return ci->sounds[i + MAX_CUSTOM_BASIC_SOUNDS + MAX_CUSTOM_COMBAT_SOUNDS + MAX_CUSTOM_EXTRA_SOUNDS + MAX_CUSTOM_CALLOUT_SOUNDS];
				}
			}
		}
		break;
...
void CG_NewClientinfo(int clientNum)
{
	...
	CG_RegisterCustomSounds(ci,
		MAX_CUSTOM_BASIC_SOUNDS + MAX_CUSTOM_COMBAT_SOUNDS + MAX_CUSTOM_EXTRA_SOUNDS + MAX_CUSTOM_JEDI_SOUNDS,
...
static void CG_PlayerAnimation(centity_t * cent, int* legsOld, int* legs, float* legsBackLerp, int* torsoOld, int* torso, float* torsoBackLerp)
{
	...
	*legsBackLerp = cent->pe.legs.backlerp;
	...
	if (newLegsFrame)
	{
		...
		{//make me parry
			if (Manual_Saberreadyanim(cent->currentState.torsoAnim))
			{
				speedScale *= 0.3f;
			}
			if (Manual_Saberblockinganim(cent->currentState.torsoAnim))
			{
				speedScale *= 0.3f;
			}
			if (cent->currentState.ManualBlockingFlags& (1 << MBF_BLOCKING) && cent->currentState.saberMove == LS_READY)
			{
				speedScale *= 0.3f;
			}
			if (cent->currentState.ManualBlockingFlags& (1 << MBF_MBLOCKING))
			{
				speedScale *= 0.3f;
			}
		}
		...
		{//invalid effect
			...
			{//some sort of hard-coded effect
				if (Q_stricmp("push_l", animEvent->stringData) == 0)
				{
					...
					cgi_S_StartSound(cent->lerpOrigin, cent->currentState.clientNum, CHAN_AUTO, cgi_S_RegisterSound("sound/weapons/force/pushyoda.mp3"));
					cgi_S_StartSound(cent->lerpOrigin, cent->currentState.clientNum, CHAN_AUTO, cgi_S_RegisterSound("sound/weapons/force/repulsepush.wav"));
...
static void CG_BreathSoundVader(centity_t * cent)
{
	...
	client->breathPuffTime = cg.time + 2500;
}
...
static void CG_BreathPuffsSith(centity_t * cent, vec3_t angles, vec3_t origin)
{
	gclient_t* client = cent->gent->client;
	...
	// Show bubbles effect if we're under water.
	if ((contents & CONTENTS_WATER) && (cent->gent->client->NPC_class == CLASS_DESANN || cent->gent->client->NPC_class == CLASS_TAVION))
	{
		CG_PlayEffectBolted("misc/waterbreathSith", cent->gent->playerModel, bolt, cent->currentState.clientNum, vEffectOrigin);
	}
	else if ((contents & CONTENTS_WATER) && (cent->gent->client->NPC_class == CLASS_VADER))
	{
		CG_PlayEffectBolted("misc/waterbreathVader", cent->gent->playerModel, bolt, cent->currentState.clientNum, vEffectOrigin);
	}
	else if (cent->gent->client->NPC_class == CLASS_DESANN || cent->gent->client->NPC_class == CLASS_TAVION)
	{
		...
		{
			CG_PlayEffectBolted("misc/breathSith", cent->gent->playerModel, bolt, cent->currentState.clientNum, vEffectOrigin);
		}
	}
	else if (cent->gent->client->NPC_class == CLASS_VADER)
	{
		...
		{
			CG_PlayEffectBolted("misc/breathVader", cent->gent->playerModel, bolt, cent->currentState.clientNum, vEffectOrigin);
		}
	}
	...
	{// If we are rendering third person, we should just force the player body to always fully face
		...
		else
		{
			...
			if (((cent->gent->client->ps.eFlags & EF_FORCE_GRIPPED)
				|| ((cent->gent->client->NPC_class == CLASS_BOBAFETT || cent->gent->client->NPC_class == CLASS_MANDO || cent->gent->client->NPC_class == CLASS_ROCKETTROOPER) && cent->gent->client->moveType == MT_FLYSWIM))
				&& cent->gent->client->ps.groundEntityNum == ENTITYNUM_NONE)
			{
				vec3_t	centFwd, centRt;
				float	divFactor = 1.0f;
				if ((cent->gent->client->NPC_class == CLASS_BOBAFETT || cent->gent->client->NPC_class == CLASS_MANDO || cent->gent->client->NPC_class == CLASS_ROCKETTROOPER)
...
//void CG_HyperspaceEffect(vec3_t origin, vec3_t normal)
//{
//	int	effectID = -1;
//
//	effectID = cgs.effects.mHyperspaceStars;
//
//	if (effectID != -1)
//	{
//		theFxScheduler.PlayEffect(effectID, origin, normal);
...
//		!cent->gent->m_pVehicle)
//	{
//		return;
//	}
//
//	pVehNPC = cent->gent->m_pVehicle;
//
//	if (cent->currentState.clientNum == cent->currentState.m_iVehicleNum//my vehicle
//		&& (cent->currentState.eFlags2&EF2_HYPERSPACE))//hyperspacing
//	{//in hyperspace!
//		if (cent->currentState.hyperSpaceTime
//			&& (cg.time - cent->currentState.hyperSpaceTime) < HYPERSPACE_TIME)
//		{
...
//				CG_HyperspaceEffect(cent->lerpOrigin, cent->currentState.origin);
...
//
//	////FLYBY sound
//	//if (cent->currentState.clientNum != cent->currentState.m_iVehicleNum
//	//	&& (pVehNPC->m_pVehicleInfo->soundFlyBy || pVehNPC->m_pVehicleInfo->soundFlyBy2))
//	//{//not my vehicle
//	//	if (cent->currentState.speed && cent->currentState.speed + cent->currentState.speed > 500)
//	//	{//he's moving and between the two of us, we're moving fast
//	//		vec3_t diff;
//	//		VectorSubtract(cent->lerpOrigin, cent->currentState.origin, diff);
...
void CG_DrawPlayerShield(centity_t * cent, vec3_t origin)
{
	...
	alpha = 255.0f * ((cent->damageTime - cg.time) / MIN_SHIELD_TIME) + Q_flrand(0.0f, 1.0f) * 16;
	if (alpha > 255)
		alpha = 255;
...
void CG_DoTFASaber(vec3_t blade_muz, vec3_t blade_tip, vec3_t trail_tip, vec3_t trail_muz, float lengthMax, float radius, saber_colors_t color, int rfx, qboolean doLight)
{
...

	{
		...
		saber.customShader = cgs.media.rgbTFASaberCoreShader;
...
void CG_DoBattlefrontSaber(vec3_t blade_muz, vec3_t blade_tip, vec3_t trail_tip, vec3_t trail_muz, float lengthMax, float radius, saber_colors_t color, int rfx, qboolean doLight)
{
	vec3_t	dif, mid, blade_dir, end_dir, trail_dir, base_dir;
	float	radiusmult, effectradius, coreradius, effectalpha, AngleScale;
	float	blade_len, end_len, trail_len, base_len, DisTip, DisMuz, DisDif;
...
void CG_DoEp1Saber(vec3_t blade_muz, vec3_t blade_tip, vec3_t trail_tip, vec3_t trail_muz, float lengthMax, float radius, saber_colors_t color, int rfx, qboolean doLight)
{
	vec3_t	dif, mid, blade_dir, end_dir, trail_dir, base_dir;
	float	radiusmult, effectradius, coreradius, effectalpha, AngleScale;
	float	blade_len, end_len, trail_len, base_len, DisTip, DisMuz, DisDif;
...
void CG_DoEp2Saber(vec3_t blade_muz, vec3_t blade_tip, vec3_t trail_tip, vec3_t trail_muz, float lengthMax, float radius, saber_colors_t color, int rfx, qboolean doLight)
{
	vec3_t	dif, mid, blade_dir, end_dir, trail_dir, base_dir;
	float	radiusmult, effectradius, coreradius, effectalpha, AngleScale;
	float	blade_len, end_len, trail_len, base_len, DisTip, DisMuz, DisDif;
	...
	effectradius = ((radius * 1.6f * v1) + Q_flrand(-1.0f, 1.0f)) * radiusmult * cg_SFXSabersGlowSize.value;
	coreradius = ((radius * 0.4f * v2) + Q_flrand(-1.0f, 1.0f)) * radiusmult * cg_SFXSabersCoreSize.value;
...
void CG_DoEp3Saber(vec3_t blade_muz, vec3_t blade_tip, vec3_t trail_tip, vec3_t trail_muz, float lengthMax, float radius, saber_colors_t color, int rfx, qboolean doLight)
{
	vec3_t	dif, mid, blade_dir, end_dir, trail_dir, base_dir;
	float	radiusmult, effectradius, coreradius, effectalpha, AngleScale;
	float	blade_len, end_len, trail_len, base_len, DisTip, DisMuz, DisDif;
...
void CG_DoOTSaber(vec3_t blade_muz, vec3_t blade_tip, vec3_t trail_tip, vec3_t trail_muz, float lengthMax, float radius, saber_colors_t color, int rfx, qboolean doLight, qboolean doTrail, int cnum, int bnum)
{
	...
	effectradius = ((radius * 1.6f * v1) + flrand(0.3f, 1.8f)) * radiusmult * cg_SFXSabersGlowSize.value;
	coreradius = ((radius * 0.4f * v2) + flrand(0.1f, 1.0f)) * radiusmult * cg_SFXSabersCoreSize.value;
...
void CG_DoCustomSaber(vec3_t blade_muz, vec3_t blade_tip, vec3_t trail_tip, vec3_t trail_muz, float lengthMax, float radius, saber_colors_t color, int rfx, qboolean doLight)
{
	vec3_t	dif, mid, blade_dir, end_dir, trail_dir, base_dir;
	float	radiusmult, effectradius, coreradius, effectalpha, AngleScale;
	float	blade_len, end_len, trail_len, base_len, DisTip, DisMuz, DisDif;
...
static void CG_DoSaber(vec3_t origin, vec3_t dir, float length, float lengthMax, float radius, saber_colors_t color, int rfx, qboolean doLight)
{
	...
	refEntity_t saber;
	float	radiusmult, effectradius, coreradius, effectalpha, AngleScale;
	float	blade_len, end_len, base_len, trail_len, DisTip, DisMuz, DisDif;
	float	glowscale = 0.5;
	vec3_t rgb = { 1,1,1 };
	float 	v1, v2, len;
	int i;
...
static void CG_DoTFA2Saber(vec3_t origin, vec3_t dir, float length, float lengthMax, float radius, saber_colors_t color, int rfx, qboolean doLight)
{
	vec3_t	    dif, mid, blade_dir, trail_tip, trail_muz, trail_dir, end_dir, base_dir;
	qhandle_t	blade = 0, glow = 0, ignite = 0;
	refEntity_t saber;
...
static void CG_AddSaberBladeGo(centity_t * cent, centity_t * scent, refEntity_t * saber, int renderfx, int modelIndex, vec3_t origin, vec3_t angles, int saberNum, int bladeNum)
{
	...
	if (cg_SFXSabers.integer < 1)
	{
		...
		if (cent->gent->client->ps.saber[saberNum].type == SABER_UNSTABLE || cent->gent->client->ps.saber[saberNum].type == SABER_STAFF_UNSTABLE || cent->gent->client->ps.saber[saberNum].type == SABER_ELECTROSTAFF)
		{
			CG_DoTFA2Saber(org_, axis_[0], length, client->ps.saber[saberNum].blade[bladeNum].lengthMax, client->ps.saber[saberNum].blade[bladeNum].radius, client->ps.saber[saberNum].blade[bladeNum].color, renderfx, (qboolean)(noDlight == qfalse));
		}
		else if ((cent->currentState.powerups & (1 << PW_CLOAKED)) || (cent->currentState.powerups & (1 << PW_UNCLOAKING)))
		{
			CG_DoSaber(org_, axis_[0], length, client->ps.saber[saberNum].blade[bladeNum].lengthMax, client->ps.saber[saberNum].blade[bladeNum].radius, client->ps.saber[saberNum].blade[bladeNum].color, renderfx, (qboolean)(noDlight == qfalse));
		}
		else
		{
			CG_DoSaber(org_, axis_[0], length, client->ps.saber[saberNum].blade[bladeNum].lengthMax, client->ps.saber[saberNum].blade[bladeNum].radius, client->ps.saber[saberNum].blade[bladeNum].color, renderfx, (qboolean)(noDlight == qfalse));
	...
	else
	{
		...
		{
			CG_DoCustomSaber(saberTrail->base, saberTrail->tip, saberTrail->dualtip, saberTrail->dualbase, (client->ps.saber[saberNum].blade[bladeNum].lengthMax), (client->ps.saber[saberNum].blade[bladeNum].radius), client->ps.saber[saberNum].blade[bladeNum].color, renderfx, (qboolean)(noDlight == qfalse));
		}
		else if (!(cent->gent->client->ps.saber[saberNum].type == SABER_SITH_SWORD || client->ps.saber[saberNum].saberFlags2 & SFL2_NO_BLADE))
		{
			...
			case 1:
				if (cent->gent->client->ps.saber[saberNum].type == SABER_UNSTABLE || cent->gent->client->ps.saber[saberNum].type == SABER_STAFF_UNSTABLE || cent->gent->client->ps.saber[saberNum].type == SABER_ELECTROSTAFF)
				{
					CG_DoTFASaber(saberTrail->base, saberTrail->tip, saberTrail->dualtip, saberTrail->dualbase, (client->ps.saber[saberNum].blade[bladeNum].lengthMax), (client->ps.saber[saberNum].blade[bladeNum].radius), client->ps.saber[saberNum].blade[bladeNum].color, renderfx, (qboolean)(noDlight == qfalse));
				}
				else if (cent->gent->client->ps.saber[saberNum].type == SABER_THIN || cent->gent->client->ps.saber[saberNum].type == SABER_STAFF_THIN)
				{
					CG_DoEp3Saber(saberTrail->base, saberTrail->tip, saberTrail->dualtip, saberTrail->dualbase, (client->ps.saber[saberNum].blade[bladeNum].lengthMax), (client->ps.saber[saberNum].blade[bladeNum].radius), client->ps.saber[saberNum].blade[bladeNum].color, renderfx, (qboolean)(noDlight == qfalse));
				}
				else if (cent->gent->client->ps.saber[saberNum].type == SABER_SFX || cent->gent->client->ps.saber[saberNum].type == SABER_STAFF_SFX)
				{
					CG_DoSFXSaber(saberTrail->base, saberTrail->tip, saberTrail->dualtip, saberTrail->dualbase, (client->ps.saber[saberNum].blade[bladeNum].lengthMax), (client->ps.saber[saberNum].blade[bladeNum].radius), client->ps.saber[saberNum].blade[bladeNum].color, renderfx, (qboolean)(noDlight == qfalse));
				}
				else if (cent->gent->client->ps.saber[saberNum].type == SABER_CUSTOMSFX)
				{
					CG_DoCustomSaber(saberTrail->base, saberTrail->tip, saberTrail->dualtip, saberTrail->dualbase, (client->ps.saber[saberNum].blade[bladeNum].lengthMax), (client->ps.saber[saberNum].blade[bladeNum].radius), client->ps.saber[saberNum].blade[bladeNum].color, renderfx, (qboolean)(noDlight == qfalse));
				...
				else if (cent->gent->client->ps.saber[saberNum].type == SABER_GRIE || cent->gent->client->ps.saber[saberNum].type == SABER_GRIE4)
				{
					CG_DoBattlefrontSaber(saberTrail->base, saberTrail->tip, saberTrail->dualtip, saberTrail->dualbase, (client->ps.saber[saberNum].blade[bladeNum].lengthMax), (client->ps.saber[saberNum].blade[bladeNum].radius), client->ps.saber[saberNum].blade[bladeNum].color, renderfx, (qboolean)(noDlight == qfalse));
				}
				else
				{
					CG_DoBattlefrontSaber(saberTrail->base, saberTrail->tip, saberTrail->dualtip, saberTrail->dualbase, (client->ps.saber[saberNum].blade[bladeNum].lengthMax), (client->ps.saber[saberNum].blade[bladeNum].radius), client->ps.saber[saberNum].blade[bladeNum].color, renderfx, (qboolean)(noDlight == qfalse));
			...
			case 3:
				...
				{
					CG_DoEp1Saber(saberTrail->base, saberTrail->tip, saberTrail->dualtip, saberTrail->dualbase, (client->ps.saber[saberNum].blade[bladeNum].lengthMax), (client->ps.saber[saberNum].blade[bladeNum].radius), client->ps.saber[saberNum].blade[bladeNum].color, renderfx, (qboolean)(noDlight == qfalse));
				}
				break;
			case 4:
				if (cent->gent->client->ps.saber[saberNum].type == SABER_UNSTABLE || cent->gent->client->ps.saber[saberNum].type == SABER_STAFF_UNSTABLE || cent->gent->client->ps.saber[saberNum].type == SABER_ELECTROSTAFF)
				...
				{
					CG_DoEp2Saber(saberTrail->base, saberTrail->tip, saberTrail->dualtip, saberTrail->dualbase, (client->ps.saber[saberNum].blade[bladeNum].lengthMax), (client->ps.saber[saberNum].blade[bladeNum].radius), client->ps.saber[saberNum].blade[bladeNum].color, renderfx, (qboolean)(noDlight == qfalse));
				}
				break;
			case 5:
				if (cent->gent->client->ps.saber[saberNum].type == SABER_UNSTABLE || cent->gent->client->ps.saber[saberNum].type == SABER_STAFF_UNSTABLE || cent->gent->client->ps.saber[saberNum].type == SABER_ELECTROSTAFF)
				...
				for (int bladeNum = 0; bladeNum < cent->gent->client->ps.saber[saberNum].numBlades; bladeNum++)
				{
					...
					{//saber blade is off
						if (cent->gent->client->ps.saber[saberNum].blade[bladeNum].length > 0)
						{
							...
							{
								cent->gent->client->ps.saber[saberNum].blade[bladeNum].length -= cent->gent->client->ps.saber[saberNum].blade[bladeNum].lengthMax / 10 * cg.frametime / 300 * cg_ignitionSpeedvader.value;
							}
							else
						...
						if (cent->gent->client->ps.saber[saberNum].blade[bladeNum].length < cent->gent->client->ps.saber[saberNum].blade[bladeNum].lengthMax)
						{
							...
							if (cg.frametime > 0)
							{
								...
								else
								{
									...
									{
									case SS_FAST:
										cent->gent->client->ps.saber[saberNum].blade[bladeNum].length += cent->gent->client->ps.saber[saberNum].blade[bladeNum].lengthMax / 5 * cg.frametime / 50 * cg_ignitionSpeed.value;
										break;
									case SS_STRONG:
										cent->gent->client->ps.saber[saberNum].blade[bladeNum].length += cent->gent->client->ps.saber[saberNum].blade[bladeNum].lengthMax / 10 * cg.frametime / 250 * cg_ignitionSpeed.value;
										break;
									case SS_MEDIUM:
										cent->gent->client->ps.saber[saberNum].blade[bladeNum].length += cent->gent->client->ps.saber[saberNum].blade[bladeNum].lengthMax / 10 * cg.frametime / 100 * cg_ignitionSpeed.value;
										break;
									case SS_DESANN:
										cent->gent->client->ps.saber[saberNum].blade[bladeNum].length += cent->gent->client->ps.saber[saberNum].blade[bladeNum].lengthMax / 10 * cg.frametime / 300 * cg_ignitionSpeed.value;
										break;
									case SS_TAVION:
										cent->gent->client->ps.saber[saberNum].blade[bladeNum].length += cent->gent->client->ps.saber[saberNum].blade[bladeNum].lengthMax / 10 * cg.frametime / 125 * cg_ignitionSpeed.value;
										break;
									case SS_DUAL:
										cent->gent->client->ps.saber[saberNum].blade[bladeNum].length += cent->gent->client->ps.saber[saberNum].blade[bladeNum].lengthMax / 10 * cg.frametime / 200 * cg_ignitionSpeed.value;
										break;
									case SS_STAFF:
										cent->gent->client->ps.saber[saberNum].blade[bladeNum].length += cent->gent->client->ps.saber[saberNum].blade[bladeNum].lengthMax / 10 * cg.frametime / 350 * cg_ignitionSpeedstaff.value;
										break;
									default:
										cent->gent->client->ps.saber[saberNum].blade[bladeNum].length += cent->gent->client->ps.saber[saberNum].blade[bladeNum].lengthMax / 10 * cg.frametime / 100 * cg_ignitionSpeed.value;
										break;
			...
			{//doing the electrocuting
				...
				{//arc
					...
					else if (cent->gent->client->NPC_class == CLASS_ALORA)
					{
						...
						{
							theFxScheduler.PlayEffect(cgs.effects.purplelightningwide, cent->gent->client->renderInfo.handLPoint, fxAxis);
						}
					}
					...
					{//jackin' 'em up, Palpatine-style
						...
						else if (cent->gent->client->NPC_class == CLASS_ALORA)
						{
							...
							{
								theFxScheduler.PlayEffect(cgs.effects.purplelightningwide, cent->gent->client->renderInfo.handRPoint, fxAxis);
							}
						}
						else if (cent->gent->client->NPC_class == CLASS_TAVION)
						{
				...
				{//line
					...
					{//jackin' 'em up, Palpatine-style
						...
						else if (cent->gent->client->NPC_class == CLASS_ALORA)
						{
							...
							{
								theFxScheduler.PlayEffect(cgs.effects.purplelightning, cent->gent->client->renderInfo.handRPoint, fxDir);
							}
						}
						else if (cent->gent->client->NPC_class == CLASS_TAVION)
						{
					...
					else if (cent->gent->client->NPC_class == CLASS_ALORA)
					{
						...
						{
							theFxScheduler.PlayEffect(cgs.effects.purplelightning, cent->gent->client->renderInfo.handLPoint, fxDir);
						}
					}
					else if (cent->gent->client->NPC_class == CLASS_TAVION)
					{
