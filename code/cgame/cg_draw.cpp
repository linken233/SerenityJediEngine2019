...
static void CG_Drawweapontype(const centity_t * cent, const int xPos, const int yPos)
{
	...
	if (cent->currentState.weapon == WP_BLASTER_PISTOL)
	{
...
static void CG_Drawsaberfatigue(const centity_t * cent, const int xPos, const int yPos)
{
	...
	vec4_t		calcColor;
	float		value, inc, percent;
	const int	saberAttackChainCount = MISHAPLEVEL_OVERLOAD;
	qboolean	flash = qfalse;
...
static void CG_DrawHUD(centity_t * cent)
{
	...
	if (cgi_UI_GetMenuInfo("lefthud", &sectionXPos, &sectionYPos, &sectionWidth, &sectionHeight) && !(G_IsRidingVehicle(cent->gent)))
	{
	...
	if (cgi_UI_GetMenuInfo("righthud", &sectionXPos, &sectionYPos, &sectionWidth, &sectionHeight) && !(G_IsRidingVehicle(cent->gent)))
	{
		// Draw all the HUD elements
		...
		if (cg_drawStatus.integer == 2)
		{
			if (cent->currentState.weapon != WP_SABER && cent->currentState.weapon != WP_STUN_BATON && cent->currentState.weapon != WP_MELEE && cent->gent)
			{
...
static void CG_DrawCrosshair(vec3_t worldPoint)
{
	...
	else
	{
		...
		cgi_R_DrawStretchPic(x + cg.refdef.x + 0.5f * (640 - w), y + cg.refdef.y + 0.5f * (480 - h), w, h, 0, 0, 1, 1, hShader);
	}
...
=====================
*/
static void CG_DrawCrosshairNamesMD(void)
{
...
static void CG_DrawCrosshairNames(void)
{
	...
	{
		CG_DrawBigStringColor(320 - w / 2, 170, va("Press Use to Interact"), colorTable[CT_LTGREY]);
		return;
	}
		...
		if (crossEnt->client)
		{
			...
			{//Allies 
				...
				case CLASS_REBEL:
					...
					{
						CG_DrawBigStringColor(320 - w / 2, 170, va("Chewbacca"), colorTable[CT_VLTPURPLE1]);
					}
				...
				case CLASS_SBD:
					...
					break;
				default:
					CG_DrawBigStringColor(320 - w / 2, 170, va("Alliance"), colorTable[CT_GREEN]);
					break;
				}
			}
			else if (crossEnt->client->playerTeam == TEAM_ENEMY)
			{
				...
				case CLASS_SBD:
					...
					break;
				default:
					CG_DrawBigStringColor(320 - w / 2, 170, va("Empire"), colorTable[CT_GREEN]);
					break;
				}
			}
			else if (crossEnt->client->playerTeam == TEAM_NEUTRAL || crossEnt->client->playerTeam == TEAM_FREE || crossEnt->client->playerTeam == TEAM_SOLO)
			{
				switch (crossEnt->client->NPC_class)
				{
				case CLASS_REBEL:
				...
				case CLASS_SBD:
					...
					break;
				default:
					CG_DrawBigStringColor(320 - w / 2, 170, va("Civillian"), colorTable[CT_GREEN]);
					break;
				}
			}
			else
			{
				CG_DrawBigStringColor(320 - w / 2, 170, va("Civilian"), colorTable[CT_YELLOW]);
			}
		}
	}
...
static void CG_Draw2D(void)
{
	...
	if (cg.snap->ps.stats[STAT_HEALTH] > 0)
	{
		if (!(cent->gent && cent->gent->s.eFlags & (EF_LOCKED_TO_WEAPON | EF_IN_ATST)) && !(G_IsRidingVehicle(cent->gent)))
		{
...
void CG_DrawIconBackground(void)
{
	...
	{
		if (background && cg.iconHUDActive)		// The time is up, but we still need to move the prongs back to their original position
		{
