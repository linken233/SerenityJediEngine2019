...
static void CG_General(centity_t *cent)
{
	...
	*/
	...
	if (cent->currentState.weapon == WP_MELEE)
	{
		...
		gi.G2API_GetBoltMatrix(parent->gent->ghoul2, 0, 0, &mat, parent->lerpAngles, parent->lerpOrigin, cg.time, cgs.model_draw, cent->currentState.modelScale);
		gi.G2API_GiveMeVectorFromMatrix(mat, ORIGIN, start);
		if (cent->currentState.eType == ET_GRAPPLE)
		{
			CG_GrappleLine(start, end, 1, color, 1);
		}
		//GOING OUT
	}
...
static void CG_Missile(centity_t *cent)
{
	...
	*/
	if (s1->weapon == WP_MELEE)
	{
		...
		gi.G2API_GetBoltMatrix(parent->gent->ghoul2, 1, 0, &mat, parent->lerpAngles, parent->lerpOrigin, cg.time, cgs.model_draw, cent->currentState.modelScale);
		gi.G2API_GiveMeVectorFromMatrix(mat, ORIGIN, start);
		if (cent->currentState.eType == ET_GRAPPLE)
		{
			CG_GrappleLine(start, end, 1, color, 1);
...
static void CG_Mover(centity_t *cent)
{
	...
	{//I'm the hyperspace brush
		...
		if (cent->currentState.m_iVehicleNum
			&& cent->currentState.hyperSpaceTime
			&& (cg.time - cent->currentState.hyperSpaceTime) < HYPERSPACE_TIME
			&& (cg.time - cent->currentState.hyperSpaceTime) > 1000)
		{
			if (cg.snap
				&& cg.snap->ps.pm_type == PM_INTERMISSION)
			{//in the intermission, stop drawing hyperspace ent
			}
			else if ((cent->currentState.eFlags2&EF2_HYPERSPACE))
			{//actually hyperspacing now
				float timeFrac = ((float)(cg.time - cent->currentState.hyperSpaceTime - 1000)) / (HYPERSPACE_TIME - 1000);
				...
				{//still in hyperspace or just popped out
					...
					VectorSet(cent->lerpAngles, cg.predicted_player_state.viewangles[PITCH], cg.predicted_player_state.viewangles[YAW] - 90.0f, 0);//cos( ( cg.time + 1000 ) *  scale ) * 4 );
	...
	if (cent->gent && cent->gent->owner && cent->gent->owner->ghoul2.size())
	{
		...
		else
		{
			...
			if (owner->client->NPC_class == CLASS_PROTOCOL
				|| debug_subdivision->integer && !in_camera
				|| g_saberRealisticCombat->integer && !in_camera)
