...
void CG_GrappleTrail(centity_t* ent, const weaponInfo_t* wi)
{
	...
	FX_AddLine(from, to, 0.1f, 0.5f, 0.0f, 0.5f, 0.0f, BLUER, BLUER, 15, cgi_R_RegisterShader("gfx/misc/bolt1"), FX_SIZE_LINEAR);
}
...
static float	CG_MachinegunSpinAngle(centity_t * cent)
{
	...
	if (cent->pe.barrelSpinning == !(cent->currentState.eFlags & EF_FIRING))
	{
		...
		if (cent->currentState.weapon == WP_REPEATER && !cent->pe.barrelSpinning)
		{
		...
		{
			cgi_S_StartSound(NULL, cent->currentState.number, CHAN_WEAPON, cgi_S_RegisterSound("sound/weapons/barrelSpinning/barrelspindown.wav"));
		}
	}
...
void CG_AddViewWeapon(playerState_t * ps)
{
	...
	for (int saberNum = 0; saberNum < numSabers; saberNum++)
	{
		...
		//---------if saberInFlight, need to draw empty hand guiding it
...
void CG_MissileHitPlayer(centity_t * cent, int weapon, vec3_t origin, vec3_t dir, qboolean altFire)
{
	...
	case WP_EMPLACED_GUN:
		FX_EmplacedHitPlayer(other, origin, dir, (qboolean)(cent->gent && cent->gent->alt_fire));
		break;
